...
export interface ReScriptFeatureFlags {
  ...
  global: {
    ...
    fallbackOnError: boolean; // Use TypeScript if ReScript throws
 ...
 */
const DEFAULT_FLAGS: ReScriptFeatureFlags = {
  timestamp: {
    enabled: false,
    rolloutPercentage: 0,
    enableLogging: true,
 ...
 */
let currentFlags: ReScriptFeatureFlags = { ...DEFAULT_FLAGS };

/**
 * Get current feature flags
 */
export function getFeatureFlags(): Readonly<ReScriptFeatureFlags> {
  return currentFlags;
}
...
export function updateFeatureFlags(
  updates: Partial<ReScriptFeatureFlags>
): void {
  currentFlags = {
    ...currentFlags,
    ...updates,
 ...
 */
export function shouldUseReScript(
  feature: keyof Omit<ReScriptFeatureFlags, 'global'>,
  userId?: string
): boolean {
  // Check global switch
 ...
 */
export function applyRolloutPreset(
  preset: keyof typeof ROLLOUT_PRESETS
): void {
...
export function getCurrentPhase(): string {
  ...
  if (pct === 10) return 'phase1_10percent';
  if (pct === 50) return 'phase2_50percent';
  if (pct === 100) {
...
export function getFeatureFlagStats(): {
  ...
  features: Record<string, { enabled: boolean; rollout: number }>;
} {
  const flags = getFeatureFlags();
  return {
