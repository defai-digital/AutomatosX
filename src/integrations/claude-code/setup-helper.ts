...
export class ClaudeCodeSetupHelper {
  ...
  async areManifestsGenerated(): Promise<boolean> {
    try {
      ...
      await access(join(this.projectDir, '.claude', 'agents', 'automatosx-coordinator.md'), constants.R_OK);
      return true;
    } catch {
      return false;
    }
  ...
  async validateManifests(): Promise<{ valid: boolean; errors: string[] }> {
    ...
    try {
      ...
      const subAgentPath = join(this.projectDir, '.claude', 'agents', 'automatosx-coordinator.md');
  ...
  async runDiagnostics(): Promise<ClaudeCodeDiagnostics> {
    ...
    const diagnostics: ClaudeCodeDiagnostics = {
      ...
      mcpProjectConfigGenerated: false,
    ...
    if (!diagnostics.manifestsGenerated) {
      diagnostics.manifestsValid = false;
    ...
    // Check settings.json
    const settingsMgr = new SettingsManager(this.projectDir);
    diagnostics.settingsConfigured = await settingsMgr.hasAutomatosXPermissions();
    if (!diagnostics.settingsConfigured) {
      diagnostics.warnings.push(
        'MCP permissions not configured in .claude/settings.json. Run: ax setup --claude-code'
      );
    }

    // Check .mcp.json
    const mcpGen = new McpProjectConfigGenerator(this.projectDir);
    diagnostics.mcpProjectConfigGenerated = await mcpGen.isGenerated();
    if (!diagnostics.mcpProjectConfigGenerated) {
      diagnostics.warnings.push(
        '.mcp.json not generated. Run: ax setup --claude-code'
      );
    }

    // Check hooks
    const hooksGen = new HooksGenerator({ projectDir: this.projectDir });
    diagnostics.hooksGenerated = await hooksGen.areHooksGenerated();
    if (!diagnostics.hooksGenerated) {
      diagnostics.warnings.push(
        'Hook scripts not generated. Run: ax setup --claude-code'
      );
    }
  ...
  /**
   * Write MCP permissions into .claude/settings.json (merge, never overwrite).
   */
  async configureSettings(): Promise<void> {
    logger.info('[Claude Code Setup] Configuring .claude/settings.json...');
    const mgr = new SettingsManager(this.projectDir);
    await mgr.writeMcpPermissions();
    logger.info('[Claude Code Setup] settings.json configured');
  }

  /**
   * Generate .mcp.json at the project root for team-shared MCP config.
   */
  async generateMcpProjectConfig(): Promise<void> {
    logger.info('[Claude Code Setup] Generating .mcp.json...');
    const gen = new McpProjectConfigGenerator(this.projectDir);
    await gen.generate();
    logger.info('[Claude Code Setup] .mcp.json generated');
  }

  /**
   * Generate hook scripts in .claude/hooks/ and write hook config into
   * .claude/settings.json.
   */
  async generateHooks(force = false): Promise<void> {
    logger.info('[Claude Code Setup] Generating hooks...');
    const gen = new HooksGenerator({ projectDir: this.projectDir, force });
    const result = await gen.generate();
    logger.info('[Claude Code Setup] Hooks generated', {
      written: result.scriptsWritten,
      skipped: result.scriptsSkipped,
    });
  }
  ...
  async setup(): Promise<void> {
    ...
    // Step 2: Generate manifests (skills, commands, coordinator, per-agent subagents)
    await this.generateManifests();

    // Step 3: Write .claude/settings.json with MCP permissions
    await this.configureSettings();

    // Step 4: Generate hook scripts + hook config in settings.json
    await this.generateHooks();

    // Step 5: Generate .mcp.json (team-shared, committable)
    await this.generateMcpProjectConfig();

    // Step 6: Register MCP server with Claude Code CLI
