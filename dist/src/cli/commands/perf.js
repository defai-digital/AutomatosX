...
import { createBenchmarkHarness, BenchmarkHarness, } from '../../performance/BenchmarkHarness.js';
        ...
        try {
            ...
            if (baselineData.results && currentData.results) {
                ...
                for (let i = 0; i < Math.min(baselineData.results.length, currentData.results.length); i++) {
                    ...
                    console.log(`  Mean: ${comparison.meanImprovement > 0 ? '✅' : '⚠️'} ${comparison.meanImprovement.toFixed(2)}% ${comparison.meanImprovement > 0 ? 'faster' : 'slower'}`);
                    console.log(`  P95: ${comparison.p95Improvement > 0 ? '✅' : '⚠️'} ${comparison.p95Improvement.toFixed(2)}% ${comparison.p95Improvement > 0 ? 'faster' : 'slower'}`);
                    console.log(`  Throughput: ${comparison.throughputImprovement > 0 ? '✅' : '⚠️'} ${comparison.throughputImprovement.toFixed(2)}% ${comparison.throughputImprovement > 0 ? 'higher' : 'lower'}`);
