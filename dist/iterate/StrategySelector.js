...
import { SimpleRetryStrategy, ExponentialBackoffStrategy, DifferentProviderStrategy, DifferentAgentStrategy, SimplifyTaskStrategy, IncrementalRetryStrategy, AdaptiveParallelismStrategy, CircuitBreakerStrategy, GradualRelaxationStrategy, HybridApproachStrategy } from './IterateStrategy.js';
/**
 * Strategy Selector
 *
...
export class StrategySelector {
     ...
     */
    select(pattern, task, currentStrategy) {
        // Filter applicable strategies
        const applicable = this.strategies.filter(s => s.isApplicable(pattern, task) && s.name !== currentStrategy?.name);
     ...
     */
    scoreStrategy(strategy, pattern, task) {
        let score = 0;
     ...
     */
    isPatternMatch(strategy, pattern) {
        const matches = {
     ...
     */
    isComplexityMatch(strategy, task) {
     ...
     */
    getSelectionReason(strategy, pattern) {
        const reasons = {
