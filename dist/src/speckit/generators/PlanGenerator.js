...
export class PlanGenerator {
     ...
     */
    async generatePlan(workflow, options = {}) {
        if (options.verbose) {
        ...
        if (graph.hasCycles()) {
            ...
            throw new Error(`Workflow contains circular dependencies: ${cycles.map(c => c.join(' → ')).join('; ')}`);
        }
        if (options.verbose) {
     ...
     */
    generatePhases(workflow, graph, options) {
        ...
        for (let i = 0; i < levels.length; i++) {
            ...
            const phaseDuration = Math.max(...stepIds.map(id => {
     ...
     * Generate phase description
     */
    generatePhaseDescription(steps, parallelizable) {
        if (steps.length === 1) {
     ...
     * Calculate resource requirements
     */
    calculateResourceRequirements(workflow, graph, costEstimate) {
        // Count API calls (one per step)
        const apiCalls = workflow.steps.length;
        // Estimate tokens
        const tokens = costEstimate?.totalTokens || 0;
        // Get required agents
        const agents = Array.from(new Set(workflow.steps.map(s => s.agent)));
        // Calculate max concurrency
        const parallelizable = graph.getParallelizableNodes();
        const maxConcurrency = Math.max(...Array.from(parallelizable.values()).map(nodes => nodes.length), 1);
        return {
            apiCalls,
            tokens,
            agents,
     ...
     */
    assessRisks(workflow, graph, phases) {
        const risks = [];
        ...
        const maxDeps = Math.max(...workflow.steps.map(s => s.dependsOn?.length || 0));
        if (maxDeps > 5) {
            risks.push({
                level: 'high',
                category: 'Complexity',
        ...
        const stepsWithoutRetry = workflow.steps.filter(s => !s.retryConfig || s.retryConfig.maxRetries === 0);
     ...
     */
    generateSummary(workflow, phases, totalDuration, totalCost) {
        const lines = [];
     ...
     * Write plan to file
     */
    async writePlan(plan, workflow, options) {
     ...
     */
    generateMarkdown(plan, workflow, options) {
        const lines = [];
        // Header
