...
export class MCPManager {
  ...
  async getMCPStats(projectDir: string): Promise<MCPDiscoveryStats> {
    ...
    // Count by transport — read from server config or default to stdio
  ...
  /**
   * Register a remote HTTP MCP server in the project's .claude/mcp/ manifest.
   *
   * @param projectDir - Project directory
   * @param name - Server name (e.g. "automatosx-remote")
   * @param url - HTTP endpoint URL
   * @param headers - Optional auth headers (e.g. Authorization)
  ...
  async registerHttpServer(
    ...
    const manifestPath = join(mcpDir, `${name}.json`);
    const server: ClaudeMCPServer = { type: 'http', url, ...(headers && { headers }) };
    const validation = validateMCPServer(server, name);

    if (!validation.valid) {
      throw new ClaudeCodeError(
        ClaudeCodeErrorType.VALIDATION_ERROR,
        `Invalid HTTP server config for "${name}": ${validation.errors.join(', ')}`,
        { name, url, errors: validation.errors }
      );
    }
