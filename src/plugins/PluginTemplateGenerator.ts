...
export class PluginTemplateGenerator extends EventEmitter {
  private templates = new Map<PluginCategory, Map<PluginLanguage, TemplateFile[]>>()
   ...
   */
  private interpolateTemplate(template: string, options: TemplateOptions): string {
    return template
   ...
   * Generate test files
   */
  private generateTestFiles(options: TemplateOptions): TemplateFile[] {
...
`,
   ...
   */
  private generateDocFiles(options: TemplateOptions): TemplateFile[] {
    return [
      {
        path: 'README.md',
...
`,
   ...
   */
  private generateExampleFiles(options: TemplateOptions): TemplateFile[] {
    const ext = options.language === PluginLanguage.TYPESCRIPT ? 'ts' : 'js'
 ...
 */
export function createTemplateGenerator(): PluginTemplateGenerator {
 ...
 */
let globalGenerator: PluginTemplateGenerator | null = null

/**
 * Get global template generator
 */
export function getGlobalGenerator(): PluginTemplateGenerator {
  if (!globalGenerator) {
