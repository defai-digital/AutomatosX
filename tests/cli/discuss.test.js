...
describe('Discuss Command', () => {
    ...
    describe('Validation', () => {
        ...
        it('should fail with unknown provider', async () => {
            const result = await discussCommand(['--providers', 'unknown-provider,claude', 'Test topic'], defaultOptions);
            expect(result.success).toBe(false);
            expect(result.message).toContain('Unknown provider');
            expect(result.message).toContain('unknown-provider');
        });
        it('should fail with only one provider', async () => {
            const result = await discussCommand(['--providers', 'claude', 'Test topic'], defaultOptions);
            expect(result.success).toBe(false);
    ...
    describe('Argument Parsing', () => {
        ...
        it('should parse topic from positional argument', async () => {
            ...
            const result = await discussCommand(['--providers', 'invalid1,invalid2', 'What is the best programming language?'], defaultOptions);
            // Should fail on invalid provider, not on topic parsing
            expect(result.success).toBe(false);
            expect(result.message).toContain('Unknown provider');
        });
        it('should parse multi-word topic from multiple args', async () => {
            const result = await discussCommand(['--providers', 'invalid1,invalid2', 'What', 'is', 'the', 'best', 'approach?'], defaultOptions);
            expect(result.success).toBe(false);
            expect(result.message).toContain('Unknown provider');
        });
        it('should parse --providers flag', async () => {
            const result = await discussCommand(['--providers', 'invalid1,invalid2', 'Test topic'], defaultOptions);
            expect(result.success).toBe(false);
        ...
        it('should parse --pattern flag with invalid providers', async () => {
            const result = await discussCommand(['--pattern', 'voting', '--providers', 'invalid1,invalid2', 'Which is better?'], defaultOptions);
            expect(result.success).toBe(false);
        });
        it('should parse --rounds flag with invalid providers', async () => {
            const result = await discussCommand(['--rounds', '3', '--providers', 'invalid1,invalid2', 'Test topic'], defaultOptions);
            expect(result.success).toBe(false);
        });
        it('should parse --consensus flag with invalid providers', async () => {
            const result = await discussCommand(['--consensus', 'voting', '--providers', 'invalid1,invalid2', 'Test topic'], defaultOptions);
            expect(result.success).toBe(false);
        });
    });
    // ============================================================================
...
//# sourceMappingURL=discuss.test.js.map