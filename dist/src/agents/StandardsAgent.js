...
export class StandardsAgent extends AgentBase {
    ...
    buildStandardsPrompt(task, context, codeContext, pastSolutions) {
    ...
    parseStandardsArtifacts(response) {
        const artifacts = [];
        ...
        while ((match = codeBlockRegex.exec(response)) !== null) {
            ...
            if (language === 'markdown' || language === 'md') {
                if (content.includes('# Compliance') || content.includes('# Standards'))
                    name = 'compliance-checklist';
                else if (content.includes('# WCAG') || content.includes('# Accessibility'))
                    name = 'accessibility-guide';
                else if (content.includes('# GDPR') || content.includes('# Privacy'))
                    name = 'privacy-compliance';
                else if (content.includes('# SOC2') || content.includes('# ISO'))
                    name = 'security-compliance';
                else if (content.includes('# Best Practices') || content.includes('# Guidelines'))
                    name = 'best-practices';
                else if (content.includes('# Code Standards') || content.includes('# Style Guide'))
                    name = 'code-standards';
            }
            else if (language === 'json' || language === 'yaml') {
                if (content.includes('eslint') || content.includes('rules'))
                    name = 'eslint-config';
                else if (content.includes('prettier'))
...
//# sourceMappingURL=StandardsAgent.js.map