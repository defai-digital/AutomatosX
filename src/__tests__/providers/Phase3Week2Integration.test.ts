...
describe('Phase 3 Week 2 Integration Tests', () => {
    ...
    beforeEach(() => {
      ...
      for (let i = 0; i < 20; i++) {
        stmt.run(
          ...
          800 + Math.random() * 200, // 800-1000ms
          1,
          1000,
          now - i * 60000
        );
      ...
      for (let i = 0; i < 20; i++) {
        stmt.run(
          ...
          'openai',
          'gpt-4-turbo-preview',
          1200 + Math.random() * 300, // 1200-1500ms
          1,
          1200,
          now - i * 60000
        );
  ...
  describe('Rate Limiting', () => {
    ...
    it('should cleanup old buckets', async () => {
      ...
      db.prepare(
        'UPDATE rate_limit_buckets SET last_refill = ? WHERE key LIKE ?'
      ).run(Date.now() - 48 * 60 * 60 * 1000, 'cleanup-%');
