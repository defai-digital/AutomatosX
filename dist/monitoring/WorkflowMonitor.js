...
export class WorkflowMonitor extends EventEmitter {
     ...
     */
    startExecution(workflowId, workflowName, tenantId, userId, stepsTotal, metadata) {
        const id = randomUUID();
        const now = Date.now();
        const execution = {
            id,
    ...
    `).run(id, workflowId, workflowName, tenantId, userId, 'running', now, stepsTotal, JSON.stringify(metadata || {}));
     ...
     */
    startStep(executionId, stepKey, stepName) {
        const id = randomUUID();
        const now = Date.now();
        const stepExecution = {
            id,
    ...
    `).get(stepId);
      ...
      SET status = 'completed',
          ...
          output_size = ?
      WHERE id = ?
    `).run(now, duration, result?.tokensUsed || null, result?.cost || null, result?.inputSize || null, result?.outputSize || null, stepId);
     ...
     */
    getRecentExecutions(limit = 100, tenantId) {
        let query = `
