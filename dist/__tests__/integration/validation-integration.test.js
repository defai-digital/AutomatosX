...
import { validateFileInput, validateSymbolInput, validateSymbolInputBatch, } from '../../types/schemas/database.schema.js';
...
beforeAll(() => {
  ...
  `);
    ...
    const result = insertFile.run(uniquePath, 'test content', 'test-hash', 12, 'typescript', now, now);
...
describe('Database Integration with Validation', () => {
    describe('FileDAO Integration', () => {
      ...
      `);
            const now = new Date().toISOString();
            const result = insertFile.run(fileInput.path, fileInput.content, 'hash123', fileInput.content.length, fileInput.language, now, now);
    ...
    describe('SymbolDAO Integration', () => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
            const result = insertSymbol.run(symbolInput.file_id, symbolInput.name, symbolInput.kind, symbolInput.line, symbolInput.column, symbolInput.end_line, symbolInput.end_column);
            expect(result.changes).toBe(1);
        });
    ...
    describe('Batch Insert with Validation', () => {
        ...
        VALUES (?, ?, ?, ?, ?)
      `);
            const insertTransaction = db.transaction((symbols) => {
            ...
            // Verify inserts
            const count = db.prepare('SELECT COUNT(*) as count FROM symbols WHERE file_id = ?').get(testFileId);
      ...
      `);
            expect(() => {
                insertSymbol.run(invalidInput.file_id, invalidInput.name, invalidInput.kind, invalidInput.line, invalidInput.column);
    ...
    describe('End-to-End: Parse → Validate → Insert', () => {
      ...
      `);
            const now = new Date().toISOString();
            const fileResult = insertFile.run(fileInput.path, fileInput.content, 'hash-e2e', fileInput.content.length, fileInput.language, now, now);
      ...
      `);
            for (const symbol of batchResult.validated) {
                insertSymbol.run(symbol.file_id, symbol.name, symbol.kind, symbol.line, symbol.column, symbol.end_line, symbol.end_column);
            }
            // 5. Verify data integrity
            const symbolCount = db.prepare('SELECT COUNT(*) as count FROM symbols WHERE file_id = ?').get(newFileId);
