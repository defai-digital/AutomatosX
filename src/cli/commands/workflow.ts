 ...
 */
async function runNormal(file: string, options: any): Promise<void> {
 ...
 */
async function runWithIterate(file: string, options: any): Promise<void> {
  ...
  const iterateOptions = {
    ...
    safetyLevel: (options.safety || 'normal') as 'permissive' | 'normal' | 'paranoid',
    maxCost: options.maxCost,
    timeout: options.timeout ? options.timeout * 1000 : undefined,
    verbose: true,
    onIteration: (iteration: any) => {
 ...
 */
function createCheckpointsCommand(): Command {
