...
export class WriterAgent extends AgentBase {
  ...
  private buildWriterPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseWriterArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'markdown' || language === 'md') {
        if (content.includes('# README') || content.toUpperCase().includes('README')) name = 'readme';
        else if (content.includes('# API') || content.includes('## Endpoints')) name = 'api-documentation';
        else if (content.includes('# Tutorial') || content.includes('# Guide')) name = 'tutorial';
        else if (content.includes('# Installation') || content.includes('# Setup')) name = 'installation-guide';
        else if (content.includes('# Changelog') || content.includes('# Release Notes')) name = 'changelog';
        else if (content.includes('# FAQ') || content.includes('# Troubleshooting')) name = 'troubleshooting';
      } else if (language === 'yaml' || language === 'yml') {
        if (content.includes('openapi') || content.includes('swagger')) name = 'openapi-spec';
      }
