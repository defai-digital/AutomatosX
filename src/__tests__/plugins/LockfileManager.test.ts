...
} from '../../plugins/LockfileManager.js'
import type { Lockfile, DependencyNode } from '../../plugins/types/Lockfile.js'
...
describe('LockfileManager', () => {
  ...
  describe('Read Lockfile', () => {
    ...
    it('should read valid lockfile', async () => {
      ...
      await fs.writeFile(
        join(tempDir, 'ax-plugin.lock'),
        JSON.stringify(mockLockfile),
        'utf-8'
      )
    ...
    it('should throw error on invalid lockfile schema', async () => {
      ...
      await fs.writeFile(
        join(tempDir, 'ax-plugin.lock'),
        JSON.stringify(invalidLockfile),
        'utf-8'
      )
  ...
  describe('Generate Lockfile', () => {
    ...
    it('should include child dependencies', async () => {
      ...
      expect(lockfile.dependencies['dep-a'].dependencies?.['dep-b']).toBe(
        '2.0.0'
      )
    })
  ...
  describe('Update Lockfile', () => {
    it('should throw error if no lockfile exists', async () => {
      ...
      await expect(manager.update(nodes)).rejects.toThrow(
        'No existing lockfile found'
      )
    })
    ...
    it('should update metadata timestamp', async () => {
      ...
      expect(updated.metadata?.generatedAt).not.toBe(
