...
import { DiscussionExecutor, createDiscussionExecutor, StubProviderExecutor, RoundRobinPattern, SynthesisPattern, DebatePattern, CritiquePattern, VotingPattern, SynthesisConsensus, VotingConsensus, ModeratorConsensus, getPatternExecutor, getConsensusExecutor, isPatternSupported, isConsensusMethodSupported, getSupportedPatterns, getSupportedConsensusMethods, interpolate, formatPreviousResponses, formatVotingOptions, formatVotes, getProviderSystemPrompt, } from '@defai.digital/discussion-domain';
import { createDefaultDiscussStepConfig, createDebateConfig, createVotingConfig, } from '@defai.digital/contracts';
// ============================================================================
...
describe('Pattern Factory', () => {
    ...
    it('should check if patterns are supported', () => {
        ...
        expect(isPatternSupported('unknown')).toBe(false);
    });
...
describe('Consensus Factory', () => {
    ...
    it('should check if consensus methods are supported', () => {
        ...
        expect(isConsensusMethodSupported('unknown')).toBe(false);
    });
...
describe('Prompt Templates', () => {
    ...
    describe('formatPreviousResponses', () => {
        ...
        it('should include role labels when provided', () => {
            const responses = [
                { provider: 'claude', content: 'For this position', role: 'proponent' },
                { provider: 'grok', content: 'Against this position', role: 'opponent' },
            ];
            const result = formatPreviousResponses(responses);
...
describe('Pattern Executors', () => {
    ...
    describe('VotingPattern', () => {
        it('should execute voting with options', async () => {
            ...
            const config = createVotingConfig('Which framework?', ['React', 'Vue', 'Angular'], ['claude', 'grok']);
            const context = {
                config,
                providerExecutor: stubExecutor,
                availableProviders: ['claude', 'grok'],
...
describe('DiscussionExecutor', () => {
    ...
    it('should execute voting', async () => {
        const result = await executor.vote('Which database to use?', ['PostgreSQL', 'MongoDB', 'MySQL']);
        expect(result.success).toBe(true);
...
//# sourceMappingURL=discussion-domain.test.js.map