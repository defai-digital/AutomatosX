...
describe('Trace Domain', () => {
  ...
  describe('Trace Recorder (INV-TR-001, INV-TR-003, INV-TR-005)', () => {
    ...
    it('INV-TR-001: should emit run.end as last event', async () => {
      ...
      expect((lastEvent?.payload as { success: boolean }).success).toBe(true);
    });

    it('INV-TR-001: should create complete event chain', async () => {
      const traceId = await recorder.startTrace('workflow-1');

      await recorder.recordRoutingDecision(
        traceId,
        'claude-3',
        'anthropic',
        'Best for task'
      );
      await recorder.recordStepExecution(
        traceId,
        'step-1',
        'prompt',
        { query: 'test' },
    ...
    it('INV-TR-003: should capture routing decisions for replay', async () => {
      ...
      expect((routingEvent?.payload as { selectedModel: string }).selectedModel).toBe('claude-3-opus');
      expect((routingEvent?.payload as { reasoning: string }).reasoning).toBe('Selected for code tasks');
    });
    ...
    it('INV-TR-005: should capture errors with full context', async () => {
      ...
      expect((errorEvent?.payload as { code: string }).code).toBe('STEP_FAILED');
      expect((errorEvent?.payload as { context: { stepId: string } }).context.stepId).toBe('step-1');
      expect((errorEvent?.payload as { stack: string }).stack).toBeDefined();
    });
    ...
    it('should prevent ending trace twice', async () => {
      ...
      await expect(recorder.endTrace(traceId, true)).rejects.toThrow(
        TraceRecorderError
      );
    });
