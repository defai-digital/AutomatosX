...
describe('StateMachine - Core Functionality', () => {
    ...
    describe('State Checking', () => {
        it('should correctly identify Idle state', () => {
            const machine = SM.create('Idle', [], undefined, undefined);
        ...
        it('should correctly identify Running state', () => {
            const taskData = { name: 'test', progress: 50 };
        ...
        it('should correctly identify Completed state', () => {
            const taskData = { name: 'test', progress: 100 };
    ...
    describe('Transitions - Basic', () => {
        it('should transition from Idle to Running without guard', async () => {
            const taskData = { name: 'task1', progress: 0 };
            const initialState = 'Idle';
            const targetState = { TAG: 'Running', _0: taskData };
            const transitions = [
                {
                    from: initialState,
                    event: 'start',
        ...
        it('should reject invalid transition in strict mode', async () => {
            ...
            const event = SM.createEvent('e1', 'invalid-event', undefined, undefined);
            const result = await SM.transition(machine, event);
        ...
        it('should record failed transition in history', async () => {
            const initialState = 'Idle';
            const machine = SM.create(initialState, [], { maxHistorySize: 100, enableLogging: false, strict: true }, undefined);
            const event = SM.createEvent('e1', 'invalid', undefined, undefined);
            await SM.transition(machine, event);
            expect(SM.getHistoryLength(machine)).toBe(1);
            const history = SM.getHistory(machine);
    ...
    describe('Transitions - With Guards', () => {
        it('should allow transition when guard returns true', async () => {
            const taskData = { name: 'task1', progress: 50 };
            const initialState = { TAG: 'Running', _0: taskData };
            const targetState = { TAG: 'Completed', _0: { ...taskData, progress: 100 } };
            const guard = (data) => data.progress >= 50;
            const transitions = [
                {
                    from: initialState,
                    event: 'complete',
        ...
        it('should block transition when guard returns false', async () => {
            const taskData = { name: 'task1', progress: 30 };
            const initialState = { TAG: 'Running', _0: taskData };
            const targetState = { TAG: 'Completed', _0: { ...taskData, progress: 100 } };
            const guard = (data) => data.progress >= 50;
            const transitions = [
            ...
            const transitions = [
                {
                    from: 'Idle',
                    event: 'start',
                    to: { TAG: 'Running', _0: taskData },
                    guard: undefined,
                    action: undefined,
                },
                {
                    from: { TAG: 'Running', _0: taskData },
                    event: 'pause',
                    to: { TAG: 'Paused', _0: taskData },
                    guard: undefined,
                    action: undefined,
                },
            ];
            const machine = SM.create('Idle', transitions, undefined, undefined);
    ...
    describe('Statistics', () => {
        it('should count total transitions', async () => {
            ...
            const machine = SM.create('Idle', transitions, undefined, undefined);
            await SM.transition(machine, SM.createEvent('e1', 'start', undefined, undefined));
            await SM.transition(machine, SM.createEvent('e2', 'invalid', undefined, undefined));
            const stats = SM.getStatistics(machine);
        ...
        it('should count failed transitions', async () => {
            const machine = SM.create('Idle', [], { maxHistorySize: 100, enableLogging: false, strict: true }, undefined);
            await SM.transition(machine, SM.createEvent('e1', 'invalid', undefined, undefined));
            const stats = SM.getStatistics(machine);
        ...
        it('should provide complete statistics', async () => {
            ...
            const transitions = [
                ...
                },
                {
                    from: { TAG: 'Running', _0: taskData },
                    event: 'complete',
                    to: { TAG: 'Completed', _0: { ...taskData, progress: 100 } },
                    guard: (d) => d.progress >= 50,
                    action: undefined,
                },
            ];
            const machine = SM.create('Idle', transitions, undefined, undefined);
    ...
    describe('Utility Functions', () => {
        it('should check if transition is possible', () => {
            ...
            expect(SM.canTransition(machine, 'pause')).toBe(false);
        });
        ...
        it('should get current state of machine', () => {
            const taskData = { name: 'task1', progress: 50 };
            const state = { TAG: 'Running', _0: taskData };
