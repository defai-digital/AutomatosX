...
import type { ParsedQuery } from '../../types/QueryFilter.js';
  ...
  describe('Multiple Filters', () => {
    it('should extract multiple different filter types', () => {
      const result = parser.parse(
        'find auth lang:python kind:function file:src/**/*.py'
      );
    ...
    it('should extract complex query with negations', () => {
      const result = parser.parse(
        'api endpoints lang:typescript -kind:test -file:**/*.spec.ts'
      );
    ...
    it('should handle filters at different positions in query', () => {
      const result = parser.parse(
        'lang:python find auth kind:function in services file:src/*.py'
      );
