...
export class LRUCache {
  ...
  getPressureLevel(): MemoryPressureLevel {
    ...
    if (ratio >= this.config.highWaterMark) return 'high';
   ...
   */
  evict(reason: EvictionResult['reason'] = 'manual'): EvictionResult {
   ...
   */
  private runEviction(reason: EvictionResult['reason']): EvictionResult {
    ...
    for (const entry of entries) {
      ...
      if (
        this.currentSizeBytes <= targetSize &&
   ...
   */
  private getSortedEntriesForEviction(): MCPCacheEntry[] {
 ...
 */
export function createLRUCache(config?: Partial<MCPCacheConfig>): LRUCache {
