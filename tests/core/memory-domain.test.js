...
import { createInMemoryEventStore, EventStoreError, createAggregateRepository, createConversationAggregate, MemoryErrorCodes, } from '@defai.digital/memory-domain';
...
describe('Memory Domain', () => {
    describe('Event Store (INV-MEM-001, INV-MEM-004)', () => {
        ...
        it('INV-MEM-001: should freeze events to make them immutable', async () => {
            const event = createMemoryEvent('memory.stored', { key: 'test', value: 'data' }, { aggregateId: 'agg-1', version: 1 });
        ...
        it('INV-MEM-004: should enforce version ordering', async () => {
            const event1 = createMemoryEvent('memory.stored', { key: 'test' }, { aggregateId: 'agg-1', version: 1 });
            ...
            try {
                ...
                expect(err.code).toBe(MemoryErrorCodes.VERSION_CONFLICT);
            }
        });
        it('should return events in order', async () => {
            ...
            expect((stored[0]?.payload).key).toBe('a');
            expect((stored[1]?.payload).key).toBe('b');
            expect((stored[2]?.payload).key).toBe('c');
        });
        it('should get current version', async () => {
            expect(await store.getVersion('new-agg')).toBe(0);
            const event = createMemoryEvent('memory.stored', { key: 'test' }, { aggregateId: 'new-agg', version: 1 });
            await store.append(event);
    ...
    describe('Event Store Correlation (INV-MEM-005)', () => {
        ...
        it('should get events by type', async () => {
            await store.append(createMemoryEvent('memory.stored', { key: 'a' }, { aggregateId: 'agg-1', version: 1 }));
            await store.append(createMemoryEvent('memory.retrieved', { key: 'a', found: true }, { aggregateId: 'agg-1', version: 2 }));
            await store.append(createMemoryEvent('memory.stored', { key: 'b' }, { aggregateId: 'agg-2', version: 1 }));
    ...
    describe('Aggregate Repository (INV-MEM-002)', () => {
        ...
        it('INV-MEM-002: should replay events consistently', async () => {
            ...
            // Store some events
            await store.append(createMemoryEvent('memory.stored', { delta: 5 }, { aggregateId: 'counter-1', version: 1 }));
            await store.append(createMemoryEvent('memory.stored', { delta: 3 }, { aggregateId: 'counter-1', version: 2 }));
            await store.append(createMemoryEvent('memory.stored', { delta: -2 }, { aggregateId: 'counter-1', version: 3 }));
        ...
        it('should support point-in-time queries', async () => {
            const repo = createAggregateRepository(store, initialState, handler);
            await store.append(createMemoryEvent('memory.stored', { delta: 10 }, { aggregateId: 'counter-1', version: 1 }));
            await store.append(createMemoryEvent('memory.stored', { delta: 5 }, { aggregateId: 'counter-1', version: 2 }));
        ...
        it('should handle optimistic concurrency', async () => {
            const repo = createAggregateRepository(store, initialState, handler);
            await store.append(createMemoryEvent('memory.stored', { delta: 1 }, { aggregateId: 'counter-1', version: 1 }));
            const newEvent = createMemoryEvent('memory.stored', { delta: 1 }, { aggregateId: 'counter-1', version: 2 });
            ...
            const staleEvent = createMemoryEvent('memory.stored', { delta: 1 }, { aggregateId: 'counter-1', version: 3 });
            try {
...
//# sourceMappingURL=memory-domain.test.js.map