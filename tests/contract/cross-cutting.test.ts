...
describe('Cross-Cutting Concerns', () => {
  ...
  describe('Dead Letter Queue (INV-DLQ-001, INV-DLQ-002, INV-DLQ-003)', () => {
    ...
    it('INV-DLQ-002: should respect maxRetries limit', async () => {
      const entry = await dlq.enqueue(
        ...
        'order.placed',
        { orderId: 'o1' },
        { code: 'TIMEOUT', message: 'Service timeout' },
        'order-service'
      );
    ...
    it('INV-DLQ-003: should mark exhausted entries appropriately', async () => {
      const entry = await dlq.enqueue(
        ...
        'payment.failed',
        { paymentId: 'p1' },
        { code: 'DECLINED', message: 'Card declined' },
        'payment-service'
      );
    ...
    it('should mark entries as resolved', async () => {
      const entry = await dlq.enqueue(
        ...
        'notification.sent',
        { notificationId: 'n1' },
        { code: 'RETRY', message: 'Temporary failure' },
    ...
    it('should discard entries', async () => {
      const entry = await dlq.enqueue(
        ...
        'log.error',
        { logId: 'l1' },
        { code: 'INVALID', message: 'Invalid log entry' },
        'logging-service'
      );
  ...
  describe('Saga Manager (INV-SG-001, INV-SG-002, INV-SG-003)', () => {
    ...
    it('should execute saga steps successfully', async () => {
      ...
      const stepExecutor = async (step: SagaStep) => {
        return { stepId: step.stepId, success: true };
      };

      const compensationExecutor = async (action: CompensationAction) => {
        compensationsCalled.push(action.stepId);
      };

      const result = await sagaManager.execute(
        steps,
        stepExecutor,
  ...
  describe('Retention Manager (INV-RT-001, INV-RT-002, INV-RT-003)', () => {
    ...
    let entries: Map<string, RetentionEntry>;
    let archiver: ReturnType<typeof createInMemoryArchiver>;
