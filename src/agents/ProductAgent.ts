...
export class ProductAgent extends AgentBase {
  constructor() {
    super({
      ...
      description:
        'Expert in product management, PRDs, feature specs, user stories, and product strategy. Specializes in defining clear product requirements.',
      capabilities: [
        {
          name: 'PRDs',
  ...
  protected async executeTask(
    ...
    try {
      ...
      const existingDocs = await context.codeIntelligence.searchCode(
  ...
  private parseProductArtifacts(response: string): TaskArtifact[] {
    ...
    if (
      (response.includes('# PRD') || response.includes('# Product Requirements')) &&
