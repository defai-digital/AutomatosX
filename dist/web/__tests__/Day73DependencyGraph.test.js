...
describe('DependencyGraphVisualization', () => {
    it('renders loading state when graph is null', () => {
        render(_jsx(DependencyGraphVisualization, { graph: null, selectedNode: null, onNodeClick: vi.fn() }));
    ...
    it('renders empty state when graph has no nodes', () => {
        ...
        render(_jsx(DependencyGraphVisualization, { graph: emptyGraph, selectedNode: null, onNodeClick: vi.fn() }));
        expect(screen.getByText(/no dependencies found/i)).toBeInTheDocument();
    });
    it('accepts all required props', () => {
        expect(() => {
            render(_jsx(DependencyGraphVisualization, { graph: mockGraph, selectedNode: null, onNodeClick: vi.fn() }));
        }).not.toThrow();
    });
    it('accepts optional layout props', () => {
        expect(() => {
            render(_jsx(DependencyGraphVisualization, { graph: mockGraph, selectedNode: null, onNodeClick: vi.fn(), layoutAlgorithm: "hierarchical", showLabels: false, nodeSize: 12 }));
        }).not.toThrow();
    });
});
// ========================================
...
describe('GraphControls', () => {
    it('renders all control buttons', () => {
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: vi.fn(), onToggleLabels: vi.fn() }));
        expect(screen.getByText(/graph controls/i)).toBeInTheDocument();
    });
    it('calls onZoomIn when zoom in button clicked', () => {
        const onZoomIn = vi.fn();
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: onZoomIn, onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: vi.fn(), onToggleLabels: vi.fn() }));
    ...
    it('calls onZoomOut when zoom out button clicked', () => {
        ...
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: onZoomOut, onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: vi.fn(), onToggleLabels: vi.fn() }));
    ...
    it('calls onLayoutChange when layout is changed', () => {
        ...
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: onLayoutChange, onNodeSizeChange: vi.fn(), onToggleLabels: vi.fn() }));
    ...
    it('calls onNodeSizeChange when slider is moved', () => {
        ...
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: onNodeSizeChange, onToggleLabels: vi.fn() }));
    ...
    it('calls onToggleLabels when labels switch is toggled', () => {
        ...
        render(_jsx(GraphControls, { layoutAlgorithm: "force", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: vi.fn(), onToggleLabels: onToggleLabels }));
    ...
    it('displays current layout algorithm', () => {
        render(_jsx(GraphControls, { layoutAlgorithm: "circular", showLabels: true, nodeSize: 8, onZoomIn: vi.fn(), onZoomOut: vi.fn(), onResetZoom: vi.fn(), onCenterGraph: vi.fn(), onLayoutChange: vi.fn(), onNodeSizeChange: vi.fn(), onToggleLabels: vi.fn() }));
...
describe('CircularDependencyDetector', () => {
    it('shows success message when no circular dependencies', () => {
        render(_jsx(CircularDependencyDetector, { circularDependencies: [], onHighlightCycle: vi.fn() }));
        expect(screen.getByText(/no circular dependencies detected/i)).toBeInTheDocument();
    });
    it('displays count of circular dependencies', () => {
        render(_jsx(CircularDependencyDetector, { circularDependencies: mockCircularDeps, onHighlightCycle: vi.fn() }));
    ...
    it('calls onHighlightCycle when cycle is clicked', () => {
        ...
        render(_jsx(CircularDependencyDetector, { circularDependencies: mockCircularDeps, onHighlightCycle: onHighlightCycle }));
    ...
    it('shows warning severity for cycles with 3-4 files', () => {
        render(_jsx(CircularDependencyDetector, { circularDependencies: [['file1', 'file2', 'file3']], onHighlightCycle: vi.fn() }));
    ...
    it('shows error severity for cycles with 5+ files', () => {
        ...
        render(_jsx(CircularDependencyDetector, { circularDependencies: [largeCycle], onHighlightCycle: vi.fn() }));
...
describe('NodeDetailsPanel', () => {
    it('shows placeholder when no node selected', () => {
        render(_jsx(NodeDetailsPanel, { node: null, onClose: vi.fn() }));
        expect(screen.getByText(/select a node to view details/i)).toBeInTheDocument();
    });
    it('displays node type and path', () => {
        render(_jsx(NodeDetailsPanel, { node: mockNodes[0], onClose: vi.fn() }));
    ...
    it('displays fan-in metric', () => {
        render(_jsx(NodeDetailsPanel, { node: mockNodes[1], onClose: vi.fn() }));
    ...
    it('calls onClose when close button clicked', () => {
        const onClose = vi.fn();
        render(_jsx(NodeDetailsPanel, { node: mockNodes[0], onClose: onClose }));
...
describe('DependencyFilters', () => {
    ...
    it('renders filter controls', () => {
        render(_jsx(DependencyFilters, { filters: mockFilters, onFiltersChange: vi.fn(), onClearFilters: vi.fn() }));
    ...
    it('displays active filters count', () => {
        ...
        render(_jsx(DependencyFilters, { filters: activeFilters, onFiltersChange: vi.fn(), onClearFilters: vi.fn() }));
        expect(screen.getByText('2')).toBeInTheDocument();
    });
    it('calls onFiltersChange when node type is toggled', () => {
        const onFiltersChange = vi.fn();
        render(_jsx(DependencyFilters, { filters: mockFilters, onFiltersChange: onFiltersChange, onClearFilters: vi.fn() }));
    ...
    it('calls onClearFilters when clear button clicked', () => {
        ...
        render(_jsx(DependencyFilters, { filters: activeFilters, onFiltersChange: vi.fn(), onClearFilters: onClearFilters }));
...
describe('GraphStatistics', () => {
    ...
    it('displays total nodes count', () => {
        render(_jsx(GraphStatistics, { ...mockStats }));
...
describe('DependenciesPage', () => {
    it('renders page with all components', async () => {
        ...
        render(_jsx(Provider, { store: store, children: _jsx(DependenciesPage, {}) }));
...
//# sourceMappingURL=Day73DependencyGraph.test.js.map