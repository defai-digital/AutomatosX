  ...
  describe('InMemorySessionStore', () => {
    ...
    describe('update', () => {
      ...
      it('should throw on version conflict', async () => {
        ...
        await expect(
          store.update(session.sessionId, staleUpdate)
        ).rejects.toThrow(SessionVersionConflictError);
      });
      ...
      it('should throw for non-existent session', async () => {
        ...
        await expect(
          store.update('non-existent', fakeSession)
        ).rejects.toThrow();
      });
    });
  ...
  describe('DefaultSessionManager', () => {
    ...
    describe('leaveSession', () => {
      it('should mark participant as left', async () => {
        ...
        const leftParticipant = updated.participants.find(
          (p) => p.agentId === 'agent-2'
        );
      ...
      it('should throw for non-participant', async () => {
        ...
        await expect(
          manager.leaveSession(session.sessionId, 'agent-999')
        ).rejects.toThrow(SessionError);
      });
    });
    ...
    describe('completeSession', () => {
      ...
      it('should throw for non-existent session', async () => {
        await expect(
          manager.completeSession('non-existent')
        ).rejects.toThrow(SessionError);
      });
    ...
    describe('failSession', () => {
      it('should mark session as failed', async () => {
        ...
        expect(failed.status).toBe('failed');
        const error = failed.metadata?.error as { code?: string } | undefined;
