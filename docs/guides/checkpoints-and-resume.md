...
### Configure defaults
Set `execution.stages.enabled` to `true` in `ax.config.json` to make stage checkpoints the default for every multi-stage agent. You can also control the storage path, cleanup window, and auto-save behaviour:
