...
import { AutomatosXConfigSchema, AutomatosXConfig } from '../../types/Config.js';
 ...
 */
function prettyPrintConfig(config: AutomatosXConfig): void {
 ...
 */
function displayConfigTable(config: AutomatosXConfig): void {
  console.log(chalk.bold('\n⚙️  AutomatosX Configuration\n'));
  ...
  generalTable.push(
    ...
    ['WAL Mode', config.database.wal ? chalk.green('enabled') : chalk.red('disabled')],
    ['Cache Enabled', config.performance.enableCache ? chalk.green('yes') : chalk.red('no')],
    ['Cache Max Size', chalk.blue(config.performance.cacheMaxSize.toLocaleString())],
    ['Log Level', chalk.magenta(config.logging.level)]
  );
  ...
  searchTable.push(
    ['Default Limit', chalk.yellow(config.search.defaultLimit)],
    ['Max Limit', chalk.yellow(config.search.maxLimit)],
    ['Symbol Search', config.search.enableSymbolSearch ? chalk.green('enabled') : chalk.red('disabled')],
    ['Natural Search', config.search.enableNaturalSearch ? chalk.green('enabled') : chalk.red('disabled')],
    ['Hybrid Search', config.search.enableHybridSearch ? chalk.green('enabled') : chalk.red('disabled')],
    ['Symbol Match Threshold', chalk.blue(config.search.symbolMatchThreshold.toFixed(2))],
    ['Hybrid Symbol Weight', chalk.blue(config.search.hybridSymbolWeight.toFixed(2))]
  );
  ...
  indexTable.push(
    ['Chunk Size', chalk.yellow(config.indexing.chunkSize)],
    ['Chunk Overlap', chalk.yellow(config.indexing.chunkOverlap)],
    ['Max File Size', chalk.blue(`${(config.indexing.maxFileSize / 1024 / 1024).toFixed(2)} MB`)],
    ['Follow Symlinks', config.indexing.followSymlinks ? chalk.green('yes') : chalk.red('no')],
    ['Respect .gitignore', config.indexing.respectGitignore ? chalk.green('yes') : chalk.red('no')],
    ['Exclude Patterns', chalk.gray(config.indexing.excludePatterns.slice(0, 3).join(', ') + '...')]
  );
