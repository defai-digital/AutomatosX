...
import { DeterministicEnvironment, MockProviderResponses } from '../../utils/DeterministicSeeds.js'
...
export class GoldenTraceRunner {
  private options: TraceRunnerOptions
  private env?: DeterministicEnvironment
   ...
   */
  async runTrace(trace: GoldenTrace): Promise<TraceExecutionResult> {
   ...
   * Run multiple traces
   */
  async runTraces(traces: GoldenTrace[]): Promise<TraceExecutionResult[]> {
    const results: TraceExecutionResult[] = []
   ...
   */
  async loadTracesFromDirectory(dir: string): Promise<GoldenTrace[]> {
    ...
    for (const file of traceFiles) {
      ...
      const trace = JSON.parse(content) as GoldenTrace
      traces.push(trace)
    }
   ...
   */
  private async setupEnvironment(trace: GoldenTrace): Promise<void> {
   ...
   */
  private async loadFixtures(trace: GoldenTrace): Promise<any> {
   ...
   */
  private async executeTrace(trace: GoldenTrace, fixtures: any): Promise<any> {
   ...
   */
  private compareOutputs(expected: any, actual: any, path: string = ''): TraceDiff[] {
   ...
   */
  private isCriticalDiff(path: string): boolean {
   ...
   */
  private async runAssertions(trace: GoldenTrace, actualOutput: any): Promise<string[]> {
    const errors: string[] = []
   ...
   */
  private evaluateAssertion(assertion: string, actual: any, expected: any): boolean {
    // Mock implementation
