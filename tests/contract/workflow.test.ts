...
describe('Workflow Contract V1', () => {
  ...
  describe('INV-WF-003: Schema Strictness', () => {
    ...
    it('should reject unknown fields at step level', () => {
      ...
      const result = strictStepSchema.safeParse(
        withUnknownStepField.steps[0]
      );
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('INV-WF-004: Step ID Validation', () => {
    it('should accept valid stepId format', () => {
      const validSteps = [
        { stepId: 'step-1', type: 'prompt' as const },
        { stepId: 'my-step', type: 'tool' as const },
        { stepId: 'a123', type: 'conditional' as const },
      ];
