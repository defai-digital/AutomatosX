...
export class IntentLearningSystem {
    ...
    cacheSize = 100;
    constructor(db, userId = 'default-user') {
        this.db = db;
        this.userId = userId;
    ...
    loadRecentCorrections() {
      ...
      LIMIT ?
    `);
        const rows = stmt.all(this.userId, this.cacheSize);
        // Group by query
     ...
     */
    recordCorrection(query, predictedIntent, correctedIntent, confidence) {
    ...
    getLearnedIntent(query) {
      ...
      LIMIT 1
    `);
        const row = stmt.get(query, this.userId);
        if (row) {
     ...
     */
    getSimilarCorrections(query, threshold = 0.7) {
     ...
     */
    applyLearning(query, baseIntent) {
    ...
    getStatistics() {
      ...
      WHERE user_id = ?
    `);
        const totalRow = totalStmt.get(this.userId);
      ...
      LIMIT 10
    `);
        const commonRows = commonStmt.all(this.userId);
    ...
    `);
        const recentRow = recentStmt.get(this.userId);
    ...
    `);
        const improvementRow = improvementStmt.get(this.userId, this.userId);
...
//# sourceMappingURL=IntentLearningSystem.js.map