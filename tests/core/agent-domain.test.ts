...
describe('Agent Domain', () => {
  describe('InMemoryAgentRegistry', () => {
    ...
    describe('update', () => {
      ...
      it('should throw for non-existent agent', async () => {
        await expect(
          registry.update('non-existent', { description: 'Test' })
        ).rejects.toThrow(AgentRegistryError);
      });
      ...
      it('should not allow changing agent ID', async () => {
        ...
        await registry.update('test-agent', {
          agentId: 'new-id' as never, // Attempt to change ID
          description: 'Updated',
        });
  ...
  describe('DefaultAgentExecutor', () => {
    ...
    it('should skip steps when condition is not met', async () => {
      ...
      expect(result.success).toBe(true);
      expect(
        result.stepResults?.find((r: { stepId: string; skipped?: boolean }) => r.stepId === 'step-2')?.skipped
      ).toBe(true);
    });
