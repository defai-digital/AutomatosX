...
describe('Memory CLI Commands', () => {
  ...
  beforeAll(() => {
    ...
    const migration008SQL = readFileSync(
    ...
    const migration009SQL = readFileSync(
      join(process.cwd(), 'src/migrations/009_create_message_embeddings_vec0.sql'),
      'utf-8'
    );
  ...
  describe('integration workflows', () => {
    it('should support full conversation lifecycle', async () => {
      ...
      const exportResult = await memoryExporter.exportConversation(
        conv.id,
        exportFile,
        'json'
      );
