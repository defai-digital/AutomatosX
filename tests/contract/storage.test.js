...
import { StorageModeSchema, StorageConfigSchema, KVEntrySchema, KVSearchResultSchema, TraceSummarySchema, FTSItemSchema, FTSResultSchema, FTSSearchOptionsSchema, validateStorageConfig, safeValidateStorageConfig, validateKVEntry, validateFTSItem, validateTraceSummary, createDefaultStorageConfig, createKVEntry, } from '@defai.digital/contracts';
...
describe('Storage Contract', () => {
    ...
    describe('TraceSummarySchema', () => {
        ...
        it('should accept all valid statuses', () => {
            const statuses = ['pending', 'running', 'success', 'failure', 'skipped'];
...
describe('INV-ST-004: Event Ordering Within Aggregate', () => {
    ...
    it('should track event count for ordering verification', () => {
        ...
        const summaries = [
            { traceId: uuid(), startTime: new Date().toISOString(), status: 'success', eventCount: 3, errorCount: 0 },
            { traceId: uuid(), startTime: new Date().toISOString(), status: 'success', eventCount: 5, errorCount: 0 },
            { traceId: uuid(), startTime: new Date().toISOString(), status: 'failure', eventCount: 2, errorCount: 1 },
        ];
...
describe('INV-ST-007: FTS Search Ranking', () => {
    ...
    it('should support descending score order verification', () => {
        ...
        const isSorted = results.every((r, i) => i === 0 || results[i - 1].score >= r.score);
