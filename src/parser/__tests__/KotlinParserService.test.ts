...
describe('KotlinParserService', () => {
  let parser: KotlinParserService;
...
`;
      ...
      const dataClasses = classes.filter(c =>
        ['Person', 'Point', 'Product'].includes(c.name)
      );
...
`;
      ...
      const interfaceLike = result.symbols.filter(s =>
        s.kind === 'interface' || s.kind === 'class'
      );
...
`;
      ...
      const objects = result.symbols.filter(s =>
        s.kind === 'class' && ['Constants', 'StringUtils'].includes(s.name)
      );
...
`;
      ...
      const enums = result.symbols.filter(s =>
        s.kind === 'enum' || s.kind === 'class'
      );
