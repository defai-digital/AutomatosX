...
- User interface design`,
    ...
    describe('Feature Detection Integration', () => {
        ...
        it('should filter features by specific feature name', async () => {
            ...
            const result = await generator.generate(options);
            expect(result.success).toBe(true);
            expect(mockMemoryService.search).toHaveBeenCalledWith('auth|login|signup|password|token|session|jwt', expect.objectContaining({ limit: 20, includeContent: true }));
        });
    ...
    describe('File Output', () => {
        it('should save PRD to specified file', async () => {
            ...
            await generator.generate(options);
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/prd.md', expect.stringContaining('Product Requirements Document'), 'utf-8');
        });
...
//# sourceMappingURL=PRDGenerator.test.js.map