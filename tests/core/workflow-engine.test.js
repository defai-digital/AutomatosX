...
import { createWorkflowRunner, prepareWorkflow, validateWorkflow, WorkflowValidationError, shouldRetry, calculateBackoff, mergeRetryPolicy, DEFAULT_RETRY_POLICY, } from '@defai.digital/workflow-engine';
describe('Workflow Engine', () => {
    describe('Validation (INV-WF-003, INV-WF-004)', () => {
        ...
        it('INV-WF-003: should reject unknown fields', () => {
            ...
            expect(() => validateWorkflow(workflowWithUnknown)).toThrow(WorkflowValidationError);
        });
        it('INV-WF-004: should reject duplicate step IDs', () => {
            ...
            expect(() => validateWorkflow(workflowWithDuplicates)).toThrow(WorkflowValidationError);
            expect(() => validateWorkflow(workflowWithDuplicates)).toThrow(/duplicate/i);
        });
    });
    describe('Preparation (INV-WF-005)', () => {
        it('INV-WF-005: should freeze workflow after preparation', () => {
            ...
            expect(() => {
                prepared.workflow.workflowId = 'modified';
            }).toThrow();
        });
