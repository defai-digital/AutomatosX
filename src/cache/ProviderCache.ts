...
import type {
  ...
  TopCachedResponse,
  EvictionPolicy,
} from '../types/schemas/cache.schema.js';
import type { ProviderRequest, ProviderResponse, ProviderType } from '../types/schemas/provider.schema.js';
...
export class ProviderCache {
  ...
  async lookup(
    ...
    const dbEntry = this.db
      ...
      .get(cacheKey, Date.now()) as CacheEntry | undefined;
  ...
  async store(
    ...
    options?: { ttl?: number; cost?: number }
  ): Promise<void> {
   ...
   */
  async getStatsByProvider(): Promise<CacheHitRateSummary[]> {
   ...
   */
  async getTopCached(limit: number = 10): Promise<TopCachedResponse[]> {
    const results = this.db
  ...
  private generateCacheKey(request: ProviderRequest): string {
    const normalized = this.normalizeRequest(request);
    const hash = createHash('sha256').update(normalized).digest('hex');
    return `${request.provider || 'claude'}:${hash.substring(0, 16)}`;
  }

  private generateRequestHash(request: ProviderRequest): string {
    const normalized = this.normalizeRequest(request);
    return createHash('sha256').update(normalized).digest('hex');
  }

  private normalizeRequest(request: ProviderRequest): string {
  ...
  private async semanticLookup(
    ...
    const similar = this.db
      ...
      .get(`${requestHash.substring(0, 32)}%`, Date.now()) as CacheEntry | undefined;
  ...
  private async recordMiss(provider: ProviderType): Promise<void> {
