...
import {
  HotReloadEngine,
  createHotReloadEngine,
  getHotReloadEngine,
  resetHotReloadEngine,
} from '../../plugins/HotReload.js'
...
describe('HotReloadEngine', () => {
  ...
  describe('Error Handling', () => {
    it('should handle reload errors', async () => {
      ...
      const originalLoadPlugin = (engine as any).loadPlugin
      ;(engine as any).loadPlugin = vi.fn().mockRejectedValue(new Error('Load failed'))
      ...
      // Restore
      ;(engine as any).loadPlugin = originalLoadPlugin
    })
  ...
  describe('Cache Clearing', () => {
    ...
    it('should clear module cache during reload', async () => {
      // Load a mock module
      require.cache['/test/plugin/index.js'] = { exports: {} } as any
  ...
  describe('Event Lifecycle', () => {
    ...
    it('should emit rollback-complete event', async () => {
      ...
      // Reload once to create state snapshot
      ;(engine as any).loadPlugin = originalLoadPlugin
      await engine.reloadPlugin('test-plugin', {
        watchPath: testDir,
        preserveState: true,
