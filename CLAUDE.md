...
AutomatosX is an AI Agent Orchestration Platform that combines declarative workflow specs, policy-driven cost optimization, and persistent memory. It's a production-ready CLI tool (v6.5.16) that routes AI requests across multiple providers (Claude, Gemini, OpenAI) based on cost, latency, and policy constraints.

## Quick Reference for Common Tasks

```bash
# Most common development workflow
npm run dev -- run backend "test task"    # Dev mode (fastest iteration)
npm run build                              # Full build (includes config generation)
npm test                                   # Run all tests
npm run verify                             # Pre-commit check (typecheck + build + unit tests)
...
**As of last update (v6.5.16):**

- Project is in active development with clean test suite (2,610 tests passing)
- Recent work focused on bug fixes and documentation improvements (see recent commits)
- Watch for modified `src/config.generated.ts` after builds - normal behavior
- The workspace uses `automatosx/PRD/` for planning docs and `automatosx/tmp/` for temporary files
- All core features are production-ready and stable
...
```bash
# ALWAYS regenerate config after changing automatosx.config.json
npm run prebuild:config    # Regenerates src/config.generated.ts
npm run build              # This also calls prebuild:config automatically
...
stmt.get(id); // Fast, safe from SQL injection
...
- `src/config.generated.ts`: Generated TypeScript config (DO NOT EDIT manually, auto-generated by prebuild:config)
...
### Important: Config Generated Files

**`src/config.generated.ts` is automatically generated and should NOT be edited directly:**

- Generated by `npm run prebuild:config` (runs automatically before build)
- Source of truth: `automatosx.config.json`
- Committed to git for type safety but may show as modified after builds
- If you see it as modified in git status, it's normal after running `npm run build`
- To regenerate: `npm run prebuild:config`
- **Never manually edit this file** - changes will be overwritten on next build
...
### Handling Generated Files in Git

When you see modified files in `git status`:

```bash
# Common scenario after running npm run build:
M  src/config.generated.ts    # Auto-generated, may need commit if schema changed
M  package-lock.json           # Commit if dependencies changed
M  package.json                # Commit if you changed dependencies/scripts
```

**Decision Guide:**

1. **`src/config.generated.ts` modified:**
   - If you changed `automatosx.config.json`: Commit it (schema change)
   - If just from rebuild: Check if meaningful changes exist (`git diff src/config.generated.ts`)
   - If only timestamps/comments changed: Can skip committing
   - If types/structure changed: Must commit for type safety

2. **`package-lock.json` modified:**
   - If you ran `npm install`: Always commit
   - If you updated dependencies: Always commit
   - If just from rebuild without dependency changes: Check diff, usually commit

3. **Good practice before committing:**
   ```bash
   # Review what actually changed
   git diff src/config.generated.ts

   # If it's a real schema change, commit with appropriate message
...
**Note**: For comprehensive end-user documentation on using AutomatosX, see [README.md](README.md) and the `docs/` directory. This CLAUDE.md focuses on development guidance.
