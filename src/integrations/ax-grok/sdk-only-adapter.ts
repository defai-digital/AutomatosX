/**
 * Grok SDK-Only Adapter
 *
 * Direct SDK access to Grok without CLI fallback.
 * Used when AutomatosX executes Grok directly (not via subprocess).
 *
 * v13.0.0: Added as part of PRD MCP Architecture Redesign.
 *          Replaces hybrid adapter for outbound execution.
 *
 * @module integrations/ax-grok/sdk-only-adapter
 */
...
/**
 * Configuration for Grok SDK-only adapter
 */
export interface GrokSdkOnlyAdapterOptions {
 ...
 * Grok SDK-Only Adapter
 *
 * Provides direct SDK execution for Grok without CLI fallback.
 ...
 * **Usage:**
 * ```typescript
 * const adapter = new GrokSdkOnlyAdapter({
 *   model: 'grok-3',
 *   maxRetries: 2
 * });
 *
 ...
 * ```
 */
export class GrokSdkOnlyAdapter {
  private sdkAdapter: GrokSdkAdapter;
  private initialized = false;
  private readonly model: GrokModel;
  private readonly maxRetries: number;
  private readonly retryDelayMs: number;

  constructor(options: GrokSdkOnlyAdapterOptions = {}) {
