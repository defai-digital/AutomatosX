...
export class CostAnalytics {
   ...
   */
  async getTrend(
    query: CostQuery & { bucketSize?: 'hour' | 'day' | 'week' } = {}
  ): Promise<Array<{ timestamp: number; cost: number; requests: number }>> {
    const {
      startTime = Date.now() - 7 * 24 * 60 * 60 * 1000, // Default: last 7 days
      endTime = Date.now(),
      provider,
      model,
      userId,
  ...
  async getProjection(
    ...
    const daysElapsed = Math.max(
      1,
      Math.floor((now - startOfPeriod) / (24 * 60 * 60 * 1000))
    );
    ...
    if (trend.length >= 2) {
      ...
      trendProjection = this.applyTrendProjection(
        currentSpend,
        dailyAvg,
        growthRate,
        daysRemaining
    ...
    const confidence = Math.max(
      0.5,
      Math.min(0.95, 1.0 - variance / (estimate || 1))
    );
    ...
    for (const item of summary.byProvider) {
      const providerProjection =
        (item.cost / currentSpend) * estimate || 0;
   ...
   */
  private async storeProjection(projection: CostProjection): Promise<void> {
    ...
    `
      ...
      .run(
        ...
        projection.estimate / projection.daysInPeriod,
        projection.estimate * (7 / projection.daysInPeriod),
        projection.estimate,
        projection.estimate * (365 / projection.daysInPeriod),
        projection.confidence,
        projection.lowEstimate,
        projection.highEstimate,
        JSON.stringify(projection.byProvider),
        JSON.stringify(projection.byModel),
        projection.daysInPeriod,
        projection.daysRemaining,
        projection.currentSpend,
   ...
   * Get all budgets
   */
  async getBudgets(enabled?: boolean): Promise<CostBudget[]> {
   ...
   * Get budget by ID
   */
  async getBudget(budgetId: string): Promise<CostBudget | null> {
   ...
   */
  async saveBudget(budget: Partial<CostBudget>): Promise<CostBudget> {
    const now = Date.now();
    ...
    if (budget.id) {
      ...
      `
        ...
        .run(
          ...
          budget.period,
          budget.limitAmount,
          budget.provider || null,
          budget.model || null,
          budget.userId || null,
          budget.alertAt50Percent ? 1 : 0,
          budget.alertAt80Percent ? 1 : 0,
          budget.alertAt95Percent ? 1 : 0,
          budget.alertAtExceeded ? 1 : 0,
          budget.enabled ? 1 : 0,
  ...
  async getOptimizationRecommendations(): Promise<CostOptimization[]> {
    ...
    const expensiveProviders = summary.byProvider.filter(
      (p) => p.percentage > 20
    );
    ...
    const expensiveModels = summary.byModel.filter(
      (m) =>
        m.percentage > 10 &&
    ...
    return recommendations.sort(
      (a, b) => a.priority - b.priority || b.estimatedSavings - a.estimatedSavings
    );
  }
  ...
  private mapBudgetFromDb(row: any): CostBudget {
    return {
      id: row.id,
      name: row.name,
