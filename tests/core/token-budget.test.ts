...
describe('Token Budget', () => {
  describe('DefaultTokenBudgetAllocator', () => {
    let allocator: DefaultTokenBudgetAllocator;
    ...
    describe('allocate', () => {
      ...
      it('should prioritize critical instructions', () => {
        ...
        const criticalIncluded = result.included.some(
      ...
      it('should record allocation timestamp', () => {
        ...
        expect(new Date(result.allocationTimestamp).getTime()).toBeLessThanOrEqual(
          Date.now()
        );
      });
    ...
    describe('getStatus', () => {
      ...
      it('should track critical reserve availability', () => {
        const status = allocator.getStatus();
        expect(status.criticalReserveAvailable).toBe(
          DEFAULT_TOKEN_BUDGET.criticalReserve
        );
      });
    });
    ...
    describe('setConfig', () => {
      it('should update configuration', () => {
        const newConfig: TokenBudgetConfig = {
          maxTotal: 10000,
          perType: {
            memory: 4000,
            todo: 2000,
  ...
  describe('Integration scenarios', () => {
    it('should handle typical agent context allocation', () => {
      ...
      const instructions: EmbeddedInstruction[] = [
        ...
        createInstruction(
          'memory',
          'Previous conversation: User asked about TypeScript...',
          'high'
        ),
    ...
    it('should handle budget overflow gracefully', () => {
      ...
      for (let i = 0; i < 10; i++) {
        instructions.push(
          createInstruction(
            'memory',
            `Memory item ${i}: Some content that takes up tokens`,
            'normal'
          )
        );
      }
      ...
      expect(result.totalTokens).toBeLessThanOrEqual(
        100 + 20 // maxTotal + criticalReserve
      );
    });
