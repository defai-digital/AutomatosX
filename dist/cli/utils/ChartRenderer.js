...
export class ChartRenderer {
    ...
    static lineChart(data, options = {}) {
        const { width = 60, height = 10, title, xLabel, yLabel, showAxis = true, colorize: useColors = true, xAxis = [], } = options;
        if (data.length === 0) {
        ...
        if (showAxis) {
            ...
            if (xAxis.length > 0) {
                ...
                const labelPositions = Array(width + yLabelWidth + 2).fill(' ');
        ...
        if (xLabel) {
            lines.push('');
            lines.push(' '.repeat(yLabelWidth + width / 2 - xLabel.length / 2) +
     ...
     */
    static drawLine(chart, x0, y0, x1, y1) {
    ...
    static barChart(data, options = {}) {
        const { width = 60, title, maxBarWidth = 40, showPercentage = true, showValue = true, colorize: useColors = true, } = options;
        if (data.length === 0) {
            return 'No data to display';
        }
        const lines = [];
    ...
    static table(options) {
        const { headers, rows, alignments = headers.map(() => 'left'), maxWidth = 100, } = options;
        const lines = [];
        // Calculate column widths
        const columnWidths = headers.map((header, i) => {
            const headerLength = header.length;
            const maxRowLength = Math.max(...rows.map((row) => (row[i] || '').length));
        ...
        // Format cell
        const formatCell = (text, width, alignment) => {
        ...
        // Add bottom border
        const bottomBorder = '─'.repeat(columnWidths.reduce((sum, w) => sum + w, 0) + columnWidths.length * 3 + 1);
    ...
    static box(text, options = {}) {
        ...
        const paddedLines = lines.map((line) => `│${' '.repeat(padding)}${line.padEnd(maxLength)}${' '.repeat(padding)}│`);
        return [topBorder, ...paddedLines, bottomBorder].join('\n');
    }
    /**
     * Create a divider line
     */
    static divider(width = 80, char = '─') {
        return colorize(char.repeat(width), 'gray');
    }
    /**
     * Render key-value pairs
     */
    static keyValue(pairs, options = {}) {
