...
describe('Monitoring REST API Integration', () => {
    ...
    beforeEach(() => {
        ...
        const monitoringAPI = new MonitoringAPI(db, metrics, alerting, health, workflows, tracer, logger);
        app = express();
    ...
    describe('GET /api/monitoring/alerts', () => {
        it('should list alerts with filters', async () => {
            ...
            alerting.createRule('Test Alert', 'Test description', 'test.metric', { operator: 'gt', threshold: 5, windowMs: 10000, aggregation: 'sum' }, 'critical');
    ...
    describe('POST /api/monitoring/alerts/:id/acknowledge', () => {
        it('should acknowledge an alert', async () => {
            ...
            alerting.createRule('Test', 'Test', 'test.metric', { operator: 'gt', threshold: 5, windowMs: 10000, aggregation: 'sum' }, 'warning');
            for (let i = 0; i < 10; i++) {
                metrics.recordMetric('test.metric', 1);
            }
            alerting.startEvaluation(50);
    ...
    describe('GET /api/monitoring/workflows', () => {
        ...
        it('should filter by status', async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.every((w) => w.status === 'completed')).toBe(true);
        });
    });
    ...
    describe('GET /api/monitoring/logs', () => {
        it('should query logs with filters', async () => {
            ...
            expect(response.status).toBe(200);
            expect(Array.isArray(response.body)).toBe(true);
            expect(response.body.every((log) => log.source === 'test-source')).toBe(true);
        });
        it('should support full-text search', async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.some((log) => log.message.includes('authentication'))).toBe(true);
        });
    });
