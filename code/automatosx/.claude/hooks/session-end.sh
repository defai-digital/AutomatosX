...
if command -v jq &>/dev/null; then
  jq -cn --arg ts "$(date -u +"%Y-%m-%dT%H:%M:%SZ")" --arg event "SessionEnd" --arg session "$SESSION"     '{ts:$ts,event:$event,session:$session}' >> "$TRACE_DIR/hook-trace.jsonl" 2>/dev/null || true
else
