...
export class ResearcherAgent extends AgentBase {
  ...
  private buildResearcherPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseResearcherArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'markdown' || language === 'md') {
        if (content.includes('# Research') || content.includes('# Study')) name = 'research-report';
        else if (content.includes('# Literature Review') || content.includes('# Survey')) name = 'literature-review';
        else if (content.includes('# Analysis') || content.includes('# Findings')) name = 'analysis-report';
        else if (content.includes('# Competitive Analysis') || content.includes('# Market Research')) name = 'competitive-analysis';
        else if (content.includes('# Whitepaper') || content.includes('# Technical Report')) name = 'whitepaper';
      } else if (language === 'csv' || language === 'json') {
        name = 'research-data';
      } else if (language === 'python' || language === 'r') {
        if (content.includes('import pandas') || content.includes('library(')) name = 'analysis-script';
