 ...
 */
function displayFlow(functionName, definition, references) {
    console.log();
    ...
    if (references.length > 0) {
        ...
        for (let i = 0; i < references.length; i++) {
            ...
            const snippet = ref.content.split('\n').find((line) => line.toLowerCase().includes(functionName.toLowerCase())) || ref.content.split('\n')[0];
...
export function createFlowCommand() {
    ...
    flowCommand
        ...
        try {
            ...
            const definition = symbols.find((s) => s.kind === 'function' || s.kind === 'method') || symbols[0] || null;
            ...
            const references = searchResults.filter((result) => {
                if (!definition)
                    return true;
                return !(result.file_path === definition.file_path &&
