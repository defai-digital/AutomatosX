...
export class WorkspaceSymbolsProvider {
   ...
   */
  async provideWorkspaceSymbols(query: string): Promise<LSPSymbolInformation[]> {
    try {
      if (!query || query.trim() === '') {
        return [];
      }
      ...
      const symbolInformation = limitedSymbols.map((symbol) =>
        this.convertToSymbolInformation(symbol)
      );
   ...
   * Filter and rank symbols by query
   */
  private filterAndRankSymbols(symbols: any[], query: string): any[] {
   ...
   */
  private fuzzyMatch(name: string, query: string): number {
    let nameIndex = 0;
    let queryIndex = 0;
    let score = 0;
   ...
   */
  private convertToSymbolInformation(symbol: any): LSPSymbolInformation {
   ...
   */
  private getContainerName(symbol: any): string | undefined {
   ...
   */
  async searchSymbolsByKind(kind: string): Promise<LSPSymbolInformation[]> {
    try {
      const symbolDAO = this.integrationService.getSymbolDAO();
      const symbols = symbolDAO.findByKind(kind);

      const symbolInformation = symbols.map((symbol) =>
        this.convertToSymbolInformation(symbol)
      );
   ...
   */
  async getFileSymbols(filePath: string): Promise<LSPSymbolInformation[]> {
    try {
