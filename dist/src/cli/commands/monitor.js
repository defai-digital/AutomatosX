...
// ============================================================================
function registerStatusCommand(monitor) {
    monitor
        .command('status')
...
async function showStatus(options) {
    ...
    console.log(ChartRenderer.box('Provider Health Status', { title: 'System Health' }));
    console.log(ChartRenderer.divider(60));
    console.log('');
    // Provider health
    for (const provider of health) {
        ...
        if (options.verbose) {
            console.log(ChartRenderer.keyValue([
                {
                    key: '    Latency (P95)',
            ...
            console.log(`    Latency: ${ChartRenderer.formatDuration(provider.p95Latency)} (p95)`);
            console.log(`    Success: ${ChartRenderer.formatPercentage(provider.successRate * 100)}`);
            console.log(`    Cost: ${ChartRenderer.formatCurrency(provider.costToday)} today`);
        }
        console.log('');
    }
    // Active alerts
    if (alerts.length > 0) {
        ...
        for (const alert of alerts.slice(0, 5)) {
            ...
            console.log(`  ${icon} ${alert.severity.toUpperCase()}  ${alert.ruleName} (${duration}m ago)`);
            console.log(`    Condition: ${alert.rule.metric} ${alert.rule.operator} ${alert.rule.threshold}`);
    ...
    // Cache stats
    console.log(ChartRenderer.divider(60));
    console.log(`Cache Hit Rate: ${ChartRenderer.formatPercentage(cacheStats.cacheHitRate * 100 || 0)}`);
    if (cacheStats.cacheSavedCost > 0) {
        console.log(`Cache Savings: ${ChartRenderer.formatCurrency(cacheStats.cacheSavedCost)} (1h)`);
    }
    // Rate limit status
...
// ============================================================================
function registerMetricsCommand(monitor) {
    monitor
...
async function showMetrics(metric, options) {
    const collector = new MetricsCollector();
    // Parse time range
...
async function showLatencyMetrics(collector, startTime, endTime, options) {
    ...
    if (options.format === 'chart') {
        console.log(ChartRenderer.lineChart(latencyData, {
...
async function showCostMetrics(collector, startTime, endTime, options) {
    const stats = await collector.getAggregated({
        startTime,
        endTime,
        provider: options.provider,
...
async function showErrorMetrics(collector, startTime, endTime, options) {
    const stats = await collector.getAggregated({
        startTime,
        endTime,
        provider: options.provider,
...
// ============================================================================
function registerAlertsCommand(monitor) {
    ...
    for (const alert of alerts) {
        const icon = alert.severity === 'critical'
            ...
            : alert.severity === 'warning'
                ...
                : 'ℹ️';
        const duration = Math.floor((Date.now() - alert.startedAt) / 60000);
        console.log(`  ${icon} ${alert.severity.toUpperCase()}  ${alert.ruleName}`);
        console.log(`    ID: ${alert.id}`);
        console.log(`    Started: ${duration}m ago`);
        console.log(`    Condition: ${alert.rule.metric} ${alert.rule.operator} ${alert.rule.threshold}`);
        console.log(`    Current: ${alert.currentValue}`);
        if (alert.state === 'acknowledged') {
            console.log(`    Acknowledged: ${alert.acknowledgedBy || 'unknown'} at ${new Date(alert.acknowledgedAt).toLocaleString()}`);
        }
        console.log('');
    }
}
async function listAlertRules(options) {
    ...
    for (const rule of rules) {
        ...
        console.log(`    Condition: ${rule.metric} ${rule.operator} ${rule.threshold} for ${rule.durationSeconds}s`);
        console.log(`    Severity: ${rule.severity}`);
        console.log('');
    }
}
async function acknowledgeAlert(alertId, options) {
    const collector = new MetricsCollector();
    const alertManager = new AlertManager(collector);
    await alertManager.acknowledgeAlert(alertId, options.user);
    console.log(ChartRenderer.statusIndicator('ok', `Alert ${alertId} acknowledged`));
}
async function disableAlertRule(ruleId) {
    const collector = new MetricsCollector();
    const alertManager = new AlertManager(collector);
    ...
    console.log(ChartRenderer.statusIndicator('ok', `Alert rule ${rule.name} disabled`));
}
// ============================================================================
// Costs Command (ax monitor costs)
// ============================================================================
function registerCostsCommand(monitor) {
    monitor
        .command('costs')
...
async function showCosts(options) {
    ...
    // Display header
    console.log(ChartRenderer.divider(80));
    console.log(ChartRenderer.box('Cost Analytics', {
    ...
    if (breakdown.length > 0) {
        ...
        console.log(ChartRenderer.barChart(breakdown.map((item) => ({
    ...
    if (options.budget) {
        ...
        if (budgets.length > 0) {
            ...
            for (const budget of budgets) {
                ...
                console.log(`    ${ChartRenderer.progressBar(status.currentSpend, budget.limitAmount, { width: 40, showPercentage: true })}`);
                console.log(`    ${ChartRenderer.formatCurrency(status.currentSpend)} / ${ChartRenderer.formatCurrency(budget.limitAmount)} (${status.percentUsed.toFixed(1)}%)`);
                if (status.projectedOverage > 0) {
                    console.log(`    ${ChartRenderer.statusIndicator('warning', `Projected to exceed by ${ChartRenderer.formatCurrency(status.projectedOverage)}`)}`);
                }
                console.log('');
            }
        }
    ...
    if (recommendations.length > 0) {
        ...
        for (const rec of recommendations.slice(0, 3)) {
            console.log(`  • ${rec.title} (save ${ChartRenderer.formatCurrency(rec.estimatedSavings)}/month)`);
