...
- Thread safety considerations in concurrent environments`,
  ...
  describe('constructor', () => {
    ...
    it('should set generatorName to "ADR"', () => {
      expect((generator as any).generatorName).toBe('ADR');
    });
  });

  describe('generate', () => {
    const baseOptions: ADRGenerateOptions = {
    ...
    it('should include code examples when requested', async () => {
      ...
      const prompt = (mockProviderRouter.route as any).mock.calls[0][0].messages[0].content;
