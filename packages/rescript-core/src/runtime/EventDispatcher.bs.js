...
function placeholderEffect(effect, context) {
  if (typeof effect !== "object") {
    switch (effect) {
      case "EvaluateGuards" :
          var guardVerdictStr = StateMachine.GuardVerdict.toString(context.guardVerdict);
...
function telemetryDecisionHandler(decision) {
  var payload = createPayload([
        [
          "status",
          StateMachine.statusToString(decision.status)
        ],
        [
          "fromState",
          StateMachine.State.toString(decision.fromState)
        ],
        [
          "toState",
          StateMachine.State.toString(decision.toState)
        ],
        [
          "event",
          StateMachine.$$Event.toString(decision.event)
        ],
        [
          "effectCount",
...
function dispatchWithHandlers(state, $$event, context, handlers) {
  var decision = StateMachine.transition(state, $$event, context);
...
function dispatchSerialized(state, $$event, context, handlers) {
  return StateMachine.serializeOutcome(dispatchWithHandlers(state, $$event, context, handlers));
}
