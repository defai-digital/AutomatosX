...
export class StructuredLogger extends EventEmitter {
    ...
    flushInterval = null;
    minLevel;
    constructor(db, bufferSize = 50, minLevel = 'info') {
        super();
     ...
     * Generic log method
     */
    log(level, message, source, options) {
    ...
    flush() {
      ...
      `);
            for (const log of logs) {
                stmt.run(log.id, log.timestamp, log.level, log.message, log.source, log.executionId || null, log.traceId || null, log.spanId || null, log.tenantId || null, log.userId || null, log.metadata ? JSON.stringify(log.metadata) : null, log.stackTrace || null);
            }
        });
        try {
