...
export class CostEstimator {
     ...
     */
    estimateStepCost(step, provider = 'claude', model, complexity = 'medium') {
        ...
        const inputTokens = Math.round((TOKEN_ESTIMATES.basePrompt +
            TOKEN_ESTIMATES.perStepOverhead +
            TOKEN_ESTIMATES.avgPromptTokens) *
            complexityMult);
        const outputTokens = Math.round(TOKEN_ESTIMATES.avgCompletionTokens * complexityMult);
     ...
     */
    estimateWorkflowCost(workflow, provider = 'claude', model) {
        ...
        return {
            provider,
            model: model || DEFAULT_MODELS[provider.toLowerCase()],
    ...
    getFastestProvider(workflow) {
        ...
        const [fastestProvider, fastestEstimate] = entries.reduce((min, current) => {
            const currentSpeed = providerSpeed[current[0]] || 999;
            const minSpeed = providerSpeed[min[0]] || 999;
    ...
    getBalancedProvider(workflow) {
        ...
        const [balancedProvider, balancedEstimate] = entries.reduce((best, current) => {
            ...
            const speed = providerSpeed[provider] || maxSpeed;
            ...
            const bestSpeed = providerSpeed[bestProvider] || maxSpeed;
     ...
     */
    determineWorkflowComplexity(workflow) {
