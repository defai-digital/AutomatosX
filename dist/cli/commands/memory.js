  ...
  $ ax memory stats                                  # Show statistics
    ...
    memory
        ...
        try {
            ...
            if (searchMode === 'semantic') {
                ...
                conversations = (await Promise.all(conversationIds.map(id => memoryService.getConversation(id)))).filter(Boolean);
            }
            ...
            console.log(chalk.green(`Found ${searchResults.total} messages (showing ${searchResults.messages.length}):\n`));
            for (const msg of searchResults.messages) {
                const conversation = searchResults.conversations.find((c) => c.id === msg.conversationId);
                ...
                if (conversation) {
                    console.log(chalk.gray(`  Conversation: ${conversation.title} (${conversation.agentId})`));
                }
                if (options.verbose) {
            ...
            if (searchResults.hasMore) {
                console.log(chalk.blue(`💡 Tip: Use --limit to see more results (max: ${searchResults.total})`));
            }
    ...
    memory
        ...
        try {
            ...
            infoTable.push(['ID', chalk.gray(conversation.id)], ['Title', chalk.white(conversation.title)], ['Agent', chalk.cyan(conversation.agentId)], ['State', conversation.state === 'active' ? chalk.green(conversation.state) : chalk.gray(conversation.state)], ['Messages', chalk.blue(conversation.messageCount.toString())], ['Total Tokens', chalk.yellow(conversation.totalTokens.toString())], ['Created', chalk.gray(formatTimestamp(conversation.createdAt))], ['Updated', chalk.gray(formatTimestamp(conversation.updatedAt))]);
            ...
            if (conversation.messages && conversation.messages.length > 0) {
                ...
                if (conversation.messages.length > limit) {
                    console.log(chalk.blue(`💡 Showing ${limit} of ${conversation.messages.length} messages. Use --limit to see more.`));
                }
            }
            else {
    ...
    memory
        ...
        try {
            ...
            resultsTable.push(['Messages Indexed', chalk.green(result.indexed.toString())], ['Messages Skipped', chalk.yellow(result.skipped.toString())], ['Messages Failed', result.failed > 0 ? chalk.red(result.failed.toString()) : chalk.gray('0')], ['Duration', chalk.gray(formatDuration(result.duration))]);
    ...
    memory
        ...
        try {
            ...
            overviewTable.push(['Total Conversations', chalk.blue(stats.totalConversations.toLocaleString())], ['Active Conversations', chalk.green(stats.activeConversations.toLocaleString())], ['Archived Conversations', chalk.yellow(stats.archivedConversations.toLocaleString())], ['Deleted Conversations', chalk.red(stats.deletedConversations.toLocaleString())], ['Total Messages', chalk.blue(stats.totalMessages.toLocaleString())], ['Total Tokens', chalk.yellow(stats.totalTokens.toLocaleString())], [
                'Avg Messages/Conversation',
                chalk.gray(stats.averageMessagesPerConversation.toFixed(1)),
            ], ['Avg Tokens/Message', chalk.gray(stats.averageTokensPerMessage.toFixed(1))], ['Storage Estimate', chalk.gray(`${stats.storageEstimateMB.toFixed(2)} MB`)], ['─'.repeat(28), '─'.repeat(18)], // Separator
            ['Embedding Coverage', chalk.magenta(`${embeddingStats.coveragePercent.toFixed(1)}% (${embeddingStats.totalEmbeddings}/${embeddingStats.totalMessages})`)], ['Model Version', chalk.gray(embeddingStats.currentModelVersion || 'N/A')]);
            console.log(overviewTable.toString());
            // Time Range
            if (stats.oldestConversation && stats.newestConversation) {
                console.log(chalk.bold('\n📅 Time Range:\n'));
                console.log(chalk.gray(`  Oldest: ${formatTimestamp(stats.oldestConversation)}`));
                console.log(chalk.gray(`  Newest: ${formatTimestamp(stats.newestConversation)}`));
            ...
            if (stats.totalConversations > 0) {
                ...
                if (stats.averageMessagesPerConversation < 2) {
                    console.log(chalk.yellow('  ⚠ Low average messages per conversation - consider consolidating'));
                }
                else if (stats.averageMessagesPerConversation > 10) {
                    console.log(chalk.green('  ✓ Good conversation engagement with multiple messages'));
                }
                if (stats.archivedConversations > stats.activeConversations * 2) {
                    console.log(chalk.blue('  ℹ Many archived conversations - consider cleaning up old archives'));
                }
                const deletedPercent = (stats.deletedConversations / stats.totalConversations) * 100;
                if (deletedPercent > 10) {
                    console.log(chalk.yellow(`  ⚠ ${deletedPercent.toFixed(1)}% of conversations are deleted - consider permanent cleanup`));
                }
