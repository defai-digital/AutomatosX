...
export class ExportService {
   ...
   */
  async export(report: QualityReport, options: ExportOptions): Promise<ExportResult> {
   ...
   */
  async exportCodeSmells(report: QualityReport, options: ExportOptions): Promise<ExportResult> {
   ...
   */
  async exportSummary(report: QualityReport, options: ExportOptions): Promise<ExportResult> {
    let result: ExportResult;
   ...
   */
  private async writeToFile(result: ExportResult, outputPath: string): Promise<void> {
   ...
   */
  private getOutputPathForFormat(basePath: string, format: ExportFormat): string {
  ...
  validateOptions(options: ExportOptions): void {
    ...
    if (!validFormats.includes(options.format)) {
      throw new Error(
        `Invalid export format: ${options.format}. Valid formats: ${validFormats.join(', ')}`
      );
    }
