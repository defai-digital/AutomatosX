...
export class WorkflowEngineV2 {
    ...
    optimizer;
    cache;
    constructor(db, cache, agentBridge) {
    ...
    async executeWorkflow(workflowDef, options = {}) {
        ...
        const machine = WorkflowStateMachineBridge.create(execution.id, workflowDef.name, stepIds);
        // 6. Execute workflow with state machine
        try {
            const result = await this.runWorkflowWithStateMachine(machine, execution.id, workflowDef, options.context || {});
    ...
    async resumeWorkflow(checkpointId) {
        ...
        try {
            const result = await this.runWorkflowWithStateMachine(machine, execution.id, workflowDef, restored.context);
            this.dao.updateExecutionState(execution.id, 'completed');
            return result;
        }
        catch (error) {
     ...
     */
    async runWorkflowWithStateMachine(initialMachine, executionId, workflowDef, context) {
        ...
        machine = await this.executeStepsWithStateMachine(machine, executionId, workflowDef, graph, context);
        ...
        return {
            ...
            stepResults: Object.fromEntries(completedSteps.map(step => [step.id, {
     ...
     */
    async executeStepsWithStateMachine(machine, executionId, workflowDef, graph, context) {
        let current = machine;
        const stepResults = {};
        ...
        for (const level of graph.levels) {
            ...
            const results = await Promise.allSettled(steps.map(step => this.executeStepWithAgent(step, context, stepResults, current)));
     ...
     */
    async executeStepWithAgent(step, context, stepResults, machine) {
        const startTime = Date.now();
...
//# sourceMappingURL=WorkflowEngineV2.js.map