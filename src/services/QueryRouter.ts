...
export interface QueryAnalysis {
  ...
  normalizedQuery: string;      // Normalized for search
  features: {
    isSingleWord: boolean;
    hasOperators: boolean;      // AND, OR, NOT
    isIdentifier: boolean;      // PascalCase, camelCase, snake_case
    wordCount: number;
    hasCommonWords: boolean;    // contains common English words
    hasSpecialChars: boolean;   // contains code-specific characters
  };
}
...
export class QueryRouter {
  ...
  private static readonly FTS5_OPERATORS = ['AND', 'OR', 'NOT'];
   ...
   */
  private determineIntent(features: QueryAnalysis['features']): QueryIntent {
  ...
  private calculateConfidence(
    ...
    if (intent === QueryIntent.SYMBOL) {
      ...
      if (features.hasOperators) confidence += 0.3;
      if (features.hasCommonWords) confidence += 0.2;
      if (features.wordCount > 2) confidence += 0.1;
  ...
  private hasOperators(query: string): boolean {
    ...
    return QueryRouter.FTS5_OPERATORS.some((op) =>
      upperQuery.includes(` ${op} `)
    );
  }
   ...
   */
  private isIdentifierWord(word: string): boolean {
    // PascalCase: Calculator, UserManager
    if (/^[A-Z][a-zA-Z0-9]*$/.test(word)) return true;
   ...
   */
  private hasCommonWords(normalizedQuery: string): boolean {
   ...
   */
  private hasSpecialChars(query: string): boolean {
