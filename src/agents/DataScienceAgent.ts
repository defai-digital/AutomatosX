...
export class DataScienceAgent extends AgentBase {
  ...
  private buildMLPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseMLArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (content.includes('tensorflow') || content.includes('keras')) name = 'tensorflow-model';
      if (content.includes('torch') || content.includes('pytorch')) name = 'pytorch-model';
