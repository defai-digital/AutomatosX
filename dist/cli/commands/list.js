...
import { handleOutputFormat, showEmptyMessage, showSectionHeader, showSectionFooter } from '../../utils/cli-formatters.js';
import { listFilesWithExtensions } from '../../utils/file-helpers.js';
import { join } from 'path';
function isAgentInfoError(info) {
...
async function listAgents(options) {
    try {
        ...
        await handleOutputFormat(agentNames, options.format, async () => {
            // Table format (default) - load agent info in parallel
            showSectionHeader('🤖 Available Agents');
            const infoPromises = agentNames.map(async (agentName) => {
                try {
...
async function listTeams(options) {
    try {
        ...
        await handleOutputFormat(teams, options.format, () => {
...
async function listAbilities(options) {
    try {
        ...
        await handleOutputFormat(abilities, options.format, () => {
            console.log(chalk.bold('\n⚡ Available Abilities\n'));
...
async function listTemplates(options) {
    try {
        ...
        await handleOutputFormat(templates, options.format, () => {
            console.log(chalk.bold('\n📋 Available Templates\n'));
