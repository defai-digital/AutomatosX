...
import { AgentParallelExecutionConfigSchema, AgentParallelTaskSchema, AgentParallelTaskResultSchema, AgentParallelGroupResultSchema, ExecutionPlanSchema, ParallelExecutionRequestSchema, validateAgentParallelExecutionConfig, validateAgentParallelTask, validateParallelExecutionRequest, validateExecutionPlan, safeValidateAgentParallelExecutionConfig, createDefaultAgentParallelExecutionConfig, createAgentParallelTask, createSharedContext, ParallelExecutionErrorCodes, } from '@defai.digital/contracts';
// ============================================================================
...
describe('AgentParallelExecutionConfigSchema', () => {
    ...
    it('should enforce maxConcurrentAgents bounds (INV-APE-001)', () => {
        // Valid range: 1-10
        expect(() => AgentParallelExecutionConfigSchema.parse({ maxConcurrentAgents: 0 })).toThrow();
        expect(() => AgentParallelExecutionConfigSchema.parse({ maxConcurrentAgents: 11 })).toThrow();
        expect(() => AgentParallelExecutionConfigSchema.parse({ maxConcurrentAgents: 5 })).not.toThrow();
    });
    it('should enforce agentTimeout bounds (INV-APE-005)', () => {
        // Min: 1000ms, Max: 7200000ms
        expect(() => AgentParallelExecutionConfigSchema.parse({ agentTimeout: 500 })).toThrow();
        expect(() => AgentParallelExecutionConfigSchema.parse({ agentTimeout: 8000000 })).toThrow();
        expect(() => AgentParallelExecutionConfigSchema.parse({ agentTimeout: 60000 })).not.toThrow();
    });
    it('should accept valid failure strategies', () => {
        const strategies = ['failFast', 'failSafe', 'continueOnError'];
        for (const strategy of strategies) {
            expect(() => AgentParallelExecutionConfigSchema.parse({ failureStrategy: strategy })).not.toThrow();
        }
        expect(() => AgentParallelExecutionConfigSchema.parse({ failureStrategy: 'invalid' })).toThrow();
    });
    it('should accept valid result aggregation strategies', () => {
        const strategies = ['merge', 'list', 'firstSuccess', 'custom'];
        for (const strategy of strategies) {
            expect(() => AgentParallelExecutionConfigSchema.parse({ resultAggregation: strategy })).not.toThrow();
        }
    });
});
// ============================================================================
...
describe('AgentParallelTaskSchema', () => {
    ...
    it('should require taskId and agentId', () => {
        expect(() => AgentParallelTaskSchema.parse({
            input: {},
            dependencies: [],
        })).toThrow();
    });
...
describe('AgentParallelTaskResultSchema', () => {
    ...
    it('should validate all status values', () => {
        ...
        for (const status of statuses) {
            expect(() => AgentParallelTaskResultSchema.parse({
                taskId: crypto.randomUUID(),
                agentId: 'agent',
                status,
...
describe('ParallelExecutionRequestSchema', () => {
    ...
    it('should require at least one task', () => {
        expect(() => ParallelExecutionRequestSchema.parse({
            tasks: [],
        })).toThrow();
    });
    it('should limit to 100 tasks', () => {
        const manyTasks = Array.from({ length: 101 }, () => createAgentParallelTask('agent', {}));
        expect(() => ParallelExecutionRequestSchema.parse({
            tasks: manyTasks,
        })).toThrow();
    });
...
describe('ParallelExecutionErrorCodes', () => {
    it('should have all required error codes', () => {
        expect(ParallelExecutionErrorCodes.CIRCULAR_DEPENDENCY).toBe('PARALLEL_CIRCULAR_DEPENDENCY');
        expect(ParallelExecutionErrorCodes.CONCURRENCY_EXCEEDED).toBe('PARALLEL_CONCURRENCY_EXCEEDED');
        expect(ParallelExecutionErrorCodes.GROUP_TIMEOUT).toBe('PARALLEL_GROUP_TIMEOUT');
        expect(ParallelExecutionErrorCodes.TASK_TIMEOUT).toBe('PARALLEL_TASK_TIMEOUT');
        expect(ParallelExecutionErrorCodes.TASK_FAILED).toBe('PARALLEL_TASK_FAILED');
        expect(ParallelExecutionErrorCodes.AGENT_NOT_FOUND).toBe('PARALLEL_AGENT_NOT_FOUND');
        expect(ParallelExecutionErrorCodes.DEPENDENCY_FAILED).toBe('PARALLEL_DEPENDENCY_FAILED');
        expect(ParallelExecutionErrorCodes.CANCELLED).toBe('PARALLEL_CANCELLED');
        expect(ParallelExecutionErrorCodes.INVALID_PLAN).toBe('PARALLEL_INVALID_PLAN');
        expect(ParallelExecutionErrorCodes.CONTEXT_MUTATION).toBe('PARALLEL_CONTEXT_MUTATION');
    });
});
// ============================================================================
...
describe('Validation Functions', () => {
    describe('validateAgentParallelExecutionConfig', () => {
        ...
        it('should throw on invalid config', () => {
            expect(() => validateAgentParallelExecutionConfig({ maxConcurrentAgents: 100 })).toThrow();
        });
    });
