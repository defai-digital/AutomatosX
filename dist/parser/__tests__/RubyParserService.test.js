...
`;
            const result = parser.parse(code);
            const classMethods = result.symbols.filter(s => s.kind === 'method' && s.name.startsWith('self.'));
...
`;
            const result = parser.parse(code);
            const classVars = result.symbols.filter(s => s.kind === 'variable' && s.name.startsWith('@@'));
...
//# sourceMappingURL=RubyParserService.test.js.map