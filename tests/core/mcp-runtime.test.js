...
// Timeout
withTimeout, withToolTimeout, TimeoutError, 
// Response
createSuccessResponse, createErrorResponse, createListResponse, truncateString, 
// Validation
validateRequest, isValidRequest, formatValidationErrors, createRuntimeGuard, quickGuardCheck, } from '@defai.digital/mcp-runtime';
    ...
    describe('LRU Cache', () => {
        ...
        describe('INV-MCP-CACHE-004: LRU Ordering', () => {
            ...
            it('should prefer evicting non-accessed entries over accessed ones (LRU)', () => {
                ...
                const neverAccessedCount = (lruCache.has('never-accessed-1') ? 1 : 0) +
            ...
            it('should evict least frequently used entries (LFU policy)', () => {
                ...
                lfuCache.get('often-used');
                lfuCache.get('often-used'); // 2 accesses
                lfuCache.get('very-used');
                lfuCache.get('very-used');
                lfuCache.get('very-used'); // 3 accesses
                for (let i = 0; i < 5; i++)
                    lfuCache.get('super-used'); // 5 accesses
                // Trigger eviction
    ...
    describe('Timeout', () => {
        ...
        describe('INV-MCP-TIMEOUT-005: Duration Tracking', () => {
            ...
            it('should track duration for timed out operations', async () => {
                const result = await withTimeout(() => new Promise((r) => setTimeout(r, 1000)), 50);
