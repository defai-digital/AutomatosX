...
export class WorkflowQueue extends EventEmitter {
  ...
  private processingTimeSum: number = 0;
   ...
   */
  getItemsByStatus(status: QueueItem['status'], limit: number = 100): QueueItem[] {
    const stmt = this.db.prepare(`
      SELECT * FROM workflow_queue
      WHERE status = ?
      ORDER BY priority DESC, created_at ASC
  ...
  getStats(): QueueStats {
    ...
    const recentCompleted = (recentStmt.get(Date.now() - 60000) as any).recent_completed;
   ...
   */
  resetStuckItems(timeoutMs: number = 600000): number {
