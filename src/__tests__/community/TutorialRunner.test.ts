...
import {
  ...
  resetGlobalRunner,
  type Tutorial,
} from '../../community/TutorialRunner.js'

describe('TutorialRunner', () => {
  let runner: TutorialRunner
  ...
  describe('Starting Tutorials', () => {
    ...
    it('should throw when starting non-existent tutorial', async () => {
      await expect(runner.startTutorial('non-existent')).rejects.toThrow(
        'Tutorial "non-existent" not found'
      )
    })
    ...
    it('should throw when starting tutorial while another is running', async () => {
      ...
      await expect(runner.startTutorial('tutorial2')).rejects.toThrow(
        'Already running tutorial'
      )
    })
