  ...
  speckit
    ...
    .action(async (description: string, options) => {
      ...
      try {
        ...
        console.log(
          chalk.white(`   Agents: ${chalk.yellow(result.metadata.agentsUsed.join(', '))}`)
        );
        console.log(
          chalk.white(`   Complexity: ${formatComplexity(result.metadata.complexity)}`)
        );
        console.log(
          chalk.white(
            `   Est. Duration: ${chalk.yellow(formatDuration(result.metadata.estimatedDuration))}`
          )
        );
        ...
        if (specOptions.includeCost) {
          console.log(
            chalk.white(
              `   Est. Cost: ${chalk.yellow('$' + result.metadata.estimatedCost.toFixed(4))}`
            )
          );
        }
        ...
        console.log(
          chalk.white(`   2. Execute: ${chalk.bold(`ax workflow run ${result.outputPath}`)}`)
        );
        console.log(
          chalk.white(`   3. Visualize: ${chalk.bold(`ax gen dag ${result.outputPath}`)}`)
        );
        console.log();
