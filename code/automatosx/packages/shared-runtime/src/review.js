...
export async function runReviewAnalysis(traceStore, request) {
...
export async function listReviewTraces(traceStore, limit) {
...
async function collectFiles(inputPaths, maxFiles, basePath) {
    const results = [];
...
async function visit(filePath, results, maxFiles) {
    if (results.length >= maxFiles) {
        return;
    }
    let stats;
...
function scanFile(filePath, content, focus, basePath) {
...
function pushFinding(findings, focus, category, pattern, finding, line) {
...
function buildMarkdownReport(traceId, focus, files, findings, counts) {
    const lines = [
        `# Review ${traceId}`,
        '',
        `- Focus: ${focus}`,
