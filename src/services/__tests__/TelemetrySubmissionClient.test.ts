...
import { TelemetryEvent, SubmissionConfig } from '../../types/schemas/telemetry.schema.js';

describe('TelemetrySubmissionClient', () => {
  let client: TelemetrySubmissionClient;
  let mockFetch: any;
  const testConfig: SubmissionConfig = {
  ...
  describe('constructor', () => {
    ...
    it('should apply default timeout if not provided', () => {
      ...
      client = new TelemetrySubmissionClient(configWithoutTimeout as SubmissionConfig);
    ...
    it('should throw error for non-HTTPS endpoint (not localhost)', () => {
      ...
      expect(() => new TelemetrySubmissionClient(httpConfig)).toThrow(
        'Telemetry endpoint must use HTTPS for security'
      );
    });
  ...
  describe('getServerInfo', () => {
    ...
    it('should return server info on success', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://telemetry.example.com/info',
        expect.objectContaining({
          method: 'GET',
        })
      );
