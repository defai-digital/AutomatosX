...
**Current Version**: v5.12.3 (October 2025)
**Status**: Production Ready
**Language**: TypeScript (ESM modules, strict mode)
**Node**: >=20.0.0
**Recommended Interface**: Gemini CLI (with Claude Code/OpenAI as fallbacks)

## Quick Reference

```bash
# Development workflow
npm run dev -- run backend "task"    # Test CLI changes
npm run build                         # Build (runs prebuild:config first)
npm run typecheck                     # Type check before commits
npm test                              # Run all tests

# Testing specific areas
npm run test:unit -- memory          # Test memory system
npm run test:unit -- spec            # Test spec-kit
...
npm run test:unit -- memory          # All memory-related tests
npm run test:unit -- router          # Router tests
npm run test:unit -- spec            # Spec-kit tests
npm run test:unit -- path/to/file    # Specific test file
...
# Manual spec execution
ax spec run                  # Execute .specify/ tasks
ax spec run --parallel       # Execute with parallel task execution
ax spec status               # Show task completion status
ax spec validate             # Validate .specify/ structure

# Resume interrupted specs
ax spec run --resume         # Resume from last checkpoint
```
...
- **In-process execution** (v5.12.0+): Native agent execution without subprocess overhead for spec-kit tasks
...
### Import Path Aliases
- **`@/*`**: Maps to `./src/*` (e.g., `import { Router } from '@/core/router'`)
- **`@tests/*`**: Maps to `./tests/*` (e.g., `import { mockAgent } from '@tests/helpers'`)
- **Note**: Path aliases are configured in `tsconfig.json` and automatically resolved by `tsx` (dev) and `tsup` (build)
...
- **IMPORTANT**: Never manually edit `src/config.generated.ts` - it will be overwritten. Edit `automatosx.config.json` instead.
...
- **Config versioning**: `automatosx.config.json` has its own version field (v5.12.0, separate from package version)
- **After config changes**: Run `npm run prebuild:config` to regenerate types before building

### Provider Integrations
- **OpenAI**: Subprocess execution via `codex` CLI command (native streaming support, 128K context, priority 1)
- **Gemini CLI**: Subprocess execution via `gemini` CLI command (no native streaming, 1M context, priority 2, **recommended for users**)
- **Claude Code**: Subprocess execution via `claude` CLI command (no native streaming, 200K context, priority 3)
- **Priority system**: Lower number = higher priority. OpenAI (1) used first for best performance, then Gemini (2), then Claude (3)
- **Automatic failover**: When a provider fails or hits limits, router automatically tries next available provider
- **Limit tracking**: System detects usage limits (daily/weekly) and auto-rotates providers until reset time
- **Process management**: All child processes cleaned up on exit via `installExitHandlers()` (fixes hanging tasks in Claude Code)
- **User interface**: While OpenAI has priority 1 for performance, Gemini CLI is the recommended user interface with natural language support
...
- **Task format**: Supports both string and JSON ops formats:
  - String: `- [ ] id:task:name ops:"ax run agent 'task'" dep:dep1,dep2`
  - JSON (v5.12.0+): `- [ ] id:task:name ops:{"agent":"backend","task":"..."} dep:dep1,dep2`
- **JSON format benefits**: More resilient parsing, better error handling, supports complex task configurations
...
- **In-process execution** (v5.12.0+): JSON format enables native agent execution without CLI subprocess overhead

### Testing Strategy
- **Unit tests**: Individual module testing (`tests/unit/`) - 106 test files, 2,384 total passing tests
...
- **Test isolation**: Each test cleans up its own resources (timers, processes, DB connections)
- **Resource leak detection**: `npm run check:timers` validates cleanup
...
### Runtime
- `better-sqlite3`: SQLite bindings (native module, rebuild required on Node version changes)
- `sqlite-vec`: Vector search extension (v0.1.7-alpha.2, optional for Plus features)
- `yargs`: CLI argument parsing (v18.x, older version for stability)
- `chalk`: Terminal colors (v5.x, ESM-only)
- `ora`: Loading spinners (v9.x, ESM-only)
- `boxen`: Terminal boxes (v8.x, ESM-only)
- `async-mutex`: Async locking primitives (prevents race conditions in multi-agent execution)
- `js-yaml`: YAML parsing for agent profiles

### Development
- `typescript`: Strict mode, ESM target (v5.3+)
- `vitest`: Test runner (fast, ESM-native, supports concurrent tests)
- `tsup`: Build tool (fast, minimal config, uses esbuild internally)
- `tsx`: Dev runner (fast, no compilation, great for debugging)
- `husky`: Git hooks (pre-commit, commit-msg, version hooks)
- `@commitlint/cli` + `@commitlint/config-conventional`: Commit message validation
- `standard-version`: Automated CHANGELOG generation and versioning
...
- Native module rebuild: If Node version changed, rebuild `better-sqlite3`: `npm rebuild better-sqlite3`
...
- **figma-expert** (Fiona) - Design-to-code automation (Figma API, design systems)
- **frontend** (Frank) - Frontend development (React/Next.js/Swift)
- **iot-engineer** (Ivy) - IoT, embedded, and robotics engineering (ROS2, edge computing)
- **mobile** (Maya) - Mobile development (iOS/Android, Swift/Kotlin/Flutter)
- **ml-engineer** (Mira) - Deep learning implementation (PyTorch/TensorFlow)
