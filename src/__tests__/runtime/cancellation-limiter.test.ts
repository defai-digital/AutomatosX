...
describe('Token Bucket Rate Limiter', () => {
  ...
  it('rejects cancellations exceeding rate limit', () => {
    ...
    const [finalLimiter, result] = CancellationLimiter.checkCancellation(
      currentLimiter,
      'task-11',
      userId,
      ctx.currentState,
  ...
  it('tracks per-user rate limits independently', () => {
    ...
    const [newLimiter2, result2] = CancellationLimiter.checkCancellation(
      currentLimiter,
      'task-user2-1',
      user2,
      ctx.currentState,
...
describe('Threat T5 Mitigation - Integration Tests', () => {
  ...
  it('enforces dual confirmation for critical EXECUTING state', () => {
    ...
    const [limiter2, result1] = CancellationLimiter.checkCancellation(
      limiter,
      'task-123',
      userId,
      ctx.currentState,
    ...
    const [limiter3, result2] = CancellationLimiter.checkCancellation(
      limiter2,
      'task-123',
      userId,
      ctx.currentState,
  ...
  it('provides complete audit trail for security monitoring', () => {
    ...
    const confirmation = CancellationLimiter.confirmCancellation(
      CancellationLimiter.createConfirmationRequirement(ctx2.currentState)
    )
