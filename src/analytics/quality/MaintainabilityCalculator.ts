...
import { ComplexityMetrics, FileComplexity } from './ComplexityAnalyzer.js';
...
export interface TechnicalDebt {
  ...
  ratio: number; // debt/development time ratio
}
...
export class MaintainabilityCalculator {
   ...
   */
  private calculateGrade(maintainabilityIndex: number): string {
   ...
   * Detect code smells
   */
  private detectCodeSmells(fileComplexity: FileComplexity): CodeSmell[] {
   ...
   */
  private calculateTechnicalDebt(fileComplexity: FileComplexity, codeSmells: CodeSmell[]): TechnicalDebt {
    let totalMinutes = 0;
   ...
   */
  private calculateQualityScore(fileComplexity: FileComplexity, codeSmells: CodeSmell[]): number {
    let score = 100;
   ...
   */
  private generateRecommendations(fileComplexity: FileComplexity, codeSmells: CodeSmell[]): string[] {
    const recommendations: string[] = [];

    // High complexity recommendations
    const highComplexityFunctions = fileComplexity.functions.filter(
      f => f.cyclomatic.complexity > this.thresholds.maxCyclomaticComplexity
    );
    if (highComplexityFunctions.length > 0) {
      recommendations.push(
        `Refactor ${highComplexityFunctions.length} high-complexity function(s): ${highComplexityFunctions.map(f => f.name).join(', ')}`
      );
    }

    // Long function recommendations
    const longFunctions = fileComplexity.functions.filter(
      f => f.linesOfCode > this.thresholds.maxFunctionLines
    );
    if (longFunctions.length > 0) {
      recommendations.push(
        `Break down ${longFunctions.length} long function(s) into smaller, focused functions`
      );
    }
    ...
    if (fileComplexity.functions.length > this.thresholds.maxFunctionsPerFile) {
      recommendations.push(
        `Consider splitting file into multiple modules (currently ${fileComplexity.functions.length} functions)`
      );
    }

    // Nesting recommendations
    const deeplyNestedFunctions = fileComplexity.functions.filter(
      f => f.cognitive.nestingPenalty > 10
    );
    ...
    if (qualityScore < 50) {
      recommendations.push(
        'Consider rewriting this file - quality score is critically low'
      );
    } else if (qualityScore < 70) {
      recommendations.push(
        'Prioritize refactoring this file in the next sprint'
      );
    }
   ...
   */
  calculateDebtTrend(historicalData: { timestamp: Date; debt: number }[]): {
