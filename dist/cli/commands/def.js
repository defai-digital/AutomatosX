 ...
 */
function displayDefinition(symbolName, kind, filePath, line, column, endLine, content, contextLines) {
    ...
    // Header
    console.log();
    console.log(colorFn(chalk.bold(`${kind} `)) +
    ...
    for (let i = startLine - 1; i < stopLine; i++) {
        ...
        if (isDefinitionLine) {
            console.log(chalk.yellow(`${lineNum.toString().padStart(4)} │ `) +
                chalk.bold(lineContent));
        }
        else {
            console.log(chalk.dim(`${lineNum.toString().padStart(4)} │ `) +
 ...
 * Create def command
 */
export function createDefCommand() {
    ...
    defCommand
        ...
        try {
            ...
            for (const symbol of symbolsToShow) {
                ...
                displayDefinition(symbol.name, symbol.kind, symbol.file_path, symbol.line, symbol.column, symbol.end_line, file.content, contextLines);
            }
