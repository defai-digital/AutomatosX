...
describe('LRU Cache Performance', () => {
  let cache: LRUCache<string, TestValue>;

  beforeEach(() => {
    cache = new LRUCache<string, TestValue>({
      max: 1000,
      ttl: 60000, // 1 minute
    });
  });
  ...
  bench('LRU eviction (1000 items, max=500)', () => {
    const smallCache = new LRUCache<string, TestValue>({ max: 500 });
...
describe('Cache Concurrency Patterns', () => {
  bench('alternating set/get (simulates real usage)', () => {
    const cache = new LRUCache<string, TestValue>({ max: 1000 });
...
describe('TTL (Time-To-Live) Performance', () => {
  bench('set with TTL', () => {
    const cache = new LRUCache<string, TestValue>({
      max: 1000,
      ttl: 5000, // 5 seconds
    });
...
describe('Complex Object Caching', () => {
  ...
  bench('cache complex objects (50 items)', () => {
    const cache = new LRUCache<string, ComplexObject>({ max: 100 });
