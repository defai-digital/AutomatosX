...
import { SubmissionConfig } from '../../types/schemas/telemetry.schema.js';
...
describe('TelemetryService remote integration', () => {
  ...
  const queueCount = (): number => {
    const row = db.prepare('SELECT COUNT(*) as count FROM telemetry_queue').get() as { count: number };
    return row.count;
  };

  const getQueueRows = (): any[] => {
    return db
      .prepare(
        'SELECT id, event_id, retry_count, next_retry_at as nextRetryAt, last_error as lastError FROM telemetry_queue ORDER BY id'
      )
      .all();
  };

  const getQueueEntry = (): any => {
    return db
      .prepare(
        'SELECT id, event_id, retry_count as retryCount, next_retry_at as nextRetryAt, last_error as lastError FROM telemetry_queue LIMIT 1'
      )
      .get();
  };

  const trackSampleEvent = async (command: string = 'index') => {
  ...
  afterEach(() => {
    (service as any)?.stopBackgroundSubmission?.();
    vi.useRealTimers();
    vi.restoreAllMocks();
    try {
      db.close();
  ...
  it('rate limiter stops submissions when tokens are unavailable', async () => {
    ...
    (service as any).rateLimiter = limiterStub;
  ...
  it('submission client exceptions keep events queued for later processing', async () => {
    ...
    (service as any).submissionClient.submitBatch = vi.fn().mockRejectedValue(new Error('boom'));
