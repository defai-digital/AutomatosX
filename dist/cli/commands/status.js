...
export function createStatusCommand() {
    const command = new Command('status')
        ...
        try {
            ...
            indexTable.push(['Files Indexed', chalk.green(stats.totalFiles.toLocaleString())], ['Symbols Extracted', chalk.yellow(stats.totalSymbols.toLocaleString())], ['Search Chunks', chalk.blue(stats.totalChunks.toLocaleString())], [
                'Avg Symbols/File',
                stats.totalFiles > 0
            ...
            const hitRateColor = cacheStats.hitRate > 0.5 ? chalk.green :
                cacheStats.hitRate > 0.25 ? chalk.yellow :
                    chalk.red;
            cacheTable.push(['Entries Cached', chalk.blue(cacheStats.size.toLocaleString())], ['Max Size', chalk.gray(cacheStats.maxSize.toLocaleString())], ['Utilization', chalk.gray(formatPercent(cacheStats.size / cacheStats.maxSize))], ['Cache Hits', chalk.green(cacheStats.hits.toLocaleString())], ['Cache Misses', chalk.red(cacheStats.misses.toLocaleString())], ['Hit Rate', hitRateColor(formatPercent(cacheStats.hitRate))], ['Evictions', chalk.yellow(cacheStats.evictions.toLocaleString())]);
