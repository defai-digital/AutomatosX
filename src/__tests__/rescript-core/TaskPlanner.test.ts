...
const createTask = (
  ...
  TaskPlanner.createTask(id, `Task ${id}`, estimatedDuration, dependencies, requiredResources, priority)
...
describe('Parallel Task Grouping', () => {
  ...
  it('should group tasks at same dependency level', () => {
    ...
    if (result.TAG === 'Ok') {
      ...
      const secondLevelGroup = plan.parallelGroups.find(g =>
        g.some(taskId => ['B', 'C', 'D'].includes(taskId))
      )
...
describe('Plan Optimization', () => {
  ...
  it('should respect dependencies despite priority', () => {
    ...
    if (result.TAG === 'Ok') {
      ...
      expect(plan.executionOrder.indexOf('A')).toBeLessThan(
        plan.executionOrder.indexOf('B')
      )
    }
  })
