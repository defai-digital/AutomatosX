...
export class QwenHybridAdapter {
  ...
  // Mutex for circuit breaker state synchronization (prevents race conditions)
  private readonly circuitMutex = new Mutex();
   ...
   */
  private async isCircuitClosed(breaker: CircuitBreaker): Promise<boolean> {
    return this.circuitMutex.runExclusive(() => {
      if (!breaker.isOpen) {
        return true;
      }
   ...
   * Uses mutex to prevent race conditions with concurrent requests
   */
  private async recordFailure(breaker: CircuitBreaker): Promise<void> {
    await this.circuitMutex.runExclusive(() => {
      breaker.failures++;
   ...
   * Uses mutex to prevent race conditions with concurrent requests
   */
  private async recordSuccess(breaker: CircuitBreaker): Promise<void> {
    await this.circuitMutex.runExclusive(() => {
      breaker.failures = 0;
      breaker.isOpen = false;
    });
  ...
  private async executeWithAuto(request: ExecutionRequest): Promise<ExecutionResponse> {
    ...
    if (await this.isCircuitClosed(this.sdkCircuitBreaker)) {
      const adapter = this.getSdkAdapter();
      ...
      try {
        if (await adapter.isAvailable()) {
          ...
          await this.recordSuccess(this.sdkCircuitBreaker);
          return response;
        }
      } catch (error) {
        ...
        await this.recordFailure(this.sdkCircuitBreaker);
      }
    }

    // Fall back to CLI (if circuit is closed)
    if (await this.isCircuitClosed(this.cliCircuitBreaker)) {
      const wrapper = this.getCliWrapper();
      ...
      try {
        if (await wrapper.isAvailable()) {
          ...
          await this.recordSuccess(this.cliCircuitBreaker);
          return response;
        }
      } catch (error) {
        ...
        await this.recordFailure(this.cliCircuitBreaker);
        throw error;
      }
    }
