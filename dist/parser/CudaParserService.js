...
export class CudaParserService extends BaseLanguageParser {
    ...
    extractFunction(node) {
        ...
        const metadata = {};
        if (isKernel)
            metadata.gpuKernel = true;
        if (isDevice)
            metadata.deviceFunction = true;
        if (isHost)
            metadata.hostFunction = true;
    ...
    extractDeclaration(node) {
        ...
        const metadata = {};
        if (isShared)
            metadata.sharedMemory = true;
        if (isManaged)
            metadata.managedMemory = true;
        return {
            name,
...
//# sourceMappingURL=CudaParserService.js.map