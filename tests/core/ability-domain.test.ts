...
`
      ...
      fs.writeFileSync(
        path.join(testDir, 'clean-code.md'),
        `---
category: engineering
priority: 80
enabled: true
...
`
    ...
    it('should reload abilities', async () => {
      ...
      // Add a new file
      fs.writeFileSync(
        path.join(testDir, 'new-ability.md'),
        `---
...
`
  ...
  describe('DefaultAbilityManager', () => {
    let registry: InMemoryAbilityRegistry;
    let manager: DefaultAbilityManager;
    ...
    describe('getAbilitiesForTask', () => {
      it('should return abilities relevant to task', async () => {
        const abilities = await manager.getAbilitiesForTask(
          'test-agent',
      ...
      it('should prioritize core abilities', async () => {
        const abilities = await manager.getAbilitiesForTask(
          'test-agent',
          'Write some code',
          ['clean-code'] // Explicit core ability
        );
      ...
      it('should respect maxAbilities limit', async () => {
        const abilities = await manager.getAbilitiesForTask(
          'test-agent',
          'Write TypeScript tests with clean code',
          undefined,
          2
        );
    ...
    describe('injectAbilities', () => {
      ...
      it('should respect maxTokens limit and set truncated flag', async () => {
        ...
        const allAbilities = await manager.getAbilitiesForTask(
          'test-agent',
          'Write TypeScript tests with clean code'
        );
        ...
        const result = await manager.injectAbilities(
          ...
          'Write TypeScript tests with clean code',
          undefined,
          { maxTokens: 25, maxAbilities: 10 } // ~100 chars, fits 1 ability only
        );
      ...
      it('should include metadata headers when requested', async () => {
        const result = await manager.injectAbilities(
          ...
          'Write TypeScript code',
          undefined,
          { includeMetadata: true, maxAbilities: 1 }
        );
