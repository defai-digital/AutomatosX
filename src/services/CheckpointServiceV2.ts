...
import { WorkflowStateMachineBridge, type Checkpoint } from '../bridge/WorkflowStateMachineBridge.js';
...
export interface RestoredCheckpoint {
  ...
  machine: WorkflowStateMachineBridge; // ReScript state machine
}
...
export class CheckpointServiceV2 {
   ...
   */
  async restoreCheckpoint(checkpointId: string): Promise<RestoredCheckpoint> {
   ...
   */
  async pruneCheckpoints(executionId: string, keepLatest: number = 5): Promise<number> {
    const checkpoints = await this.listCheckpoints(executionId, 1000);
   ...
   */
  private mapRescriptStateToWorkflowState(rescriptState: string): WorkflowState {
   ...
   * Use with caution.
   */
  async migrateV1CheckpointToV2(checkpointId: string): Promise<WorkflowCheckpoint> {
    const checkpoint = this.dao.getCheckpointById(checkpointId);
    if (!checkpoint) {
      throw new Error(`Checkpoint not found: ${checkpointId}`);
    }
    ...
    const machine = WorkflowStateMachineBridge.create(
      ...
      'migrated-workflow', // TODO: Get actual workflow name
      allSteps
    );
