...
describe('State-Based Guard', () => {
  it('allows transition when current state is in allowed states list', () => {
    const ctx = Guards.createGuardContext(
      runtimeStates.idle,
...
describe('Guard Combinators - AND', () => {
  ...
  it('fails when any guard fails', () => {
    ...
    const guard1 = Guards.alwaysPassGuard
    const guard2 = (ctx: any) => Guards.alwaysFailGuard('second guard failed', ctx)
  ...
  it('defers when any guard defers and no guards fail', () => {
    ...
    const guard1 = Guards.alwaysPassGuard
    const guard2 = (_ctx: any) => ({ TAG: 'Defer', _0: 'async check pending' })
  ...
  it('returns first failure when multiple guards fail', () => {
    ...
    const guard1 = (ctx: any) => Guards.alwaysFailGuard('first failure', ctx)
    const guard2 = (ctx: any) => Guards.alwaysFailGuard('second failure', ctx)
...
describe('Guard Combinators - OR', () => {
  it('passes when at least one guard passes', () => {
    ...
    const guard1 = (ctx: any) => Guards.alwaysFailGuard('first failed', ctx)
    const guard2 = Guards.alwaysPassGuard
  ...
  it('fails when all guards fail', () => {
    ...
    const guard1 = (ctx: any) => Guards.alwaysFailGuard('first failed', ctx)
    const guard2 = (ctx: any) => Guards.alwaysFailGuard('second failed', ctx)
...
describe('Guard Combinators - NOT', () => {
  ...
  it('inverts Fail to Pass', () => {
    ...
    const failGuard = (ctx: any) => Guards.alwaysFailGuard('should be inverted', ctx)
  ...
  it('preserves Defer verdicts', () => {
    ...
    const deferGuard = (_ctx: any) => ({ TAG: 'Defer', _0: 'async pending' })
...
describe('Schema Validation Guard', () => {
  it('passes when schema validation succeeds', () => {
    const validator = (_payload: any) => 'Valid'
  ...
  it('fails when schema validation fails', () => {
    const validator = (_payload: any) => ({ TAG: 'Invalid', _0: 'field "name" is required' })
    const metadata = Object.create(null)
  ...
  it('fails when no metadata is provided for validation', () => {
    const validator = (_payload: any) => ({ TAG: 'Valid' })
    const ctx = Guards.createGuardContext(
      runtimeStates.idle,
      runtimeEvents.dependenciesReady,
      undefined
...
describe('Execute Guards - Batch Execution', () => {
  ...
  it('stops at first guard failure', () => {
    ...
    const guards: Array<[any, string]> = [
      [Guards.alwaysPassGuard, 'guard1'],
      [(ctx: any) => Guards.alwaysFailGuard('guard2 failed', ctx), 'guard2'],
