...
describe('Memory System Error Handling', () => {
    ...
    describe('Input Validation', () => {
        it('should reject conversation with empty agentId', async () => {
            await expect(memoryService.createConversation({
                agentId: '',
                title: 'Test',
            })).rejects.toThrow();
        });
        ...
        it('should reject message with empty conversationId', async () => {
            await expect(memoryService.addMessage({
                conversationId: '',
                role: 'user',
                content: 'Test',
            })).rejects.toThrow();
        ...
        it('should reject search with empty query', async () => {
            await expect(memoryService.searchMessages({
                query: '',
                limit: 10,
                offset: 0,
                sortBy: 'relevance',
        ...
        it('should reject list with limit > 100', async () => {
            await expect(memoryService.listConversations({
                limit: 101,
                offset: 0,
                sortBy: 'createdAt',
                sortOrder: 'desc',
    ...
    describe('Cache Error Handling', () => {
        it('should handle null/undefined cache keys', () => {
            ...
            expect(cache.getConversation(null)).toBeNull();
            expect(cache.getConversation(undefined)).toBeNull();
            expect(cache.getMessage(null)).toBeNull();
            expect(cache.getMessage(undefined)).toBeNull();
        });
    ...
    describe('Export/Import Errors', () => {
        it('should handle export to non-existent directory', async () => {
            ...
            await expect(exporter.exportToJSON('/nonexistent/path/export.json', {})).rejects.toThrow();
        });
        it('should handle import from non-existent file', async () => {
            const exporter = new MemoryExporter(memoryService);
            await expect(exporter.importFromJSON('/nonexistent/file.json')).rejects.toThrow();
        });
        it('should handle import with invalid JSON', async () => {
            ...
            await expect(exporter.importFromJSON(tempFile)).rejects.toThrow();
            // Cleanup
    ...
    describe('Concurrency Edge Cases', () => {
        it('should handle concurrent conversation creates', async () => {
            const promises = Array.from({ length: 10 }, (_, i) => memoryService.createConversation({
                agentId: 'test-agent',
        ...
        it('should handle concurrent message adds to same conversation', async () => {
            ...
            const promises = Array.from({ length: 10 }, (_, i) => memoryService.addMessage({
                conversationId: conversation.id,
                role: 'user',
        ...
        it('should handle concurrent searches', async () => {
            ...
            const promises = Array.from({ length: 10 }, () => memoryService.searchMessages({
                query: 'test',
                limit: 10,
                offset: 0,
                sortBy: 'relevance',
