...
describe('FileService - Query Filters (Simple)', () => {
    ...
    let counter = 0;
    const u = (name) => `/test-filter-${Date.now()}-${++counter}-${name}`;
...
`;
        ...
        const symbolResults = result.results.filter(r => r.type === 'symbol');
        if (symbolResults.length > 0) {
            const classResults = symbolResults.filter(r => r.kind === 'class' && r.file_path.endsWith('.ts'));
            expect(classResults.length).toBe(symbolResults.length);
        }
    });
...
//# sourceMappingURL=FileService-Filters.simple.test.js.map