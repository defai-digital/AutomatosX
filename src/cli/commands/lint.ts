 ...
 */
const LINT_PATTERNS: Record<string, { pattern: string; description: string; severity: 'error' | 'warning' | 'info' }> = {
  todo: {
    pattern: 'TODO',
...
function displayLintResults(
  ...
  for (const result of results) {
    ...
    const matchingLine = lines.find((line) =>
      line.toLowerCase().includes(pattern.toLowerCase().split(' ')[0])
    ) || lines[0];
  ...
  lintCommand
    ...
    .action(async (pattern: string | undefined, options) => {
      try {
        ...
        let patternsToRun: Array<{ name: string; pattern: string; description: string; severity: 'error' | 'warning' | 'info' }> = [];
        ...
        for (const lintPattern of patternsToRun) {
          ...
          displayLintResults(
            lintPattern.pattern,
            lintPattern.description,
            lintPattern.severity,
            results
          );
