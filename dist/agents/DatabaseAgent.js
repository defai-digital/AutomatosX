...
export class DatabaseAgent extends AgentBase {
    ...
    buildDatabasePrompt(task, context, codeContext, pastSolutions) {
    ...
    parseDatabaseArtifacts(response) {
        const artifacts = [];
        ...
        while ((match = codeBlockRegex.exec(response)) !== null) {
            ...
            if (language === 'sql') {
                if (content.toUpperCase().includes('CREATE TABLE') || content.toUpperCase().includes('CREATE INDEX'))
                    name = 'schema-ddl';
                else if (content.toUpperCase().includes('ALTER TABLE'))
                    name = 'migration-script';
                else if (content.toUpperCase().includes('SELECT'))
