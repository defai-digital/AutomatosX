 ...
 */
export function parseWorkflowDefinition(jsonString) {
 ...
 */
export function parseWorkflowContext(jsonString) {
    const parsed = JSON.parse(jsonString);
 ...
 */
export function canResumeFromState(state) {
    return ['paused', 'failed'].includes(state);
}
/**
 * Check if a step execution state is terminal
 */
export function isStepTerminalState(state) {
