...
export class PDFExporter {
   ...
   * Add summary section
   */
  private addSummary(doc: jsPDF, report: QualityReport, yPosition: number): number {
   ...
   * Add metrics section
   */
  private addMetrics(doc: jsPDF, report: QualityReport, yPosition: number): number {
   ...
   */
  private addCodeSmells(doc: jsPDF, report: QualityReport, yPosition: number): number {
    ...
    topSmells.forEach((smell, index) => {
      ...
      filePathLines.forEach((line: string) => {
   ...
   */
  private addFooter(doc: jsPDF, report: QualityReport): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Generated by AutomatosX', this.margin, this.pageHeight - 10);
