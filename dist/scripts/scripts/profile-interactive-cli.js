    ...
    printReport() {
        ...
        for (const [category, metrics] of Object.entries(categories)) {
            ...
            for (const metric of metrics) {
                ...
                console.log(`${chalk.white(metric.operation.padEnd(50))} ${timeColor(time.padStart(8))}ms ${memColor(memDelta.padStart(8))}MB`);
            }
            // Summary stats
            ...
            console.log(chalk.gray('─'.repeat(80)));
            console.log(chalk.cyan(`Average: ${avgTime.toFixed(2)}ms | Max: ${maxTime.toFixed(2)}ms | Total Memory: ${(totalMem / 1024 / 1024).toFixed(2)}MB`));
        }
        console.log(chalk.gray('\n' + '─'.repeat(80)));
        console.log(chalk.bold.green('✓ Profiling Complete\n'));
    }
}
async function profileCommands(profiler) {
...
async function profileContextOperations(profiler) {
...
async function profileDatabaseOperations(profiler) {
...
async function profileAutoSave(profiler) {
...
async function main() {
    ...
    try {
        ...
        console.log(`${chalk.white('Average command execution time:')} ${avgCommandTime < 50 ? chalk.green('✓') : chalk.yellow('⚠')} ${avgCommandTime.toFixed(2)}ms ${avgCommandTime < 50 ? chalk.green('(Good)') : chalk.yellow('(Could be improved)')}`);
        console.log(`${chalk.white('Average database operation time:')} ${avgDbTime < 10 ? chalk.green('✓') : chalk.yellow('⚠')} ${avgDbTime.toFixed(2)}ms ${avgDbTime < 10 ? chalk.green('(Excellent)') : chalk.yellow('(Acceptable)')}`);
        console.log(`${chalk.white('Auto-save time (5 messages):')} ${autoSaveTime < 100 ? chalk.green('✓') : chalk.yellow('⚠')} ${autoSaveTime.toFixed(2)}ms ${autoSaveTime < 100 ? chalk.green('(Non-blocking)') : chalk.red('(Needs optimization)')}`);
        console.log(chalk.gray('─'.repeat(80)));
