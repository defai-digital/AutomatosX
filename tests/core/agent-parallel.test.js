...
import { createAgentParallelOrchestrator, createDAGAnalyzer, createContextManager, createResultAggregator, DAGAnalysisError, StubAgentExecutor, findCyclePath, AggregationStrategies, createKeyedAggregator, } from '@defai.digital/agent-parallel';
import { createAgentParallelTask, ParallelExecutionErrorCodes, } from '@defai.digital/contracts';
// ============================================================================
// DAG Analyzer Tests
// ============================================================================
...
describe('ResultAggregator', () => {
    ...
    const createResult = (agentId, output, success = true, layer = 0) => ({
    ...
    describe('list strategy (INV-APE-004)', () => {
        it('should return array of results', () => {
            ...
            const aggregated = aggregator.aggregate(results, { strategy: 'list' });
        ...
        it('should order by taskId', () => {
            ...
            const aggregated = aggregator.aggregate([result2, result1], { strategy: 'list' });
    ...
    describe('custom strategy', () => {
        it('should use custom aggregator function', () => {
            ...
            const customAggregator = (r) => r.filter((x) => x.success).reduce((sum, x) => sum + x.output, 0);
        ...
        it('should throw if no custom aggregator provided', () => {
            expect(() => aggregator.aggregate([], { strategy: 'custom' })).toThrow('customAggregator function');
        });
    });
});
...
describe('createKeyedAggregator', () => {
    it('should group results by key', () => {
        ...
        const result = aggregator(results);
...
describe('AgentParallelOrchestrator', () => {
    ...
    describe('executeParallel', () => {
        ...
        it('should handle agent not found', async () => {
            ...
            const result = await orchestrator.executeParallel(tasks);
            expect(result.allSucceeded).toBe(false);
            expect(result.taskResults[0]?.errorCode).toBe(ParallelExecutionErrorCodes.AGENT_NOT_FOUND);
        });
    ...
    describe('cancel', () => {
        it('should cancel ongoing execution', async () => {
            ...
            // Should complete but with cancelled status
            expect(result.cancelledTasks?.length).toBeGreaterThan(0);
        });
    });
    describe('getConfig', () => {
...
//# sourceMappingURL=agent-parallel.test.js.map