...
export class APIAgent extends AgentBase {
  ...
  private buildAPIPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseAPIArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'yaml' || language === 'json') {
        ...
        else if (content.includes('paths:') || content.includes('endpoints')) name = 'api-spec';
