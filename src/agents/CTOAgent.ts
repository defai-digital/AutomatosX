...
export class CTOAgent extends AgentBase {
  ...
  private buildCTOPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseCTOArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'markdown' || language === 'md') {
        if (content.includes('# Technical Strategy') || content.includes('# Roadmap')) name = 'technical-strategy';
        else if (content.includes('# Architecture Decision Record') || content.includes('# ADR')) name = 'adr-document';
        else if (content.includes('# Team Structure') || content.includes('# Hiring')) name = 'team-plan';
        else if (content.includes('# Technology Evaluation') || content.includes('# Vendor')) name = 'tech-evaluation';
