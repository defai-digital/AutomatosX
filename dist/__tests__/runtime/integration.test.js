...
describe('Integration - Manifest Validation → Dependency Checks → Rule Evaluation', () => {
    it('validates manifest, checks dependencies, and evaluates rules for secure task submission', () => {
        ...
        const manifest = ManifestValidator.createManifest('integration-task-001', '1.0.0', ['dep-a', 'dep-b'], undefined, 'valid-signature', undefined);
        const hmacVerifier = (_canonical, _key, sig) => sig === 'valid-signature';
        ...
        const checksumVerifier = (actual, expected) => actual === expected;
        ...
        const rule = PolicyDSL.createPolicyRule('secure-submission-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('secure-policy', '1.0.0', 'Secure submission policy', [rule]);
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, policy, undefined);
    ...
    it('rejects tampered manifest signature in integration flow', () => {
        const secretKey = 'integration-secret-key';
        const manifest = ManifestValidator.createManifest('tampered-task', '1.0.0', ['dep-a'], undefined, 'tampered-signature', undefined);
        const hmacVerifier = (_canonical, _key, sig) => sig === 'valid-signature';
        const manifestResult = ManifestValidator.validateManifest(manifest, secretKey, hmacVerifier);
    ...
    it('blocks rule violation in integration flow', () => {
        const rule = PolicyDSL.createPolicyRule('block-executing-rule', [PolicyDSL.stateIsCondition('EXECUTING')], PolicyDSL.denyAction('cannot submit task while executing'));
        const policy = PolicyDSL.createPolicy('blocking-policy', '1.0.0', 'Block policy', [rule]);
        const context = RuleEngine.createExecutionContext(runtimeStates.executing, runtimeEvents.dependenciesReady, undefined, policy, undefined);
        const outcome = RuleEngine.execute(context);
    ...
    it('validates config bundle with all dependencies and signature', () => {
        ...
        const depA = DependencyValidator.createDependency('dep-a', '1.0.0', checksumA, undefined);
        const depB = DependencyValidator.createDependency('dep-b', '2.0.0', checksumB, undefined);
        const bundle = DependencyValidator.createConfigBundle('integration-bundle', '1.0.0', [depA, depB], 'valid-bundle-sig', undefined);
        const bundleSignatureVerifier = (_canonical, _key, sig) => {
            return sig === 'valid-bundle-sig';
        };
        const checksumVerifier = (actual, expected) => actual === expected;
        const getActualChecksum = (depName) => {
            if (depName === 'dep-a')
                return checksumA;
            if (depName === 'dep-b')
                return checksumB;
        ...
        const result = DependencyValidator.validateConfigBundle(bundle, secretKey, getActualChecksum, checksumVerifier, bundleSignatureVerifier);
...
describe('Integration - Event Authentication → Replay Detection → State Transitions', () => {
    ...
    it('authenticates event and transitions state machine', () => {
        ...
        const context = StateMachine.makeContext(true, // dependenciesReady
        undefined, undefined, undefined, undefined);
        const outcome = StateMachine.transition(runtimeStates.preparing, runtimeEvents.dependenciesReady, context);
    ...
    it('attaches trace context to all events for telemetry', () => {
        ...
        const authEvents = events.map((event, idx) => EventAuth.authenticateEvent(event, idx + 1, uuidGen));
    ...
    it('combines event authentication with rule evaluation', () => {
        ...
        const rule = PolicyDSL.createPolicyRule('auth-rule', [PolicyDSL.eventIsCondition('DEPS_READY')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('auth-policy', '1.0.0', 'Auth policy', [rule]);
        const context = RuleEngine.createExecutionContext(runtimeStates.preparing, authEvent.event, undefined, policy, undefined);
        const outcome = RuleEngine.execute(context);
        expect(RuleEngine.isPassed(outcome.result)).toBe(true);
    });
});
describe('Integration - Complete Secure Task Flow', () => {
    it('executes complete secure task submission flow', () => {
        ...
        // 1. Validate manifest
        const manifest = ManifestValidator.createManifest('complete-flow-task', '1.0.0', ['dep-x', 'dep-y'], undefined, 'valid-signature', undefined);
        const hmacVerifier = (_canonical, _key, sig) => sig === 'valid-signature';
        const manifestResult = ManifestValidator.validateManifest(manifest, secretKey, hmacVerifier);
        expect(manifestResult.TAG).toBe('Ok');
        ...
        // 4. Evaluate rules
        const rule = PolicyDSL.createPolicyRule('flow-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('flow-policy', '1.0.0', 'Complete flow policy', [rule]);
        const ruleContext = RuleEngine.createExecutionContext(runtimeStates.idle, authEvent.event, undefined, policy, undefined);
    ...
    it('rejects insecure task submission at manifest validation stage', () => {
        ...
        const manifest = ManifestValidator.createManifest('insecure-task', '1.0.0', ['dep-a'], undefined, undefined, undefined);
    ...
    it('rejects insecure task submission at dependency validation stage', () => {
        const secretKey = 'security-key';
        // Valid manifest
        const manifest = ManifestValidator.createManifest('task-with-bad-deps', '1.0.0', ['dep-a'], undefined, 'valid-signature', undefined);
        const hmacVerifier = (_canonical, _key, sig) => sig === 'valid-signature';
        const manifestResult = ManifestValidator.validateManifest(manifest, secretKey, hmacVerifier);
        expect(manifestResult.TAG).toBe('Ok');
    ...
    it('rejects insecure task submission at rule evaluation stage', () => {
        ...
        expect(authResult.TAG).toBe('Ok');
        // Block via rule
        const rule = PolicyDSL.createPolicyRule('block-rule', [PolicyDSL.stateIsCondition('EXECUTING')], PolicyDSL.denyAction('task submission blocked during execution'));
        const policy = PolicyDSL.createPolicy('block-policy', '1.0.0', 'Blocking policy', [rule]);
        const context = RuleEngine.createExecutionContext(runtimeStates.executing, authEvent.event, undefined, policy, undefined);
        const outcome = RuleEngine.execute(context);
        expect(RuleEngine.isFailed(outcome.result)).toBe(true);
