...
describe('ClaudeProvider', () => {
    ...
    describe('request', () => {
        ...
        it('should throw ProviderAuthError on 401', async () => {
            const apiError = new Anthropic.APIError(401, 'Invalid API key');
        ...
        it('should throw ProviderRateLimitError on 429', async () => {
            const apiError = new Anthropic.APIError(429, 'Rate limit exceeded', {
                'retry-after': '60',
            });
            mockCreate.mockRejectedValueOnce(apiError);
            await expect(provider.request({
        ...
        it('should throw ProviderTimeoutError on 408', async () => {
            const apiError = new Anthropic.APIError(408, 'Request timeout');
            mockCreate.mockRejectedValueOnce(apiError);
            await expect(provider.request({
                messages: [{ role: 'user', content: 'Test' }],
        ...
        it('should throw ProviderNetworkError on 500', async () => {
            const apiError = new Anthropic.APIError(500, 'Internal server error');
            mockCreate.mockRejectedValueOnce(apiError);
            await expect(provider.request({
                messages: [{ role: 'user', content: 'Test' }],
        ...
        it('should retry on retryable errors', async () => {
            const apiError = new Anthropic.APIError(503, 'Service unavailable');
            const mockResponse = {
                id: 'msg_123',
                model: CLAUDE_MODELS['claude-sonnet-4-5'],
    ...
    describe('streamRequest', () => {
        ...
        it('should handle streaming errors', async () => {
            const apiError = new Anthropic.APIError(500, 'Streaming error');
            mockCreate.mockRejectedValueOnce(apiError);
    ...
    describe('healthCheck', () => {
        ...
        it('should return unhealthy status on error', async () => {
            const apiError = new Anthropic.APIError(500, 'Service down');
            mockCreate.mockRejectedValueOnce(apiError);
            const health = await provider.healthCheck();
...
//# sourceMappingURL=ClaudeProvider.test.js.map