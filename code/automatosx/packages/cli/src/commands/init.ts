...
interface ProviderIntegrationReport {
...
const INIT_FLAG_MAP = new Map<string, keyof InitFlags>([
  ['--skip-mcp', 'skipMcp'],
...
export async function initCommand(args: string[], options: CLIOptions): Promise<CommandResult> {
  ...
  if (!flags.skipMcp && !flags.skipGemini) {
    const geminiPath = await writeJsonMcpConfig(
      ...
      createStandardMcpConfig({ transport: 'stdio' }),
    );
  ...
  if (!flags.skipMcp && !flags.skipGrok) {
    const grokPath = await writeJsonMcpConfig(
      join(basePath, '.ax-grok', 'settings.json'),
      {
        mcpServers: {
          [MCP_SERVER_ID]: {
            command: MCP_COMMAND,
...
function parseInitFlags(args: string[]): { flags: InitFlags; error?: string } {
  ...
  if (typeof override === 'string' && override.trim().length > 0) {
    const available = new Set(
      override
        ...
        .filter((entry): entry is ProviderId => PROVIDER_CLIENT_IDS.includes(entry as ProviderId)),
    );
    return Object.fromEntries(PROVIDER_CLIENT_IDS.map((providerId) => [providerId, available.has(providerId)])) as Record<ProviderId, boolean>;
  }
...
async function writeClaudeCodeIntegration(basePath: string, tools: string[]): Promise<string[]> {
  ...
  const existingSettings = await readJsonFile<ClaudeSettings>(settingsPath) ?? {};
...
function createStandardMcpConfig(extra: { transport?: string } = {}): JsonMcpConfig {
  return {
    mcpServers: {
      [MCP_SERVER_ID]: {
        command: MCP_COMMAND,
...
async function writeJsonMcpConfig(path: string, value: ProjectMcpConfig | JsonMcpConfig): Promise<string> {
  const existing = await readJsonFile<ProjectMcpConfig | JsonMcpConfig>(path) ?? {};
  const merged = {
    ...existing,
    mcpServers: {
      ...(existing.mcpServers ?? {}),
...
function buildAxMd(projectName: string): string {
  return [
