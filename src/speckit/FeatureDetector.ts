...
export class FeatureDetector {
  ...
  private async detectUIFeatures(): Promise<DetectedFeature[]> {
    ...
    const uiFiles = results.filter(r =>
      (r.file.includes('component') || r.file.includes('page')) &&
  ...
  private async detectIntegrationFeatures(): Promise<DetectedFeature[]> {
    const integrations = [
      { name: 'Stripe', keywords: ['stripe', 'payment'], category: 'payment' as const },
      { name: 'SendGrid', keywords: ['sendgrid', 'email'], category: 'notification' as const },
      { name: 'AWS S3', keywords: ['s3', 'aws-sdk'], category: 'integration' as const },
    ];
   ...
   */
  private extractDependencies(results: any[], libs: string[]): string[] {
   ...
   */
  private inferComponentType(name: string, file: string): 'page' | 'component' | 'widget' | 'layout' {
