...
import { GuardPositionSchema, GuardFailActionSchema, WorkflowStepGuardSchema, GuardCheckStatusSchema, StepGateResultSchema, StepGuardResultSchema, StepGuardPolicySchema, StepGuardContextSchema, StageProgressStatusSchema, StageProgressEventSchema, GoalAnchorConfigSchema, GoalAnchorContextSchema, DEFAULT_STEP_GUARD, createStepGuardResult, createProgressEvent, } from '@defai.digital/contracts';
describe('Step Guard Contract', () => {
    ...
    describe('WorkflowStepGuardSchema', () => {
        ...
        it('requires non-empty guardId and stepId', () => {
            expect(() => WorkflowStepGuardSchema.parse({
                guardId: '',
                stepId: 'step-1',
                position: 'before',
                gates: ['test'],
    ...
    describe('StepGateResultSchema', () => {
        ...
        it('requires message field', () => {
            expect(() => StepGateResultSchema.parse({
                gateId: 'test',
                status: 'FAIL',
            })).toThrow();
        });
    ...
    describe('createStepGuardResult', () => {
        it('creates valid result', () => {
            const result = createStepGuardResult('guard-1', 'step-1', 'before', [{ gateId: 'test', status: 'PASS', message: 'Passed' }], false);
            expect(result.guardId).toBe('guard-1');
        ...
        it('validates created result against schema', () => {
            const result = createStepGuardResult('guard-1', 'step-1', 'after', [{ gateId: 'validation', status: 'FAIL', message: 'Failed' }], true);
            expect(() => StepGuardResultSchema.parse(result)).not.toThrow();
        });
    });
    describe('createProgressEvent', () => {
        it('creates valid progress event', () => {
            const event = createProgressEvent('exec-123', 'agent-1', 0, 5, 'init', 'prompt', 'starting');
        ...
        it('includes optional fields', () => {
            const event = createProgressEvent('exec-123', 'agent-1', 1, 5, 'process', 'tool', 'completed', {
                sessionId: 'session-1',
                durationMs: 100,
            });
        ...
        it('validates created event against schema', () => {
            const event = createProgressEvent('exec-123', 'agent-1', 2, 5, 'validate', 'conditional', 'failed', { error: 'Validation failed' });
            expect(() => StageProgressEventSchema.parse(event)).not.toThrow();
        });
    });
});
//# sourceMappingURL=step-guard.test.js.map