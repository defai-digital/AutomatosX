...
export class TestingAgent extends AgentBase {
  ...
  private buildTestingPrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parseTestingArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'typescript' || language === 'javascript') {
        if (content.includes('describe(') && content.includes('it(')) {
          if (content.includes('vitest') || content.includes('from \'vitest\'')) name = 'vitest-test';
          else if (content.includes('jest') || content.includes('from \'@jest')) name = 'jest-test';
          else name = 'unit-test';
        ...
        if (content.includes('def test_') || content.includes('import pytest')) name = 'pytest-test';
        else if (content.includes('import unittest')) name = 'unittest-test';
      } else if (language === 'java') {
        if (content.includes('@Test') || content.includes('import org.junit')) name = 'junit-test';
      } else if (language === 'json') {
        if (content.includes('jest') || content.includes('coverage')) name = 'jest-config';
        else if (content.includes('playwright')) name = 'playwright-config';
      }
