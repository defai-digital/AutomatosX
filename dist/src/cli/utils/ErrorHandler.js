...
(function (ErrorCategory) {
    ...
    ErrorCategory["DIRECTORY_NOT_FOUND"] = "DIRECTORY_NOT_FOUND";
    ErrorCategory["NO_FILES_TO_INDEX"] = "NO_FILES_TO_INDEX";
    ErrorCategory["NO_RESULTS_FOUND"] = "NO_RESULTS_FOUND";
    ErrorCategory["INVALID_QUERY"] = "INVALID_QUERY";
    ErrorCategory["DATABASE_ERROR"] = "DATABASE_ERROR";
    ErrorCategory["PARSER_ERROR"] = "PARSER_ERROR";
    ErrorCategory["MIGRATION_ERROR"] = "MIGRATION_ERROR";
...
export class ErrorHandler {
    ...
    static enhance(error) {
        ...
        if (errorMsg.includes('ENOENT') || errorMsg.includes('not found')) {
            if (errorMsg.includes('directory')) {
                return this.directoryNotFound(errorMsg, error);
            }
            return this.fileNotFound(errorMsg, error);
        }
        if (errorMsg.includes('EACCES') || errorMsg.includes('permission denied')) {
            return this.permissionError(errorMsg, error);
        }
        if (errorMsg.includes('database') || errorMsg.includes('sqlite')) {
            return this.databaseError(errorMsg, error);
        }
        if (errorMsg.includes('parse') || errorMsg.includes('syntax')) {
            return this.parserError(errorMsg, error);
        }
        if (errorMsg.includes('migration')) {
            return this.migrationError(errorMsg, error);
        }
        return this.unknownError(errorMsg, error);
    }
    /**
     * File not found error
     */
    static fileNotFound(message, originalError) {
        return {
     ...
     */
    static directoryNotFound(message, originalError) {
        return {
     ...
     */
    static noFilesToIndex(extensions) {
        return {
     ...
     * Invalid query error
     */
    static invalidQuery(query, reason) {
        return {
     ...
     * Database error
     */
    static databaseError(message, originalError) {
        return {
     ...
     * Parser error
     */
    static parserError(message, originalError) {
        return {
     ...
     * Migration error
     */
    static migrationError(message, originalError) {
        return {
     ...
     * Configuration error
     */
    static configurationError(message, originalError) {
        return {
     ...
     * Permission error
     */
    static permissionError(message, originalError) {
        return {
     ...
     */
    static display(error, verbose = false) {
        console.log();
     ...
     */
    static handleAndExit(error, verbose = false) {
     ...
     */
    static validateDirectoryExists(path) {
        const fs = require('fs');
        try {
     ...
     */
    static validateFileExists(path) {
        const fs = require('fs');
        try {
            const stats = fs.statSync(path);
            if (!stats.isFile()) {
...
export class SuccessMessage {
     ...
     */
    static display(message, details) {
        console.log();
