...
describe('TelemetryQueue', () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      testEventIds.push(result.lastInsertRowid as number);
    }
  });
  ...
  describe('dequeue', () => {
    ...
    it('should not return events scheduled for future retry', () => {
      ...
      db.prepare('UPDATE telemetry_queue SET next_retry_at = ? WHERE id = ?').run(
        futureTime,
        batch1[0].id
      );
  ...
  describe('markFailed', () => {
    ...
    it('should remove event after max retries exceeded', () => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (!current) break; // Already removed
  ...
  describe('getStats', () => {
    ...
    it('should return oldest queued timestamp', () => {
      ...
      db.prepare('INSERT INTO telemetry_queue (event_id, queued_at, retry_count) VALUES (?, ?, 0)').run(
        testEventIds[1],
        time2
      );
