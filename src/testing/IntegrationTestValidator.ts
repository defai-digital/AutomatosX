...
export class IntegrationTestValidator extends EventEmitter {
   ...
   */
  async runScenario(scenarioId: string): Promise<IntegrationTestResult> {
    ...
    try {
      ...
      const assertionResults = await Promise.all(
        scenario.assertions.map((assertion) => this.runAssertion(assertion))
      )
   ...
   * Run assertion
   */
  private async runAssertion(assertion: TestAssertion): Promise<boolean> {
    try {
 ...
 */
export function createIntegrationTestValidator(): IntegrationTestValidator {
 ...
 */
let globalValidator: IntegrationTestValidator | null = null

/**
 * Get global integration test validator
 */
export function getGlobalIntegrationTestValidator(): IntegrationTestValidator {
  if (!globalValidator) {
 ...
 */
export function resetGlobalIntegrationTestValidator(): void {
