...
export class TestAnalyzer {
  ...
  private identifyPhases(steps: WorkflowStep[]): TestPhase[] {
    ...
    levels.forEach((stepIds, index) => {
      ...
      const totalDuration = phaseSteps.reduce(
        (sum, s) => sum + this.estimateStepDuration(s),
        0
      );
   ...
   */
  private identifyRequiredMocks(steps: TestableStep[]): MockRequirement[] {
    const mocks = new Map<string, MockRequirement>();
   ...
   */
  private detectSideEffects(step: WorkflowStep): boolean {
    ...
    return sideEffectActions.some(action =>
   ...
   */
  private determineRequiredMocks(step: WorkflowStep): string[] {
    const mocks: string[] = [];
