...
describe('FailureAnalyzer', () => {
    ...
    describe('Severity Assessment', () => {
        ...
        it('should assess critical severity after many iterations', async () => {
            const error = new Error('Timeout');
            const progress = createMockProgress();
            const history = Array.from({ length: 8 }, (_, i) => createFailedIteration(i + 1, new Error(`Error ${i + 1}`)));
            const analysis = await analyzer.analyze(error, progress, history);
            expect(analysis.severity).toBe('critical');
        });
    });
    ...
    describe('Recommendation Generation', () => {
        ...
        it('should recommend manual intervention after many failures', async () => {
            const error = new Error('Error');
            const progress = createMockProgress();
            const history = Array.from({ length: 7 }, (_, i) => createFailedIteration(i + 1, new Error(`Error ${i + 1}`)));
            const analysis = await analyzer.analyze(error, progress, history);
...
function createFailedIteration(iteration, error) {
    return {
        iteration,
        success: false,
        complete: false,
...
//# sourceMappingURL=FailureAnalyzer.test.js.map