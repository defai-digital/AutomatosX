...
export class StrategyTelemetry {
  ...
  getStrategyStats(strategyName: string): StrategyStats | null {
    ...
    const stats = statsStmt.get(strategyName) as {
    ...
    const failureRows = failureStmt.all(strategyName) as Array<{
      failure_type: string;
      count: number;
    }>;
  ...
  getAllStats(): StrategyStats[] {
    ...
    const rows = stmt.all() as Array<{ strategy_name: string }>;
  ...
  getBestStrategyForFailure(failureType: string): string | null {
    ...
    const row = stmt.get(failureType) as {
  ...
  generateReport(): TelemetryReport {
    ...
    const failureRows = failureStmt.all() as Array<{
      failure_type: string;
      count: number;
    }>;
  ...
  private generateRecommendations(
    ...
    if (underperforming.length > 0) {
      recommendations.push(
        `Consider disabling or improving: ${underperforming.map(s => s.strategyName).join(', ')}`
      );
    }
    ...
    if (topPerformers.length > 0) {
      recommendations.push(
        `Increase priority for high-performing strategies: ${topPerformers.map(s => s.strategyName).join(', ')}`
      );
    }
    ...
    if (entries.length > 0) {
      ...
      if (bestStrategy) {
        recommendations.push(
          `For '${topFailure[0]}' failures (${topFailure[1]} occurrences), use '${bestStrategy}' strategy`
        );
      }
    }
    ...
    if (unused.length > 0) {
      recommendations.push(
        `Underutilized strategies: ${unused.join(', ')} - consider testing these`
      );
    }
  ...
  displayReport(): void {
    ...
    for (const ranking of top5) {
      ...
      console.log(
        chalk.gray(`  ${ranking.rank}. ${ranking.strategy.padEnd(25)} ${bar} ${this.formatSuccessRate(ranking.successRate)} (${ranking.usage} uses)`)
      );
    }
