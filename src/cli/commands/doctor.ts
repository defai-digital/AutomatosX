...
async function runClaudeCodeDiagnostics(verbose: boolean): Promise<void> {
  ...
  checks.push({
    name: 'settings.json',
    passed: diagnostics.settingsConfigured,
    message: diagnostics.settingsConfigured
      ? 'MCP permissions configured in .claude/settings.json'
      : 'MCP permissions not configured',
    fix: diagnostics.settingsConfigured ? undefined : 'Run: ax setup --claude-code'
  });

  // .mcp.json project config
  checks.push({
    name: '.mcp.json',
    passed: diagnostics.mcpProjectConfigGenerated,
    message: diagnostics.mcpProjectConfigGenerated
      ? 'Team MCP config present at project root'
      : 'Missing (team-shared MCP config)',
    fix: diagnostics.mcpProjectConfigGenerated ? undefined : 'Run: ax setup --claude-code'
  });

  // Hook scripts
  checks.push({
    name: 'Hook Scripts',
    passed: diagnostics.hooksGenerated,
    message: diagnostics.hooksGenerated
      ? 'Hook scripts present in .claude/hooks/'
      : 'Hook scripts not generated',
    fix: diagnostics.hooksGenerated ? undefined : 'Run: ax setup --claude-code'
  });
