...
export class PerformanceAgent extends AgentBase {
  ...
  private buildPerformancePrompt(task: Task, context: AgentContext, codeContext: any[], pastSolutions: any[]): string {
  ...
  private parsePerformanceArtifacts(response: string): TaskArtifact[] {
    ...
    while ((match = codeBlockRegex.exec(response)) !== null) {
      ...
      if (language === 'javascript' || language === 'typescript') {
        if (content.includes('k6') || content.includes('import http from')) name = 'k6-load-test';
        else if (content.includes('cache') || content.includes('Redis')) name = 'caching-implementation';
        else if (content.includes('benchmark') || content.includes('performance.now()')) name = 'benchmark-script';
      } else if (language === 'bash' || language === 'sh') {
        if (content.includes('ab ') || content.includes('apache bench')) name = 'apache-bench-test';
        else if (content.includes('wrk') || content.includes('hey')) name = 'load-test-command';
      }
