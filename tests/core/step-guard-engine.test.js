...
import { StepGuardEngine, createStepGuardEngine, createGateRegistry, createProgressTracker, DEFAULT_STEP_GUARD_ENGINE_CONFIG, } from '@defai.digital/workflow-engine';
describe('StepGuardEngine', () => {
    ...
    describe('gate registration', () => {
        it('registers custom gates', async () => {
            const customGate = async (context) => ({
                gateId: 'custom',
                status: 'PASS',
            ...
            const results = [
                {
                    ...
                    position: 'before',
                    status: 'PASS',
                    gates: [{ gateId: 'test', status: 'PASS', message: 'OK' }],
                    blocked: false,
                    summary: 'All passed',
                ...
                },
                {
                    ...
                    position: 'before',
                    status: 'FAIL',
                    gates: [{ gateId: 'test', status: 'FAIL', message: 'Failed' }],
                    blocked: true,
                    summary: '1 failed',
                    timestamp: new Date().toISOString(),
                    durationMs: 0,
    ...
    describe('emitProgress', () => {
        it('calls onProgressEvent callback', () => {
            ...
            engineWithProgress.emitProgress('exec-123', 'agent-1', 0, 5, 'init', 'prompt', 'starting');
        ...
        it('does nothing when no callback configured', () => {
            // Should not throw
            expect(() => { engine.emitProgress('exec-123', 'agent-1', 0, 5, 'init', 'prompt', 'starting'); }).not.toThrow();
        });
    });
});
...
describe('ProgressTracker', () => {
    ...
    beforeEach(() => {
        events = [];
        tracker = createProgressTracker('exec-123', 'agent-1', 5, (event) => events.push(event), 'session-1');
    });
...
//# sourceMappingURL=step-guard-engine.test.js.map