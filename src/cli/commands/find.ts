...
import { FileService, SearchResultType, UnifiedSearchResult } from '../../services/FileService.js';
 ...
 */
function displayResults(results: UnifiedSearchResult[], intent: QueryIntent, query?: string): void {
  if (results.length === 0) {
 ...
 */
function displaySymbolResults(results: UnifiedSearchResult[]): void {
 ...
 */
function displayChunkResults(results: UnifiedSearchResult[]): void {
 ...
 */
function displayHybridResults(results: UnifiedSearchResult[]): void {
  for (let i = 0; i < results.length; i++) {
    const result = results[i];
  ...
  findCommand
    ...
    .action(async (query: string, options) => {
      try {
        ...
        // Determine intent
        let forceIntent: QueryIntent | undefined;
        if (options.symbol) {
