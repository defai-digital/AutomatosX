...
describe('TransitionValidator - Pre-Condition Validation', () => {
    it('validates source state accepts correct events', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Idle' }, { TAG: 'Preparing' }, taskSubmission, context, []);
    ...
    it('rejects invalid source state and event combinations', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Idle' }, { TAG: 'Executing' }, depsReady, context, []);
    ...
    it('validates context requirements for DependenciesReady event', () => {
        ...
        const depsReady = { TAG: 'DependenciesReady' };
        const result = TransitionValidator.validateTransition({ TAG: 'Preparing' }, { TAG: 'Executing' }, depsReady, invalidContext, []);
        expect(result.TAG).toBe('Invalid');
    ...
    it('validates context requirements for CancelRequest event', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Preparing' }, { TAG: 'Canceled' }, cancelEvent, invalidContext, []);
        expect(result.TAG).toBe('Invalid');
    ...
    it('validates guard verdict allows transition from Preparing to Executing', () => {
        ...
        const blockedContext = StateMachine.makeContext(true, StateMachine.blockGuards('Security policy violation'), undefined, undefined);
        const depsReady = { TAG: 'DependenciesReady' };
        const result = TransitionValidator.validateTransition({ TAG: 'Preparing' }, { TAG: 'Executing' }, depsReady, blockedContext, []);
        expect(result.TAG).toBe('Invalid');
...
describe('TransitionValidator - Post-Condition Validation', () => {
    it('validates correct destination state for transitions', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Bootstrapping' }, { TAG: 'Idle' }, depsReady, context, [{ TAG: 'EmitTelemetry', _0: 'runtime.bootstrapped' }]);
        expect(result).toBe('Valid');
    });
    it('rejects invalid destination states for transitions', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Bootstrapping' }, { TAG: 'Executing' }, depsReady, context, []);
        expect(result.TAG).toBe('Invalid');
    ...
    it('validates rollback effects are only used for Failed/Canceled states', () => {
        ...
        const invalidResult = TransitionValidator.validateTransition({ TAG: 'Executing' }, { TAG: 'Completed' }, timeout, context, [{ TAG: 'PerformRollback', _0: 'timeout' }]);
        ...
        const validResult = TransitionValidator.validateTransition({ TAG: 'Executing' }, { TAG: 'Failed' }, timeout, context, [{ TAG: 'PerformRollback', _0: 'timeout:30000' }]);
...
describe('TransitionValidator - Comprehensive Validation', () => {
    it('validates complete transition pipeline with all checks', () => {
        ...
        const result = TransitionValidator.validateTransition({ TAG: 'Preparing' }, { TAG: 'Executing' }, depsReady, context, effects);
        expect(result).toBe('Valid');
    });
    it('determines appropriate error recovery actions for validation failures', () => {
        const context = StateMachine.makeContext(false, StateMachine.allowGuards(), undefined, undefined);
        ...
        const [result, recoveryAction] = TransitionValidator.validateWithRecovery({ TAG: 'Preparing' }, { TAG: 'Executing' }, depsReady, context, [], validationContext);
...
describe('TransitionValidator - Error Recovery', () => {
    it('determines rollback strategy for timeout errors', () => {
        const timeout = { TAG: 'Timeout', _0: 30000 };
        const strategy = TransitionValidator.determineRollbackStrategy({ TAG: 'Executing' }, { TAG: 'Failed' }, timeout);
    ...
    it('determines rollback strategy for rule violations', () => {
        ...
        const strategy = TransitionValidator.determineRollbackStrategy({ TAG: 'Executing' }, { TAG: 'Failed' }, ruleViolation);
    ...
    it('determines rollback strategy for cancellations', () => {
        const cancelEvent = StateMachine.makeCancellationEvent('user-123');
        const strategy = TransitionValidator.determineRollbackStrategy({ TAG: 'Preparing' }, { TAG: 'Canceled' }, cancelEvent);
    ...
    it('aborts transition after too many retry attempts', () => {
        const context = StateMachine.defaultContext;
        const validationContext = TransitionValidator.createValidationContext(context, undefined, 3, // Too many attempts
        undefined);
        const depsReady = { TAG: 'DependenciesReady' };
        const [result, recoveryAction] = TransitionValidator.validateWithRecovery({ TAG: 'Preparing' }, { TAG: 'Executing' }, depsReady, context, [], validationContext);
        // Should fail validation
...
//# sourceMappingURL=transition-validator.test.js.map