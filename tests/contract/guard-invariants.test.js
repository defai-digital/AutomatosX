...
describe('INV-GUARD-RAD: Change Radius Gate', () => {
    ...
    function countPackages(changedFiles) {
...
describe('INV-GUARD-POL: Policy Resolution', () => {
    describe('INV-GUARD-POL-001: Deterministic Resolution', () => {
        it('should return same context for same inputs', () => {
            ...
            const resolvedAllowed = policy.allowedPaths.map((p) => p.replace(/\{\{(\w+)\}\}/g, (_, key) => variables[key] ?? ''));
            const resolvedAllowed2 = policy.allowedPaths.map((p) => p.replace(/\{\{(\w+)\}\}/g, (_, key) => variables[key] ?? ''));
    ...
    describe('INV-GUARD-POL-002: Variable Substitution', () => {
        it('should substitute all {{variable}} placeholders', () => {
            ...
            const resolved = pattern.replace(/\{\{(\w+)\}\}/g, (_, key) => variables[key] ?? '');
...
//# sourceMappingURL=guard-invariants.test.js.map