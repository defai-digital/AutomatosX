...
export class MarketplaceAnalyticsAPI extends EventEmitter {
  constructor(private analytics: MarketplaceAnalytics) {
    super()
  }
   ...
   */
  getCommunityOverview(period: 'daily' | 'weekly' | 'monthly' = 'weekly'): CommunityOverview {
  ...
  getPluginDetailAnalytics(pluginId: string): PluginDetailAnalytics | null {
    ...
    const detail: PluginDetailAnalytics = {
      pluginId,
      metrics,
      downloadsTrend,
      versionAdoption,
   ...
   */
  getTrendingPlugins(period: 'daily' | 'weekly' | 'monthly', limit: number = 10) {
   ...
   */
  private aggregateDownloadsTrend(period: 'daily' | 'weekly' | 'monthly'): TrendData {
    const allAnalytics = this.analytics.getAllAnalytics()
  ...
  ): { timestamp: number; average: number; count: number }[] {
    const allAnalytics = this.analytics.getAllAnalytics()
    ...
    return sampleTrend.timestamps.map((timestamp, index) => {
      const allRatings = allAnalytics.flatMap((plugin) => {
        ...
        return ratings.filter(
          (r) => r.timestamp >= timestamp && r.timestamp < (sampleTrend.timestamps[index + 1] || Date.now())
        )
      })
   ...
   */
  private convertToCSV(detail: PluginDetailAnalytics): string {
    const lines: string[] = []
   ...
   */
  private convertOverviewToCSV(overview: CommunityOverview): string {
    const lines: string[] = []
 ...
 */
export function createMarketplaceAnalyticsAPI(
 ...
 * Global API instance
 */
let globalAPI: MarketplaceAnalyticsAPI | null = null

/**
 * Get global analytics API
 */
export function getGlobalAPI(analytics: MarketplaceAnalytics): MarketplaceAnalyticsAPI {
  if (!globalAPI) {
 ...
 * Reset global API
 */
export function resetGlobalAPI(): void {
  globalAPI = null
}
