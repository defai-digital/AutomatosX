...
} from '../../packages/cli/src/workflow-adapter.js';
  ...
  it('preserves preview artifact contract for all five workflow commands', async () => {
    for (const commandCase of COMMAND_CASES) {
      ...
      const input = parseWorkflowCommandInput(
        ...
        [...commandCase.args, '--dry-run', '--output-dir', tempDir],
        'claude',
      );
...
async function expectWorkflowArtifacts(
  manifestPath: string | undefined,
  summaryPath: string | undefined,
  artifactPaths: string[] | undefined,
  expected: {
    commandId: WorkflowCommandId;
    status: 'preview' | 'dispatched' | 'failed';
    traceId: string;
    error?: string;
  },
): Promise<void> {
