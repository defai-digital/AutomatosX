...
// Extended DependencyState with filters
...
const initialState: ExtendedDependencyState = {
  graph: null,
  selectedNode: null,
  filters: {
    nodeTypes: [],
...
// Async thunks
export const fetchDependencyGraph = createAsyncThunk(
  'dependency/fetchGraph',
  async (projectPath: string) => {
    ...
    return response.json() as Promise<DependencyGraph>;
  }
);

export const analyzeDependencies = createAsyncThunk(
  'dependency/analyze',
  async (projectPath: string) => {
    // TODO: Replace with actual API call
...
const dependencySlice = createSlice({
  ...
  initialState,
  reducers: {
    selectNode: (state, action: PayloadAction<DependencyNode | null>) => {
      state.selectedNode = action.payload;
    },
    clearSelection: (state) => {
      state.selectedNode = null;
    },
    setFilters: (state, action: PayloadAction<Partial<DependencyFilters>>) => {
      state.filters = { ...state.filters, ...action.payload };
    },
    clearFilters: (state) => {
      state.filters = initialState.filters;
    },
    setLayoutAlgorithm: (state, action: PayloadAction<'force' | 'hierarchical' | 'circular'>) => {
...
// Basic selectors
export const selectDependencyGraph = (state: RootState) => state.dependency.graph;
export const selectSelectedNode = (state: RootState) => state.dependency.selectedNode;
export const selectDependencyFilters = (state: RootState) => (state.dependency as ExtendedDependencyState).filters;
export const selectLayoutAlgorithm = (state: RootState) => (state.dependency as ExtendedDependencyState).layoutAlgorithm;
export const selectShowLabels = (state: RootState) => (state.dependency as ExtendedDependencyState).showLabels;
export const selectNodeSize = (state: RootState) => (state.dependency as ExtendedDependencyState).nodeSize;
export const selectDependencyLoading = (state: RootState) => state.dependency.loading;
export const selectDependencyError = (state: RootState) => state.dependency.error;

// Circular dependencies selector
export const selectCircularDependencies = createSelector(
...
export const selectGraphStatistics = createSelector(
  ...
  (graph, circularDeps) => {
    ...
    const isolatedNodesCount = graph.nodes.filter(
