...
export class OnboardingAnalytics extends EventEmitter {
  ...
  getFunnelMetrics(): FunnelMetrics[] {
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      const journeysAtStage = this.getAllJourneys().filter((j) =>
        j.stages.some((s) => s.stage === stage)
      )

      const nextStage = stages[i + 1]
      const conversions = nextStage
        ? this.getAllJourneys().filter((j) =>
            j.stages.some((s) => s.stage === nextStage)
          ).length
        : 0
      ...
      const times = journeysAtStage
        .map((j) => {
          ...
          if (stageIndex >= 0 && stageIndex < j.stages.length - 1) {
            return (
              j.stages[stageIndex + 1].timestamp - j.stages[stageIndex].timestamp
            )
          }
          return null
        })
  ...
  getStatistics(): {
    ...
    const totalDuration = completedJourneys.reduce(
      (sum, j) => sum + (j.lastActivityTime - j.startTime),
      0
    )

    const averageJourneyDuration =
      completedJourneys.length > 0 ? totalDuration / completedJourneys.length : 0
  ...
  getConversionRate(fromStage: FunnelStage, toStage: FunnelStage): number {
    const journeysAtFrom = this.getAllJourneys().filter((j) =>
      j.stages.some((s) => s.stage === fromStage)
    ).length

    const journeysAtTo = this.getAllJourneys().filter((j) =>
      j.stages.some((s) => s.stage === toStage)
    ).length
  ...
  getDropoffRate(stage: FunnelStage): number {
    ...
    const journeysAtNext = this.getAllJourneys().filter((j) =>
      j.stages.some((s) => s.stage === nextStage)
    ).length
 ...
 */
export function createOnboardingAnalytics(): OnboardingAnalytics {
 ...
 */
let globalAnalytics: OnboardingAnalytics | null = null

/**
 * Get global analytics
 */
export function getGlobalAnalytics(): OnboardingAnalytics {
  if (!globalAnalytics) {
