...
export class AlertManager {
     ...
     */
    async handleTriggeredAlert(rule, currentValue) {
    ...
    async createAlert(alert) {
        ...
        this.db
          ...
          labels, annotations, notified, created_at, updated_at
            ...
            .run(newAlert.id, newAlert.ruleId, newAlert.ruleName, newAlert.state, newAlert.severity, newAlert.startedAt, newAlert.currentValue, newAlert.thresholdValue, newAlert.provider || null, newAlert.model || null, newAlert.labels ? JSON.stringify(newAlert.labels) : null, newAlert.annotations ? JSON.stringify(newAlert.annotations) : null, newAlert.notified ? 1 : 0, newAlert.createdAt, newAlert.updatedAt);
        return newAlert;
    }
    /**
     * Update an alert
    ...
    getActiveAlerts(severity) {
        ...
        return results.map((row) => this.rowToAlert(row));
    }
    /**
    ...
    async saveRule(rule) {
        ...
        this.db
            .prepare(`INSERT OR REPLACE INTO alert_rules (
          id, name, description, metric, operator, threshold, duration_seconds,
          provider, model, severity, enabled, labels, annotations,
          created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(alertRule.id, alertRule.name, alertRule.description || null, alertRule.metric, alertRule.operator, alertRule.threshold, alertRule.durationSeconds, alertRule.provider || null, alertRule.model || null, alertRule.severity, alertRule.enabled ? 1 : 0, alertRule.labels ? JSON.stringify(alertRule.labels) : null, alertRule.annotations ? JSON.stringify(alertRule.annotations) : null, alertRule.createdAt, alertRule.updatedAt);
        return alertRule;
    }
    /**
     * Get all alert rules
