...
export function validateMCPServer(
  ...
  if (transport === 'http' || transport === 'sse') {
    // HTTP/SSE transports require url, not command
    if (!server.url) {
      errors.push(`Server "${name}" missing required field: url (required for ${transport} transport)`);
    } else if (typeof server.url !== 'string') {
      errors.push(`Server "${name}" url must be a string`);
    } else if (server.url.trim().length === 0) {
      errors.push(`Server "${name}" url cannot be empty`);
    }
  } else {
  ...
  // Validate args (optional array, stdio only)
