...
import { CLIOutputFixtureSchema, } from '@defai.digital/contracts';
import { parseOutput, classifyError, } from '@defai.digital/provider-adapters';
 ...
 */
function loadProviderFixtures(provider) {
...
describe('CLI Output Consumer-Driven Contracts', () => {
        ...
        for (const provider of PROVIDERS) {
            it(`${provider} fixtures should be valid`, () => {
                ...
                for (const file of files) {
                    ...
                    const data = JSON.parse(content);
                    const result = CLIOutputFixtureSchema.safeParse(data);
                    expect(result.success, `Fixture ${file} should be valid: ${result.success ? '' : result.error.message}`).toBe(true);
                }
            });
        }
    });
        ...
        for (const provider of PROVIDERS) {
            ...
            for (const fixture of fixtures) {
                it(`${provider}/${fixture.scenario} should parse without exception`, () => {
                    expect(() => parseOutput(fixture.rawOutput.stdout, fixture.outputFormat)).not.toThrow();
                });
            }
        }
    });
    ...
    describe('Provider Output Format Documentation', () => {
        it('should have fixtures for all providers', () => {
            for (const provider of PROVIDERS) {
                const fixtures = loadProviderFixtures(provider);
                expect(fixtures.length, `Provider ${provider} should have at least one fixture`).toBeGreaterThan(0);
            }
        });
        it('should have at least one success fixture per provider', () => {
            for (const provider of PROVIDERS) {
                const fixtures = loadProviderFixtures(provider);
                const successFixtures = fixtures.filter((f) => !f.expected.isError);
                expect(successFixtures.length, `Provider ${provider} should have at least one success fixture`).toBeGreaterThan(0);
            }
        });
    });
    describe('Edge Cases', () => {
...
//# sourceMappingURL=cli-outputs.test.js.map