...
export class SecurityAgent extends AgentBase {
    constructor() {
        super({
            ...
            description: 'Expert in security auditing, threat modeling, vulnerability assessment, and secure coding practices. Specializes in identifying and mitigating security risks.',
            capabilities: [
                {
    ...
    async executeTask(task, context, options) {
        ...
        try {
            ...
            const relevantCode = await context.codeIntelligence.searchCode(task.description + ' security auth');
    ...
    buildSecurityPrompt(task, context, codeContext, pastAudits) {
    ...
    parseSecurityArtifacts(response) {
        const artifacts = [];
        // Extract code blocks
