...
import { DiscussionExecutor, StubProviderExecutor, createSimpleProviderBridge, } from '@defai.digital/discussion-domain';
// ============================================================================
// Test Helpers
// ============================================================================
/**
 ...
 */
function createDiscussionExecutorAdapter(executor) {
    return {
        async execute(config, options) {
            ...
            if (options?.onProgress) {
                executorOptions.onProgress = options.onProgress;
            }
            // Cast the config to the expected format
            const result = await executor.execute(config, executorOptions);
            return result;
        },
    };
}
...
describe('Discussion Workflow Integration', () => {
    let stubProviderExecutor;
    let discussionExecutor;
    let discussionExecutorAdapter;
    beforeEach(() => {
...
describe('Provider Bridge', () => {
    describe('createSimpleProviderBridge', () => {
        it('should create a working provider bridge', async () => {
            ...
            providers.set('test-provider', (prompt) => Promise.resolve({
        ...
        it('should handle provider errors gracefully', async () => {
            const providers = new Map();
            providers.set('failing-provider', () => Promise.reject(new Error('Provider failed')));
            const bridge = createSimpleProviderBridge(providers);
            const result = await bridge.execute({
...
//# sourceMappingURL=discussion-workflow.test.js.map