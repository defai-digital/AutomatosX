      ...
      try {
        ...
        if (config) {
          configTable.push(
            ...
            ['Remote Submission', config.remote ? chalk.green('Yes') : chalk.red('No')],
            ['Session ID', chalk.gray(config.sessionId.substring(0, 16) + '...')],
            [
              'Consent Date',
              config.consentDate
        ...
        if (config && config.enabled && config.remote) {
          ...
          if (queueStats) {
            ...
            queueTable.push(
              ['Pending Events', chalk.yellow(queueStats.pending.toString())],
              ['Retrying Events', chalk.blue(queueStats.retrying.toString())],
              ['Total in Queue', chalk.cyan(queueStats.total.toString())]
            );
        ...
        if (config && config.enabled) {
          ...
          storageTable.push(
            ['Total Events', chalk.yellow(eventCount.toLocaleString())]
          );
