...
import { ManifestGenerator } from '../../integrations/claude-code/manifest-generator.js';
...
export const setupCommand: CommandModule<Record<string, unknown>, SetupOptions> = {
  ...
  builder: (yargs) => {
    return yargs
      ...
      .option('agents-only', {
        describe: 'Only regenerate .claude/agents/ subagent files (skip MCP registration and hooks)',
        type: 'boolean',
        default: false
      })
      .option('dry-run', {
        describe: 'Preview what Claude Code setup would generate without writing any files',
        type: 'boolean',
        default: false
      });
  },
  ...
  handler: async (argv) => {
    ...
    try {
      ...
      if (argv.claudeCode || argv.agentsOnly || argv.dryRun) {
        const isDryRun = argv.dryRun ?? false;
        const isAgentsOnly = argv.agentsOnly ?? false;

        if (isDryRun) {
          console.log(chalk.blue('\n🔍 Dry run — Claude Code setup preview (no files written)\n'));
        } else if (isAgentsOnly) {
          console.log(chalk.blue('\n🤖 Regenerating Claude Code agent files...\n'));
        } else {
        ...
        try {
          ...
          if (isDryRun) {
            ...
            if (agentNames.length === 0) {
              console.log(chalk.yellow('   ⚠ No agent profiles found in .automatosx/agents/'));
              console.log(chalk.gray('     Run ax setup first to install example agents, then retry.'));
            } else {
              // 8 fixed files + 2 per-agent (command + subagent)
              const fixedFiles = [
                '.claude/skills/automatosx/SKILL.md',
                '.claude/agents/automatosx-coordinator.md',
                '.mcp.json',
                '.claude/settings.json (MCP permissions + hooks)',
              ...
              for (const name of agentNames) {
                console.log(chalk.gray(`     .claude/commands/agent-${name}.md`));
                console.log(chalk.gray(`     .claude/agents/${name}.md`));
              }
              console.log(chalk.cyan(`\n   Total: ${fixedFiles.length + agentNames.length * 2} files across ${agentNames.length} agents\n`));
            }
            claudeCodeSetupSucceeded = true;
          } else if (isAgentsOnly) {
            // Regenerate only agent subagent files
            const generator = new ManifestGenerator({ profileLoader, projectDir });
            await generator.generatePerAgentSubAgents();
            const agentNames = await profileLoader.listProfiles();
            console.log(chalk.green(`   ✓ Regenerated ${agentNames.length} agent subagent files in .claude/agents/`));
            claudeCodeSetupSucceeded = true;
          } else {
