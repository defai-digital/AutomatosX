...
export async function sessionCommand(args, options) {
    ...
    switch (subcommand) {
        case 'list': {
            ...
            const lines = [
                'Sessions:',
                ...sessions.map((session) => (`- ${session.sessionId} ${session.status} ${session.initiator} ${session.task}`)),
            ];
...
function parseJsonInput(input, allowEmpty = false) {
    if (input === undefined) {
