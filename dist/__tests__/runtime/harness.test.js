...
describe('runtime harness', () => {
    it('promotes idle tasks through preparing into execution when guards allow', () => {
        ...
        const idleToPreparing = StateMachine.transition(runtimeStates.idle, submissionEvent, buildGuardContext({ dependenciesReady: true }));
        expect(StateMachine.serializeOutcome(idleToPreparing)).toMatchObject({
            status: 'transitioned',
            toState: 'PREPARING',
        });
        const preparingToExecuting = StateMachine.transition(runtimeStates.preparing, runtimeEvents.dependenciesReady, buildGuardContext({ dependenciesReady: true, guardVerdict: { kind: 'allow' } }));
    ...
    it('rejects execution when policy guard blocks the transition', () => {
        const blockedOutcome = StateMachine.transition(runtimeStates.preparing, runtimeEvents.dependenciesReady, buildGuardContext({
    ...
    it('routes events and placeholder effects through the dispatcher', () => {
        ...
        const dispatchOutcome = EventDispatcher.dispatchWithHandlers(runtimeStates.idle, runtimeEvents.taskSubmitted(buildSubmission('task-dispatcher')), buildGuardContext({ dependenciesReady: true }), handlers);
