...
describe('Monitoring System Integration', () => {
    ...
    describe('End-to-End Workflow Monitoring', () => {
        it('should track complete workflow lifecycle with metrics, traces, and logs', async () => {
            ...
            // 8. Verify metrics
            const aggregation = metrics.getAggregation('workflow.step.duration', Date.now() - 1000, Date.now());
    ...
    describe('Alert Evaluation and Triggering', () => {
        it('should trigger alert when metric threshold is exceeded', async () => {
            return new Promise((resolve) => {
                ...
                const ruleId = alerting.createRule('High Error Rate', 'Error rate exceeds 10% in last 1 minute', 'workflow.errors', {
                    operator: 'gt',
                    threshold: 5,
                    windowMs: 60000,
                    aggregation: 'sum',
                }, 'critical', { cooldownMs: 1000 });
        ...
        it('should respect cooldown period for alerts', async () => {
            const ruleId = alerting.createRule('High CPU', 'CPU usage above 80%', 'system.cpu', {
                operator: 'gt',
                threshold: 80,
                windowMs: 10000,
                aggregation: 'avg',
            }, 'warning', { cooldownMs: 5000 });
            let alertCount = 0;
    ...
    describe('Metrics Aggregation and Percentiles', () => {
        it('should calculate accurate percentiles for latency metrics', () => {
            ...
            const aggregation = metrics.getAggregation('api.latency', Date.now() - 1000, Date.now());
            expect(aggregation).toBeDefined();
