...
import { LogLevelSchema, OutputFormatSchema, ConfigScopeSchema, ProviderConfigSchema, ExecutionPolicySchema, FeatureFlagsSchema, WorkspaceConfigSchema, UserPreferencesSchema, validateConfig, safeValidateConfig, validateConfigWithErrors, DEFAULT_CONFIG, CONFIG_VERSION, 
// Provider config
ProviderIdSchema, ProviderDetectionResultSchema, KNOWN_PROVIDERS, isKnownProvider, getProviderDefault, createEmptyDetectionResult, 
// Events
ConfigStatusSchema, ConfigEventTypeSchema, ConfigEventSchema, CONFIG_TRANSITIONS, isValidConfigTransition, createConfigCreatedEvent, createConfigUpdatedEvent, 
// Operations
SetupInputSchema, ConfigGetInputSchema, ConfigSetInputSchema, ConfigResetInputSchema, validateSetupInput, validateConfigGetInput, validateConfigSetInput, 
// Errors
ConfigErrorCode, ConfigError, getRecoveryHint, isConfigError, configNotFoundError, } from '@defai.digital/contracts';
describe('Config Contract V1', () => {
    ...
    describe('ProviderConfig Schema', () => {
        ...
        it('should reject priority out of range', () => {
            expect(ProviderConfigSchema.safeParse({ priority: 0 }).success).toBe(false);
            expect(ProviderConfigSchema.safeParse({ priority: 101 }).success).toBe(false);
        });
    });
    describe('ExecutionPolicy Schema', () => {
        ...
        it('should reject invalid timeout values', () => {
            expect(ExecutionPolicySchema.safeParse({ defaultTimeoutMs: 500 }).success).toBe(false);
            expect(ExecutionPolicySchema.safeParse({ defaultTimeoutMs: 700000 }).success).toBe(false);
        });
        it('should reject invalid retry values', () => {
            expect(ExecutionPolicySchema.safeParse({ maxRetries: -1 }).success).toBe(false);
            expect(ExecutionPolicySchema.safeParse({ maxRetries: 11 }).success).toBe(false);
        });
    });
...
describe('Provider Config Contract', () => {
    ...
    describe('getProviderDefault', () => {
        ...
        it('should return undefined for unknown providers', () => {
            const result = getProviderDefault('unknown-provider');
...
describe('Config Operations Contract', () => {
    describe('SetupInput', () => {
        it('should validate setup input', () => {
            const input = { force: true, scope: 'global' };
    ...
    describe('ConfigGetInput', () => {
        it('should validate get input', () => {
            const input = { path: 'logLevel', scope: 'merged' };
    ...
    describe('ConfigResetInput', () => {
        it('should validate reset input', () => {
            const input = { confirm: true, scope: 'global' };
...
describe('Config Errors Contract', () => {
    ...
    describe('ConfigError', () => {
        it('should create error with code and message', () => {
            const error = new ConfigError(ConfigErrorCode.CONFIG_NOT_FOUND, 'Config file not found');
            expect(error.code).toBe(ConfigErrorCode.CONFIG_NOT_FOUND);
            expect(error.message).toBe('Config file not found');
            expect(error.name).toBe('ConfigError');
        });
        it('should create error with details', () => {
            const error = new ConfigError(ConfigErrorCode.CONFIG_VALIDATION_ERROR, 'Validation failed', { path: 'logLevel', value: 'invalid' });
...
describe('INV-CFG-GOV: Config Governance Invariants', () => {
    describe('INV-CFG-GOV-001: Audit Trail', () => {
        ...
        it('should create events with timestamps', () => {
            const event = createConfigCreatedEvent('global', DEFAULT_CONFIG, crypto.randomUUID(), 1);
        ...
        it('should include old and new values in update events', () => {
            const event = createConfigUpdatedEvent('global', 'logLevel', 'info', 'debug', crypto.randomUUID(), 1);
...
describe('INV-CFG-ADP: Config Adapter Invariants', () => {
    ...
    describe('INV-CFG-ADP-003: No Network in Detection', () => {
        ...
        it('should only detect known providers', () => {
            ...
            const unknownDefault = getProviderDefault('unknown-provider');
