...
import type { WorkflowDefinition, ScaffoldOptions } from '../types/speckit.types.js';
...
describe('ScaffoldGenerator', () => {
  ...
  describe('Scaffold Generation', () => {
    ...
    it('should validate workflow before scaffolding', async () => {
      ...
      await expect(
        generator.generateScaffold(invalidWorkflow, {})
      ).rejects.toThrow();
    });

    it('should detect duplicate step IDs', async () => {
      const duplicateWorkflow: WorkflowDefinition = {
        name: 'Duplicate Test',
        version: '1.0.0',
        steps: [
      ...
      await expect(
        generator.generateScaffold(duplicateWorkflow, {})
      ).rejects.toThrow('Duplicate step ID');
    });

    it('should detect invalid dependencies', async () => {
      const invalidDepWorkflow: WorkflowDefinition = {
      ...
      await expect(
        generator.generateScaffold(invalidDepWorkflow, {})
