...
> **Note (v8.2.0)**: AutomatosX removed the standalone chatbot (`ax cli`). Use your preferred AI assistant (Claude Code, Gemini CLI, OpenAI Codex) for conversations and have them invoke `ax` commands.

This project follows the [AGENTS.md standard](https://agents.md) for cross-tool compatibility.

**Supported AI Assistants:**
- **Claude Code** - Recommended, with MCP support
- **Gemini CLI** - Google's natural language CLI
- **OpenAI Codex** - OpenAI's development assistant
- **Cursor/Copilot/Codeium/Windsurf** - IDE-integrated assistants

**How to Use:**
- Any AI assistant with shell access can issue `ax` commands; wrap them in `bash -lc` when using the Codex CLI harness.
- For IDE tools, instruct the assistant to run the desired `ax run ...` command verbatim.
