...
describe('End-to-End Workflows', () => {
    ...
    describe('multi-step workflows', () => {
        ...
        it('should handle parallel task execution', async () => {
            ...
            const results = await Promise.all(tasks.map((task) => runtime.executeTask({
                taskId: task.taskId,
    ...
    describe('checkpoint workflows', () => {
        ...
        it('should restore and continue from checkpoint', async () => {
            ...
            if (status && !status.isActive) {
                const result2 = await runtime.resumeTask('restore-1', provider, {
    ...
    describe('performance workflows', () => {
        ...
        it('should handle concurrent burst of tasks', async () => {
            ...
            const taskCount = 10;
            const startTime = Date.now();
            const tasks = Array.from({ length: taskCount }, (_, i) => runtime.executeTask({
                taskId: `burst-${i}`,
                agentName: 'backend',
                provider,
                request: {
