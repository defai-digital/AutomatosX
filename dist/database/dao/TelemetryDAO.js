...
export class TelemetryDAO {
    ...
    saveEvent(event) {
      ...
      VALUES (?, ?, ?, ?)
    `);
        const result = stmt.run(event.sessionId, event.eventType, event.eventData ? JSON.stringify(event.eventData) : null, event.timestamp);
    ...
    batchInsertEvents(events) {
      ...
      `);
            for (const event of events) {
                stmt.run(event.sessionId, event.eventType, event.eventData ? JSON.stringify(event.eventData) : null, event.timestamp);
            }
        });
        insertMany(events);
    ...
    `);
        stmt.run(stats.statDate, stats.statType, stats.statKey, stats.count, stats.totalDuration || null, stats.avgDuration || null, stats.minDuration || null, stats.maxDuration || null, stats.metadata ? JSON.stringify(stats.metadata) : null);
    }
    /**
     ...
     * @returns Array of telemetry statistics
     */
    getStatsByType(statType, limit = 100) {
        const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
        const rows = stmt.all(statType, limit);
        return rows.map(this.statsRecordToStats);
    }
    /**
    ...
    async saveConfig(config) {
        ...
        WHERE id = 1
      `);
            stmt.run(config.enabled !== undefined ? (config.enabled ? 1 : 0) : (existingConfig.enabled ? 1 : 0), config.remote !== undefined ? (config.remote ? 1 : 0) : (existingConfig.remote ? 1 : 0), config.sessionId || existingConfig.sessionId, config.consentDate || existingConfig.consentDate || null, config.optOutDate || existingConfig.optOutDate || null, Date.now());
        }
        else {
            // Insert new config
            const stmt = this.db.prepare(`
        ...
        VALUES (1, ?, ?, ?, ?, ?)
      `);
            stmt.run(config.enabled ? 1 : 0, config.remote ? 1 : 0, config.sessionId || '', config.consentDate || null, config.optOutDate || null);
        }
    }
    /**
     ...
     */
    statsRecordToStats(record) {
        return {
            statDate: record.stat_date,
            statType: record.stat_type,
...
//# sourceMappingURL=TelemetryDAO.js.map