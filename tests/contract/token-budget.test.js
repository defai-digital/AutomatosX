...
import { TokenBudgetConfigSchema, EmbeddedInstructionSchema, BudgetAllocationSchema, BudgetStatusSchema, InstructionTypeSchema, InstructionPrioritySchema, DEFAULT_TOKEN_BUDGET, PRIORITY_ORDER, TYPE_PRIORITY_ORDER, validateTokenBudgetConfig, safeValidateTokenBudgetConfig, validateEmbeddedInstruction, getPriorityValue, getTypePriorityValue, TokenBudgetErrorCode, } from '@defai.digital/contracts';
describe('Token Budget Contract', () => {
    ...
    describe('Validation Functions', () => {
        it('validateTokenBudgetConfig should throw on invalid input', () => {
            expect(() => validateTokenBudgetConfig({ maxTotal: 10 })).toThrow();
        });
        ...
        it('validateEmbeddedInstruction should throw on invalid input', () => {
            expect(() => validateEmbeddedInstruction({ type: 'invalid' })).toThrow();
        });
    });
    describe('Default Budget', () => {
        ...
        it('per-type limits should not exceed maxTotal', () => {
            const totalPerType = Object.values(DEFAULT_TOKEN_BUDGET.perType).reduce((sum, val) => sum + (val ?? 0), 0);
    ...
    describe('Error Codes', () => {
        it('should have all required error codes', () => {
            ...
            expect(TokenBudgetErrorCode.TOKEN_CRITICAL_DROPPED).toBe('TOKEN_CRITICAL_DROPPED');
