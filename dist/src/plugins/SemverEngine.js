...
export class SemverEngine {
     ...
     */
    checkCompatibility(version, range) {
     ...
     */
    findConflicts(dependencies) {
        const conflicts = [];
        for (const [pkg, versions] of dependencies.entries()) {
            if (versions.length > 1) {
                // Check if all versions are compatible
                const uniqueMajors = new Set(versions.map((v) => this.major(v)).filter((m) => m !== null));
 ...
 */
export function createSemverEngine() {
