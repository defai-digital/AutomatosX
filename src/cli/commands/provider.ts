      ...
      try {
        ...
        for (const [provider, healthy] of healthMap) {
          ...
          const circuit = circuitState
            ? chalk[circuitState.state === 'closed' ? 'green' : circuitState.state === 'open' ? 'red' : 'yellow'](
                circuitState.state.toUpperCase()
              )
            : chalk.gray('Unknown');
  ...
  stats
    ...
    .action(async (options) => {
      ...
      try {
        ...
        // Summary
        const totalRequests = stats.reduce((sum: number, s: any) => sum + s.total_requests, 0);
        const totalTokens = stats.reduce((sum: number, s: any) => sum + s.total_tokens, 0);
  ...
  logs
    ...
    .action(async (options) => {
      ...
      try {
        ...
        // Apply filters
        if (options.provider) {
          logs = logs.filter((log: any) => log.provider === options.provider);
        }

        if (options.failed) {
          logs = logs.filter((log: any) => log.state === 'failed');
        }
 ...
 */
function createCircuitBreakerCommand(): Command {
  ...
  circuit
    ...
    .action((provider) => {
      try {
        providerService.resetCircuitBreaker(provider as any);
  ...
  test
    ...
    .action(async (provider, options) => {
      ...
      try {
        ...
        if (options.stream) {
          let chunks = 0;
          await providerService.sendStreamingRequest(
            request,
            {
              onChunk: () => {
                chunks++;
