...
export class OnboardingManager extends EventEmitter {
   ...
   */
  private registerBuiltInFlows(): void {
    // Quick Start Flow
    this.registerFlow({
      id: 'quick-start',
  ...
  getStats(): OnboardingStats {
    ...
    const averageCompletionTime =
      completedTimes.length > 0 ? completedTimes.reduce((a, b) => a + b, 0) / completedTimes.length : 0
 ...
 */
let globalManager: OnboardingManager | null = null

/**
 * Get global onboarding manager
 */
export function getGlobalOnboardingManager(): OnboardingManager {
  if (!globalManager) {
 ...
 */
export function resetGlobalOnboardingManager(): void {
  globalManager = null
}
