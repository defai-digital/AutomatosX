...
export class TenantService extends EventEmitter {
    ...
    async createTenant(name, slug, plan = 'free') {
    ...
    `).run(id, name, slug, plan, now, now);
        ...
        const tenant = { id, name, slug, plan, status: 'active', createdAt: now, updatedAt: now };
    ...
    async initializeQuotas(tenantId, plan) {
    ...
    `).run(tenantId, quotas.maxWorkflows, quotas.maxExecutionsPerMonth, quotas.maxConcurrentExecutions, quotas.maxStorageBytes, quotas.maxApiCallsPerMonth, now);
    }
