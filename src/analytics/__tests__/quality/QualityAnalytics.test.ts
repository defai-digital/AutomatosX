...
import {
  ...
  CognitiveAnalyzer,
  HalsteadAnalyzer,
} from '../../quality/ComplexityAnalyzer.js';
...
async function createTempFile(content: string, filename: string = 'test.ts'): Promise<string> {
...
describe('MaintainabilityCalculator', () => {
  let calculator: MaintainabilityCalculator;
  let analyzer: ComplexityAnalyzer;
  ...
  it('should detect high complexity code smell', async () => {
    ...
    try {
      ...
      const complexitySmells = maintainability.codeSmells.filter(
        s => s.type === CodeSmellType.HighComplexity
      );
  ...
  it('should detect long function code smell', async () => {
    ...
    try {
      ...
      const longFunctionSmells = maintainability.codeSmells.filter(
        s => s.type === CodeSmellType.LongFunction
      );
  ...
  it('should calculate technical debt', async () => {
    ...
    try {
      ...
      expect(maintainability.technicalDebt.totalHours).toBe(
        maintainability.technicalDebt.totalMinutes / 60
      );
