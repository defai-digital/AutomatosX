...
export class WorkflowProviderBridge extends EventEmitter {
     ...
     */
    async executeStep(step, prompt, context) {
     ...
     */
    async callProvider(config, prompt, context) {
    ...
    async executeBatch(steps) {
        const promises = steps.map(({ step, prompt, context }) => this.executeStep(step, prompt, context));
...
//# sourceMappingURL=WorkflowProviderBridge.js.map