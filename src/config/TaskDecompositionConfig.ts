...
let cachedConfig: TaskDecompositionConfig | null = null;

/**
 * Load task decomposition rules from YAML configuration (with caching)
 */
export function loadTaskDecompositionConfig(): TaskDecompositionConfig {
  ...
  try {
    ...
    const config = YAML.parse(yamlContent) as TaskDecompositionConfig;
 ...
 */
export function clearTaskDecompositionConfigCache(): void {
  cachedConfig = null;
}
 ...
 */
function getDefaultTaskDecompositionConfig(): TaskDecompositionConfig {
  return {
    version: '1.0',
    rules: [
      {
