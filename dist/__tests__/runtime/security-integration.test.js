...
describe('Security Stack Integration - Threat T2 + T4 + T5', () => {
    it('enforces complete security pipeline for task lifecycle', () => {
        ...
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, rawMetadata);
        ...
        const authGuard = (ctx) => 'Pass';
        const rateLimitGuard = (ctx) => 'Pass';
        const guards = [
        ...
        const [newLimiter, cancellationResult] = CancellationLimiter.checkCancellation(limiter, 'task-<script>alert(1)</script>', // Use sanitized taskId
        userId, 'Preparing', 'NotRequired');
    ...
    it('prevents multi-vector attack combining metadata injection + guard bypass + cancellation flood', () => {
        ...
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, attackMetadata);
        ...
        const whitelisted = MetadataValidator.whitelistFields(MetadataValidator.taskMetadataSchema, attackMetadata);
        ...
        for (let i = 0; i < 30; i++) {
            const [newLimiter, result] = CancellationLimiter.checkCancellation(currentLimiter, `task-${i}`, attackerId, 'Idle', 'NotRequired');
    ...
    it('maintains security boundaries in concurrent task execution scenario', () => {
        ...
        for (let i = 0; i < 10; i++) {
            const [newLimiter, result] = CancellationLimiter.checkCancellation(limiterState, `user1-task-${i}`, 'user1@example.com', 'Idle', 'NotRequired');
            limiterState = newLimiter;
        }
        // User 1's 11th cancellation should be rate limited
        const [limiter1, result1Cancel] = CancellationLimiter.checkCancellation(limiterState, 'user1-task-11', 'user1@example.com', 'Idle', 'NotRequired');
        expect(result1Cancel.TAG).toBe('RateLimitExceeded');
        // User 2 should still have full token bucket (independent rate limit)
        const [limiter2, result2Cancel] = CancellationLimiter.checkCancellation(limiter1, 'user2-task-1', 'user2@example.com', 'Idle', 'NotRequired');
    ...
    it('enforces dual confirmation with metadata validation for EXECUTING state', () => {
        ...
        // Validate metadata
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, metadata);
        expect(validationReport.valid).toBe(true);
        ...
        const [limiter1, result1] = CancellationLimiter.checkCancellation(limiter, 'executing-task-123', userId, 'Executing', confirmation1);
        ...
        const [limiter2, result2] = CancellationLimiter.checkCancellation(limiter1, 'executing-task-123', userId, 'Executing', confirmation2);
    ...
    it('provides complete audit trail across all security layers', () => {
        ...
        const guards = [
            ...
            [(ctx) => ({ TAG: 'Defer', _0: 'Async check pending' }), 'dependency-guard']
        ];
        const guardResults = GuardIsolation.executeGuardsBatch(guards, ctx, mockSecretKey);
        ...
        let currentLimiter = limiter;
        for (let i = 0; i < 5; i++) {
            const [newLimiter, result] = CancellationLimiter.checkCancellation(currentLimiter, `audit-test-task-${i}`, userId, 'Idle', 'NotRequired');
            currentLimiter = newLimiter;
        }
    ...
    it('enforces size limits to prevent resource exhaustion attacks', () => {
        ...
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, oversizedMetadata);
        ...
        const fieldValidationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, longFieldMetadata);
    ...
    it('maintains performance under high load while enforcing security', () => {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            // Cancellation check
            const [newLimiter, cancellationResult] = CancellationLimiter.checkCancellation(currentLimiter, `task-${i}`, `user${i % 10}@example.com`, 'Preparing', 'NotRequired');
            currentLimiter = newLimiter;
            results.push({
...
describe('Edge Cases and Error Handling', () => {
    it('handles null and undefined metadata gracefully', () => {
        ...
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, nullMetadata);
        // Should handle null values
        expect(validationReport).toBeTruthy();
    });
    it('handles empty metadata object', () => {
        const emptyMetadata = {};
        const validationReport = MetadataValidator.validateMetadata(MetadataValidator.taskMetadataSchema, emptyMetadata);
    ...
    it('handles concurrent guard execution with shared context', () => {
        ...
        const guard = (ctx) => 'Pass';
        const results = Array.from({ length: 5 }, (_, i) => GuardIsolation.executeGuardIsolated(guard, `guard-${i}`, ctx, mockSecretKey));
