...
export class HotReloadEngine extends EventEmitter {
  ...
  private pluginStates = new Map<string, PluginStateSnapshot>()
   ...
   * Save plugin state
   */
  private async savePluginState(pluginName: string): Promise<void> {
   ...
   */
  private async restorePluginState(pluginName: string): Promise<void> {
   ...
   * Unload plugin
   */
  private async unloadPlugin(pluginName: string): Promise<string | undefined> {
    const plugin = this.loadedPlugins.get(pluginName)
 ...
 */
export function createHotReloadEngine(): HotReloadEngine {
 ...
 */
export function getHotReloadEngine(): HotReloadEngine {
  if (!globalEngine) {
 ...
 * Reset global engine
 */
export function resetHotReloadEngine(): void {
  if (globalEngine) {
