...
3. Define persona details, specialties, delegation limits, and timeouts; align config keys with `automatosx.config.json`.
4. Document new capabilities in `docs/` and sync sample configs when introducing new options.

## Configuration Essentials
- Provider priorities and limits reside in `automatosx.config.json`; regenerate type-safe accessors via `npm run prebuild:config`.
