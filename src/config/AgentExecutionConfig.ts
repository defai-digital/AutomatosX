...
let cachedConfig: AgentExecutionConfig | null = null;

/**
 * Load agent execution configuration from YAML (with caching)
 */
export function loadAgentExecutionConfig(): AgentExecutionConfig {
  ...
  try {
    ...
    const config = YAML.parse(yamlContent) as AgentExecutionConfig;
 ...
 */
export function clearAgentExecutionConfigCache(): void {
  cachedConfig = null;
}
 ...
 */
function getDefaultAgentExecutionConfig(): AgentExecutionConfig {
  return {
    version: '1.0',
    maxRetries: 3,
    timeoutMs: 300000,
