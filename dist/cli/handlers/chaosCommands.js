...
import { ChaosEnableSchema, ChaosDisableSchema, ChaosStatusSchema, ChaosTestSchema, } from '../schemas/ChaosCommandSchema.js';
 ...
 * ```
 */
export async function chaosEnableCommand(rawArgs) {
 ...
 * ```
 */
export async function chaosDisableCommand(rawArgs) {
    const logger = new StreamingLogger({ minLevel: 'info' });
    try {
 ...
 * ```
 */
export async function chaosStatusCommand(rawArgs) {
    const logger = new StreamingLogger({ minLevel: 'info' });
    try {
        ...
        if (args.stats) {
            ...
            logger.info(`Total events: ${summary.totalEvents}`);
            logger.info(`Failures injected: ${summary.eventsInjected}`);
            logger.info(`Actual failure rate: ${(summary.failureRate * 100).toFixed(1)}%`);
            logger.info('\n📋 Scenario Counts:');
        ...
        if (args.events) {
            ...
            events.slice(-10).forEach((event, index) => {
                logger.info(`  ${index + 1}. [${event.type}] ${event.target} - ${event.injected ? '❌ FAILED' : '✅ OK'}`);
            });
        }
        if (args.json) {
            console.log(JSON.stringify({
 ...
 * ```
 */
export async function chaosTestCommand(rawArgs) {
    const logger = new StreamingLogger({ minLevel: 'info' });
    try {
        ...
        const rateMatch = Math.abs(actualFailureRate - expectedFailureRate) <= tolerance;
        if (rateMatch) {
...
//# sourceMappingURL=chaosCommands.js.map