...
export class PatternDetector {
  ...
  private async detectFactory(): Promise<DetectedPattern[]> {
    ...
    const factories = results.filter(
      (r) =>
  ...
  private async detectDependencyInjection(): Promise<DetectedPattern[]> {
    ...
    const diPatterns = results.filter(
      (r) =>
        r.content.includes('constructor(') &&
        (r.content.includes('private') ||
          r.content.includes('readonly') ||
          r.content.includes('@inject'))
    );

    if (diPatterns.length === 0) return [];
  ...
  private async detectObserver(): Promise<DetectedPattern[]> {
    ...
    const observers = results.filter(
      (r) =>
        r.content.includes('addEventListener') ||
   ...
   */
  private async detectAdapter(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('adapter');
    const adapters = results.filter(
      (r) =>
        r.content.includes('Adapter') ||
   ...
   */
  private async detectDecorator(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('decorator');
    const decorators = results.filter(
      (r) =>
        r.content.includes('@') ||
   ...
   */
  private async detectBuilder(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('builder');
    const builders = results.filter(
      (r) =>
        r.content.includes('Builder') ||
   ...
   */
  private async detectLayeredArchitecture(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('service');
    const layers = results.filter(
      (r) =>
        r.content.includes('Service') ||
   ...
   */
  private async detectEventDriven(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('event');
    const events = results.filter(
      (r) =>
        r.content.includes('EventEmitter') ||
   ...
   */
  private async detectMicroservices(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('service');
    const services = results.filter(
      (r) =>
        r.content.includes('microservice') ||
   ...
   * Detect CQRS pattern
   */
  private async detectCQRS(): Promise<DetectedPattern[]> {
    const results = await this.searchCode('command');
    const cqrs = results.filter(
      (r) =>
        (r.content.includes('Command') && r.content.includes('Query')) ||
        r.content.includes('CommandHandler') ||
        r.content.includes('QueryHandler')
    );

    if (cqrs.length === 0) return [];
