  ...
  describe('SqliteEventStore', () => {
    ...
    describe('INV-MEM-004: Version Ordering', () => {
      it('should enforce version ordering', async () => {
        ...
        try {
          ...
          expect((err as SqliteEventStoreError).code).toBe(
      ...
      it('should return events in version order', async () => {
        ...
        expect(retrieved.map((e) => (e.payload as { key: string }).key)).toEqual([
          'a',
          'b',
          'c',
        ]);
    ...
    describe('Version Tracking', () => {
      it('should track aggregate versions correctly', async () => {
        ...
        await store.append(
          createMemoryEvent('memory.stored', { key: 'a' }, { aggregateId: 'new-agg', version: 1 })
        );
        expect(await store.getVersion('new-agg')).toBe(1);

        await store.append(
          createMemoryEvent('memory.stored', { key: 'b' }, { aggregateId: 'new-agg', version: 2 })
        );
  ...
  describe('SqliteTraceStore', () => {
    ...
    describe('Event Attributes', () => {
      it('should preserve all event attributes', async () => {
        ...
        (event as { durationMs: number }).durationMs = 150;
