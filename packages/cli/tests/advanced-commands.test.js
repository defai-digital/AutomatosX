...
import { abilityCommand, agentCommand, callCommand, cleanupCommand, configCommand, guardCommand, feedbackCommand, listCommand, mcpCommand, sessionCommand, setupCommand, statusCommand, } from '../src/commands/index.js';
...
describe('advanced retained commands', () => {
    ...
    it('serves MCP JSON-RPC over stdio and handles initialize + tools/list + tools/call', async () => {
        ...
        expect(initResp?.result?.serverInfo?.name).toBe('automatosx');
        const listResp = responses.find((r) => r.id === 2);
        const listTools = listResp?.result?.tools ?? [];
        ...
        const callContent = callResp?.result?.content ?? [];
        const parsed = JSON.parse(callContent[0]?.text ?? '[]');
    ...
    it('preserves an explicit empty summary when completing a session', async () => {
        ...
        expect(completed.success).toBe(true);
        expect(completed.data.summary).toBe('');
    });
