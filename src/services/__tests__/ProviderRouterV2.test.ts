...
import { ProviderRouterV2, createProviderRouter, type ProviderRouterOptions } from '../ProviderRouterV2.js'
import { ProviderAuthError, ProviderRateLimitError } from '../../providers/ProviderBase.js'
...
describe('ProviderRouterV2', () => {
  ...
  describe('initialization', () => {
    ...
    it('should initialize only enabled providers', () => {
      const config: ProviderRouterOptions = {
        providers: {
  ...
  describe('fallback mechanism', () => {
    it('should fallback to next provider on failure', async () => {
      const executeRequestSpy = vi.spyOn(router as any, 'executeRequest')
  ...
  describe('health metrics', () => {
    ...
    it('should update health on failed request', async () => {
      vi.spyOn(router as any, 'executeRequest').mockRejectedValue(new Error('Request failed'))
    ...
    it('should track requests per minute', async () => {
      vi.spyOn(router as any, 'executeRequest').mockResolvedValue({
        provider: 'claude',
        model: 'claude-sonnet-4-5-20250929',
        content: 'Response',
        tokensUsed: 50,
  ...
  describe('chaos mode', () => {
    it('should randomly fail requests in chaos mode', async () => {
      ...
      await expect(
        chaosRouter.request({ prompt: 'Test' })
      ).rejects.toThrow('Chaos mode')
    })
    ...
    it('should succeed some requests in chaos mode', async () => {
      ...
      vi.spyOn(chaosRouter as any, 'executeRequest').mockResolvedValueOnce({
        provider: 'claude',
        model: 'claude-sonnet-4-5-20250929',
        content: 'Response',
        tokensUsed: 50,
  ...
  describe('event emission', () => {
    ...
    it('should emit error event on failures', async () => {
      ...
      vi.spyOn(router as any, 'executeRequest').mockRejectedValue(new Error('Test error'))
  ...
  describe('health checks', () => {
    it('should perform health checks on all providers', async () => {
      ...
      const geminiProvider = (router as any).providers.get('gemini')
