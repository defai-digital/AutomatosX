...
export class IterateEngine {
  ...
  constructor(
    ...
    strategyMode: 'auto' | 'conservative' | 'aggressive' = 'auto'
  ) {
    ...
    for (let i = 1; i <= options.maxIterations; i++) {
      ...
      if (options.checkpointInterval && i % options.checkpointInterval === 0) {
        try {
          ...
          this.log(options, chalk.yellow(`   ⚠️  Checkpoint failed: ${(error as Error).message}`));
        }
      }
      ...
      if (!result.success) {
        ...
        const nextStrategy = await this.strategySelector.selectNext(
          currentStrategy,
          analysis,
          iterations
        );
   ...
   */
  private captureProgress(result: any): ProgressSnapshot {
    if (!result) {
      return {
        totalSteps: 0,
        completedSteps: 0,
   ...
   */
  private calculateTotalCost(iterations: IterationResult[]): number {
   ...
   */
  private log(options: IterateOptions, message: string): void {
    console.log(message);
  }
