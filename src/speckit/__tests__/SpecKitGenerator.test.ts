...
This document provides a thorough analysis of the codebase architecture and patterns.`,
  ...
  describe('Caching', () => {
    ...
    it('should expire cache after TTL', async () => {
      ...
      const entry = (generator as any).cache.get(cacheKey);
      if (entry) {
  ...
  describe('AI Provider Integration', () => {
    it('should call AI provider with correct parameters', async () => {
      await (generator as any).callAI('Test prompt', {
        provider: 'claude',
      });
    ...
    it('should return AI generated content', async () => {
      const content = await (generator as any).callAI('Test prompt', {});
  ...
  describe('Code Search Integration', () => {
    it('should search code using MemoryService', async () => {
      const results = await (generator as any).searchCode('TestClass');
    ...
    it('should respect search limit option', async () => {
      await (generator as any).searchCode('TestClass', { limit: 5 });
  ...
  describe('Error Handling', () => {
    it('should handle analyze() errors gracefully', async () => {
      ...
      vi.spyOn(generator as any, 'analyze').mockRejectedValue(new Error('Analysis failed'));
    ...
    it('should handle detect() errors gracefully', async () => {
      vi.spyOn(generator as any, 'detect').mockRejectedValue(new Error('Detection failed'));
    ...
    it('should handle generateContent() errors gracefully', async () => {
      vi.spyOn(generator as any, 'generateContent').mockRejectedValue(new Error('Generation failed'));
    ...
    it('should handle validation failures', async () => {
      ...
      vi.spyOn(generator as any, 'validate').mockResolvedValue({
        valid: false,
  ...
  describe('Validation', () => {
    it('should reject empty content', async () => {
      const validation = await (generator as any).validate('', {});

      expect(validation.valid).toBe(false);
      expect(validation.errors.length).toBeGreaterThan(0);
      expect(validation.errors.some((e: any) => e.message.includes('empty'))).toBe(true);
    });

    it('should reject too short content', async () => {
      const validation = await (generator as any).validate('Short', {});
