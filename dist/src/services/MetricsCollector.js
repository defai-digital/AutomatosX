...
export class MetricsCollector {
        ...
        const insertMany = this.db.transaction((metrics) => {
            for (const metric of metrics) {
                ...
                if (metric.type === 'request') {
                    stmt.run(id, metric.timestamp, 'request', metric.provider, metric.model, metric.userId || null, metric.latency, metric.success ? 1 : 0, metric.errorMessage || null, metric.inputTokens || 0, metric.outputTokens || 0, metric.totalTokens || 0, metric.cost || 0, null, null, null, null, null, null, metric.metadata ? JSON.stringify(metric.metadata) : null, now);
                }
                else if (metric.type === 'cache') {
                    stmt.run(id, metric.timestamp, 'cache', metric.provider || null, null, null, null, null, null, null, null, null, null, metric.event, metric.savedCost || 0, metric.savedTokens || 0, null, null, null, null, now);
                }
                else if (metric.type === 'rate_limit') {
                    stmt.run(id, metric.timestamp, 'rate_limit', null, null, null, null, null, null, null, null, null, null, null, null, null, metric.event, metric.rateLimitType, metric.remaining || null, JSON.stringify({ key: metric.key }), now);
                }
            }
        });
        insertMany(batch);
    ...
    async query(filters = {}) {
        ...
        return results.map((row) => this.rowToMetric(row));
    }
    /**
    ...
    `;
        ...
        return results.map((row) => {
            ...
            const latencies = this.db
                .prepare(`SELECT latency FROM metrics_raw
           WHERE metric_type = 'request'
           AND provider = ?
           AND timestamp >= ?
           ORDER BY latency ASC`)
            ...
            const costTodayResult = this.db
                .prepare(`SELECT SUM(cost) as cost_today FROM metrics_raw
           WHERE metric_type = 'request'
           AND provider = ?
           AND timestamp >= ?`)
                .get(row.provider, startOfToday);
            const costToday = costTodayResult?.cost_today || 0;
            let status;
     ...
     */
    async getTimeSeries(bucketSize, filters = {}) {
