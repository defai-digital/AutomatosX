...
export class InfrastructureAgent extends AgentBase {
    ...
    buildInfrastructurePrompt(task, context, codeContext, pastSolutions) {
    ...
    parseInfrastructureArtifacts(response) {
        const artifacts = [];
        ...
        while ((match = codeBlockRegex.exec(response)) !== null) {
            ...
            if (language === 'hcl' || language === 'terraform') {
                if (content.includes('resource "aws_'))
                    name = 'terraform-aws';
                else if (content.includes('resource "google_'))
                    name = 'terraform-gcp';
                else if (content.includes('resource "azurerm_'))
                ...
                if (content.includes('apiVersion') && content.includes('kind:')) {
                    if (content.includes('kind: Deployment'))
                        name = 'k8s-deployment';
                    else if (content.includes('kind: Service'))
                        name = 'k8s-service';
                    else if (content.includes('kind: Ingress'))
                        name = 'k8s-ingress';
                    else if (content.includes('kind: ConfigMap'))
                        name = 'k8s-configmap';
                    else
                        name = 'k8s-manifest';
                }
...
//# sourceMappingURL=InfrastructureAgent.js.map