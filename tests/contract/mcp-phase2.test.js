...
import { ALL_TOOLS } from '../../packages/mcp-server/src/tools/index.js';
...
describe('INV-MCP-002: No Undeclared Side Effects', () => {
    ...
    it('should identify mutating tools by name pattern', () => {
        const mutatingTools = ALL_TOOLS.filter((tool) => MUTATING_TOOL_PATTERNS.some((pattern) => tool.name.toLowerCase().includes(pattern)));
    ...
    it('should identify read-only tools by name pattern', () => {
        const readOnlyTools = ALL_TOOLS.filter((tool) => READ_ONLY_TOOL_PATTERNS.some((pattern) => tool.name.toLowerCase().includes(pattern)));
    ...
    it('should categorize tools into mutating vs read-only', () => {
        const categorized = ALL_TOOLS.map((tool) => {
            const isMutating = MUTATING_TOOL_PATTERNS.some((p) => tool.name.toLowerCase().includes(p));
            const isReadOnly = READ_ONLY_TOOL_PATTERNS.some((p) => tool.name.toLowerCase().includes(p));
            return {
                name: tool.name,
    ...
    it('should have input schemas for most tools', () => {
        const toolsWithSchemas = ALL_TOOLS.filter((tool) => INPUT_SCHEMAS[tool.name] !== undefined);
...
describe('INV-MCP-004: Idempotency Declaration', () => {
    ...
    it('should identify non-idempotent tools correctly', () => {
        ...
        for (const toolName of NON_IDEMPOTENT_TOOLS) {
            if (actualTools.includes(toolName)) {
                ...
                expect(tool?.name.includes('create') ||
                    tool?.name.includes('add') ||
                    tool?.name.includes('run') ||
                    tool?.name.includes('inject')).toBe(true);
            }
        }
    });
    it('should have predictable behavior for list operations', () => {
        ...
        const listTools = ALL_TOOLS.filter((t) => t.name.includes('list') || t.name.includes('search'));
        expect(listTools.length).toBeGreaterThan(0);
        // Most list tools should have input schemas
        const listToolsWithSchemas = listTools.filter((tool) => INPUT_SCHEMAS[tool.name] !== undefined);
    ...
    it('should have predictable behavior for get operations', () => {
        const getTools = ALL_TOOLS.filter((t) => t.name.startsWith('get_') || t.name.includes('_get'));
        ...
        if (getTools.length > 0) {
            ...
            const getToolsWithSchemas = getTools.filter((tool) => INPUT_SCHEMAS[tool.name] !== undefined);
...
describe('INV-MCP-005: Input Isolation', () => {
    ...
    it('should validate input schemas preserve data integrity', () => {
        ...
        const toolsWithSchemas = ALL_TOOLS.filter((t) => INPUT_SCHEMAS[t.name] !== undefined);
...
describe('MCP Tool Invariants - Cross-Validation', () => {
    ...
    it('should categorize all tools by domain', () => {
        ...
        const categorized = ALL_TOOLS.filter((tool) => domains.some((domain) => tool.name.includes(domain)));
...
//# sourceMappingURL=mcp-phase2.test.js.map