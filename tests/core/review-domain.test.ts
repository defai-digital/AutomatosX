...
describe('Focus Modes', () => {
  describe('FOCUS_MODE_PROMPTS', () => {
    it('should have prompts for all focus modes', () => {
      const expectedModes: ReviewFocus[] = [
        'security',
        'architecture',
        'performance',
        'maintainability',
  ...
  describe('FOCUS_MODE_CATEGORIES', () => {
    ...
    it('should have security categories with OWASP-related items', () => {
      ...
      expect(securityCategories.some((c: string) => c.includes('injection'))).toBe(true);
    });
  });
...
describe('Comment Builder', () => {
  ...
  describe('filterCommentsByFocus', () => {
    const createComment = (category: string, focus: ReviewFocus): ReviewComment => ({
      commentId: '123',
      file: 'test.ts',
      line: 1,
      severity: 'warning',
  ...
  describe('filterCommentsByConfidence', () => {
    const createComment = (confidence: number): ReviewComment => ({
      commentId: '123',
      file: 'test.ts',
      line: 1,
      severity: 'warning',
...
describe('Markdown Formatter', () => {
  const createResult = (comments: ReviewComment[]): ReviewResult => ({
    resultId: 'result-1',
    requestId: 'request-1',
    comments,
    summary: {
  ...
  const createComment = (severity: 'critical' | 'warning' | 'suggestion' | 'note'): ReviewComment => ({
    commentId: '123',
    file: 'src/test.ts',
    line: 10,
    severity,
...
describe('SARIF Formatter', () => {
  ...
  const createComment = (): ReviewComment => ({
    commentId: '123',
    file: 'src/test.ts',
    line: 10,
    lineEnd: 15,
...
describe('Review Service', () => {
  ...
  describe('ReviewService', () => {
    let mockExecutor: ReviewPromptExecutor;
