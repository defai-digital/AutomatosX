...
import { createQueryCache } from '../../optimization/QueryCache.js';
describe('QueryCache', () => {
    let cache;
    beforeEach(() => {
        cache = createQueryCache({ maxSize: 5, ttl: 100 });
    });
    ...
    describe('TTL Expiration', () => {
        it('should expire entries after TTL', async () => {
            const shortTTL = createQueryCache({ ttl: 50 });
        ...
        it('should only clean expired entries, not fresh ones', async () => {
            const cache = createQueryCache({ ttl: 100 });
    ...
    describe('LRU Eviction', () => {
        it.skip('should evict least recently used when at capacity', () => {
            ...
            const lruCache = createQueryCache({ maxSize: 5, ttl: 10000 });
        ...
        it('should call onEvict callback when evicting', () => {
            const onEvict = vi.fn();
            const cache = createQueryCache({ maxSize: 2, onEvict });
            cache.set('key1', 'value1');
            cache.set('key2', 'value2');
    ...
    describe('Access Tracking', () => {
        ...
        it('should update lastAccessed timestamp on get', async () => {
            const trackCache = createQueryCache({ ttl: 10000 });
    ...
    describe('TTL Management', () => {
        ...
        it.skip('should apply new TTL to future entries only', async () => {
            const ttlCache = createQueryCache({ ttl: 50 });
    ...
    describe('Edge Cases', () => {
        it('should handle maxSize of 1', () => {
            const tiny = createQueryCache({ maxSize: 1 });
