...
import { 
// Store
createConfigStore, expandPath, getConfigPath, CONFIG_PATHS, CONFIG_SUBDIRS, 
// Operations
parsePath, getValue, setValue, removeValue, mergeConfigs, diffConfigs, hasPath, getAllPaths, createConfigAggregate, createAggregateFromConfig, 
// Repository
InMemoryConfigEventStore, createConfigRepository, getConfigRepository, setConfigRepository, resetConfigRepository, 
// Migrator
CURRENT_VERSION, getConfigVersion, compareVersions, isVersionSupported, getMigrationPath, needsMigration, migrateConfig, safeMigrateConfig, getMigrationInfo, 
// Detection Port
setDetectionAdapter, getDetectionAdapter, resetDetectionAdapter, nullDetectionAdapter, } from '@defai.digital/config-domain';
import { DEFAULT_CONFIG } from '@defai.digital/contracts';
describe('Config Domain', () => {
    describe('Config Operations', () => {
        ...
        describe('setValue', () => {
            ...
            it('should set nested value', () => {
                const config = { features: { enableTracing: true } };
        ...
        describe('removeValue', () => {
            it('should remove top-level value', () => {
                const config = { logLevel: 'info', telemetryEnabled: false };
            ...
            it('should remove nested value', () => {
                const config = { features: { enableTracing: true, enableGuard: false } };
        ...
        describe('hasPath', () => {
            const config = { logLevel: 'info', features: { enableTracing: true } };
        ...
        describe('mergeConfigs', () => {
            it('should merge two configs', () => {
                const base = { logLevel: 'info', telemetryEnabled: false };
                const override = { logLevel: 'debug' };
                const result = mergeConfigs(base, override);
                expect(result.logLevel).toBe('debug');
                expect(result.telemetryEnabled).toBe(false);
            });
            it('should deep merge nested objects', () => {
                const base = { features: { enableTracing: true, enableGuard: false } };
                const override = { features: { enableGuard: true } };
                const result = mergeConfigs(base, override);
        ...
        describe('diffConfigs', () => {
            ...
            it('should return differences', () => {
                const a = { ...DEFAULT_CONFIG, logLevel: 'info' };
                const b = { ...DEFAULT_CONFIG, logLevel: 'debug' };
                const diff = diffConfigs(a, b);
    ...
    describe('createAggregateFromConfig', () => {
        it('should create aggregate from existing config', () => {
            const correlationId = crypto.randomUUID();
            const config = { ...DEFAULT_CONFIG, logLevel: 'debug' };
            ...
            const events = [
                {
                    ...
                    correlationId: crypto.randomUUID(),
                    type: 'config.created',
                    payload: { type: 'created', scope: 'global', config: DEFAULT_CONFIG },
                },
                {
                    eventId: crypto.randomUUID(),
                    timestamp: new Date().toISOString(),
                    ...
                    correlationId: crypto.randomUUID(),
                    type: 'config.updated',
                    payload: { type: 'updated', scope: 'global', path: 'logLevel', oldValue: 'info', newValue: 'debug' },
                },
            ];
...
//# sourceMappingURL=config-domain.test.js.map