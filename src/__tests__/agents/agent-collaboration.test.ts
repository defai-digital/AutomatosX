...
import { AgentCollaborator, SubTask, WorkflowResult } from '../../agents/AgentCollaborator.js';
import { TaskRouter } from '../../agents/TaskRouter.js';
import { Task, AgentContext, TaskStatus } from '../../types/agents.types.js';
...
function createMockTask(description: string, keywords?: string[]): Task {
  return {
    id: `task-${Date.now()}`,
    description,
...
describe('AgentCollaborator', () => {
  ...
  describe('Event Emissions', () => {
    it('should emit collaboration:start event', async () => {
      ...
      expect(startSpy).toHaveBeenCalledWith(
        expect.objectContaining({ task, strategy: 'auto' })
      );
    });
  ...
  describe('Error Handling', () => {
    it('should handle agent not found', async () => {
      ...
      vi.spyOn(collaborator as any, 'decomposeTask').mockResolvedValue([
        {
          id: 'test-1',
          description: 'Test',
