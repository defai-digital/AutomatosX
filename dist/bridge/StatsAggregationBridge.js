 ...
 */
async function getConversationStatsReScript(db, conversationId) {
    ...
    try {
        ...
        if (getFeatureFlags().global.enablePerformanceTracking) {
            console.log(`[ReScript] getConversationStats took ${(endTime - startTime).toFixed(2)}ms`);
        }
        return stats;
    }
    catch (error) {
 ...
 */
async function getGlobalStatsReScript(db) {
    const startTime = performance.now();
    try {
        // Build SQL query using ReScript
        ...
        const endTime = performance.now();
        if (getFeatureFlags().global.enablePerformanceTracking) {
            console.log(`[ReScript] getGlobalStats took ${(endTime - startTime).toFixed(2)}ms`);
        }
        return stats;
    }
    catch (error) {
 ...
 */
async function getTimeRangeStatsReScript(db, startTime, endTime) {
    ...
    try {
        ...
        if (getFeatureFlags().global.enablePerformanceTracking) {
            console.log(`[ReScript] getTimeRangeStats took ${(perfEnd - perfStart).toFixed(2)}ms`);
        }
        return stats;
    }
    catch (error) {
 ...
 */
async function getConversationStatsTypeScript(db, conversationId) {
    const startTime = performance.now();
    // ❌ BUG #13: Fetches ALL messages (could be millions!)
    const messages = db
        .prepare(`SELECT role, tokens FROM messages
    ...
    const endTime = performance.now();
    if (getFeatureFlags().global.enablePerformanceTracking) {
        console.log(`[TypeScript] getConversationStats took ${(endTime - startTime).toFixed(2)}ms (fetched ${messageCount} messages)`);
    }
    return {
        conversationId,
        messageCount,
 ...
 */
async function getGlobalStatsTypeScript(db) {
    const startTime = performance.now();
    ...
    const endTime = performance.now();
    if (getFeatureFlags().global.enablePerformanceTracking) {
        console.log(`[TypeScript] getGlobalStats took ${(endTime - startTime).toFixed(2)}ms (fetched ${totalMessages} messages)`);
    }
    return {
        totalConversations,
        totalMessages,
 ...
 */
async function getTimeRangeStatsTypeScript(db, startTime, endTime) {
    const perfStart = performance.now();
    // ❌ Fetches ALL messages in range
    const messages = db
        .prepare(`SELECT tokens FROM messages
    ...
    const perfEnd = performance.now();
    if (getFeatureFlags().global.enablePerformanceTracking) {
        console.log(`[TypeScript] getTimeRangeStats took ${(perfEnd - perfStart).toFixed(2)}ms (fetched ${messageCount} messages)`);
    }
    return {
        startTime,
        endTime,
 ...
 */
export async function getConversationStats(db, conversationId, userId) {
 ...
 */
export async function getGlobalStats(db, userId) {
    const useReScript = shouldUseReScript('statsAggregation', userId);
    logFeatureFlagDecision('statsAggregation', useReScript, userId);
    if (!useReScript) {
 ...
 */
export async function getTimeRangeStats(db, startTime, endTime, userId) {
    const useReScript = shouldUseReScript('statsAggregation', userId);
    logFeatureFlagDecision('statsAggregation', useReScript, userId);
    if (!useReScript) {
 ...
 */
export function selectAggregationStrategy(estimatedRows) {
    return StatsAggregation.selectStrategy(estimatedRows);
}
/**
 * Get explanation of aggregation strategy
 */
export function explainAggregationStrategy(strategy) {
    return StatsAggregation.explainStrategy(strategy);
}
// ============================================================================
 ...
 */
export async function comparePerformance(db, conversationId) {
...
//# sourceMappingURL=StatsAggregationBridge.js.map