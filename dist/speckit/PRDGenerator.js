...
export class PRDGenerator extends SpecKitGenerator {
    ...
    constructor(providerRouter, memoryService) {
        ...
        this.featureDetector = new FeatureDetector(this.searchCode.bind(this));
    }
    /**
    ...
    async detect(analysis, options) {
        ...
        const features = analysis.patterns.map(p => ({
            name: p.name,
            type: 'core',
     ...
     */
    async generateContent(features, analysis, options) {
     ...
     */
    buildPRDPrompt(features, analysis, options) {
