...
export async function iterateCommand(args, options) {
    if (args.length === 0 || args[0] === 'help' || options.help) {
        return success('Usage: ax iterate <command> [args...]\n\n' +
            'Commands:\n' +
    ...
    if (handler === undefined) {
        return failure(`Unknown command for iterate: "${subcommand}"\n` +
            'Supported: run\n' +
    ...
    for (let attempt = 1; attempt <= maxIterations; attempt++) {
        if (Date.now() >= deadline) {
            return failure(`Iterate time budget exceeded after ${attempt - 1} attempt(s). Last result: ${lastResult?.message ?? 'none'}`, { attempts: attempt - 1, lastResult });
        }
        lastResult = await handler(subArgs, options);
        if (lastResult.success) {
            return success(`Succeeded on attempt ${attempt}/${maxIterations}.\n${lastResult.message ?? ''}`.trim(), { attempts: attempt, lastResult });
        }
        const remaining = deadline - Date.now();
        if (remaining <= 0)
            break;
        if (attempt < maxIterations) {
            process.stderr.write(`[iterate] Attempt ${attempt}/${maxIterations} failed: ${lastResult.message ?? 'unknown error'}. Retrying...\n`);
        }
    }
    return failure(`All ${maxIterations} attempt(s) failed. Last error: ${lastResult?.message ?? 'unknown'}`, { attempts: maxIterations, lastResult });
}
