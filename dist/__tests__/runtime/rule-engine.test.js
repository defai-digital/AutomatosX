...
describe('Rule Engine - Policy Condition Evaluation', () => {
    it('evaluates StateIs condition correctly', () => {
        ...
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, undefined, undefined);
    ...
    it('evaluates MetadataHas condition when field exists', () => {
        ...
        metadata['taskId'] = '123';
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, metadata, undefined, undefined);
        const result = RuleEngine.evaluateCondition(condition, context);
        expect(result).toBe(true);
    });
...
describe('Rule Engine - Rule Evaluation', () => {
    it('passes rule when conditions are met and action is Allow', () => {
        const rule = PolicyDSL.createPolicyRule('allow-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, undefined, undefined);
    ...
    it('fails rule when conditions are met and action is Deny', () => {
        const rule = PolicyDSL.createPolicyRule('deny-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.denyAction('state is idle, operation not allowed'));
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, undefined, undefined);
        const result = RuleEngine.evaluateRule(rule, context);
    ...
    it('skips rule when conditions are not met', () => {
        const rule = PolicyDSL.createPolicyRule('skip-rule', [PolicyDSL.stateIsCondition('EXECUTING')], PolicyDSL.denyAction('should not reach here'));
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, undefined, undefined);
        const result = RuleEngine.evaluateRule(rule, context);
    ...
    it('evaluates Require action when metadata satisfies requirement', () => {
        const rule = PolicyDSL.createPolicyRule('require-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.requireAction('authToken'));
        const metadata = Object.create(null);
...
describe('Rule Engine - Policy Execution', () => {
    it('executes policy with multiple passing rules', () => {
        const rule1 = PolicyDSL.createPolicyRule('rule-1', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const rule2 = PolicyDSL.createPolicyRule('rule-2', [PolicyDSL.eventIsCondition('DEPS_READY')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('test-policy', '1.0.0', 'Test policy with multiple rules', [rule1, rule2]);
    ...
    it('stops execution at first failing rule', () => {
        const rule1 = PolicyDSL.createPolicyRule('rule-1', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const rule2 = PolicyDSL.createPolicyRule('rule-2', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.denyAction('access denied'));
        const rule3 = PolicyDSL.createPolicyRule('rule-3', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('test-policy', '1.0.0', 'Test policy with failing rule', [rule1, rule2, rule3]);
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, policy, undefined);
        const outcome = RuleEngine.execute(context);
    ...
    it('converts evaluation outcome to string for logging', () => {
        const rule = PolicyDSL.createPolicyRule('test-rule', [PolicyDSL.stateIsCondition('IDLE')], PolicyDSL.allowAction());
        const policy = PolicyDSL.createPolicy('test-policy', '1.0.0', 'Test policy', [rule]);
        const context = RuleEngine.createExecutionContext(runtimeStates.idle, runtimeEvents.dependenciesReady, undefined, policy, undefined);
        const outcome = RuleEngine.execute(context);
