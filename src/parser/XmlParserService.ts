...
export class XmlParserService extends BaseLanguageParser {
   ...
   */
  private extractElement(node: Parser.SyntaxNode): Symbol | null {
   ...
   */
  private extractRootProject(node: Parser.SyntaxNode): Symbol | null {
    const startTagNode = node.childForFieldName('start_tag');
    if (!startTagNode) return null;
   ...
   */
  private findChildElementText(node: Parser.SyntaxNode, elementName: string): string | undefined {
   ...
   */
  private extractTextContent(node: Parser.SyntaxNode): string {
   ...
   */
  private extractAttributes(startTagNode: Parser.SyntaxNode): string[] {
   ...
   */
  private extractAttributesAsMap(startTagNode: Parser.SyntaxNode): Map<string, string> {
